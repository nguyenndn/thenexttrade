...
## Admin Routes & Operations Console

### `/admin/users/behavior`

Purpose:

- User Behavior Segmentation & Operations Console (Doc #2).
- Classify users into actionable segments to prioritize support and outreach:
  - Tab A: **Needs Attention** (Urgency score, Active signals e.g. TILT / REVENGE / DRAWDOWN / WIN_STREAK, Contact links via Telegram / Email, Impersonate action).
  - Tab B: **Value Radar** (High IB value traders, active real account telemetry, trading tempo, lot size trend, stayed-after-loss loyalty indicator, product adoption).
...
- Tab selection (`tab=attention` | `tab=value`).
- Filter params: `risk` (ALL | HIGH | MEDIUM | LOW), `urgency` (high | all), `product` (ALL | JOURNAL | COACH | PLAN | EXPERIMENT).
- Pagination (`page`, `pageSize`).

Expected behavior:

- Strictly gated behind `requireAdminPageAccess()` (`ADMIN` or `SUPERADMIN`).
- Urgency score computed dynamically from active Coach activation signals, risk severity, and inactivity window.
- Quick action links to user detail (`/admin/users/[id]`), one-click impersonation with audit log, and direct outreach.

Code paths:

- `src/app/admin/users/behavior/page.tsx`
- `src/app/admin/users/behavior/client.tsx`
- `src/app/admin/users/behavior/actions.ts`
- `src/lib/admin/behavior/classify.server.ts`
- `src/lib/admin/behavior/activity.server.ts`

---

### `/admin/users/[id]` — Expanded 8-Domain Telemetry & Narrative Summary

Purpose:

- Full-spectrum trader inspection & support console (Doc #1 & Doc #4).
- Provide administrative read-only visibility into the entire user trading loop.
...
1. **User Narrative Summary (Hero)**: On-the-fly behavioral synopsis (`classifyUser`), active risk chip badges, trading tempo, product adoption, and value indicator. Strict `persist: false` guarantee (zero DB writes on view).
2. **Overview**: Account info, profile, Telegram, notes, registration source.
3. **Access & VIP**: VIP tiers, trial expiration, manual override, audit logs.
4. **Broker & IB**: Linked MT5 accounts, broker detection, and interactive `AccountIbAttributionBadge` (`CONFIRMED`, `NOT_OURS`, `UNKNOWN`).
5. **Journal & Trades**: Filterable trade table, Net P&L, Win Rate %, Trade Telemetry Inspector modal with entry/exit, SL/TP, swap, and emotion logs.
6. **Plans & Playbooks**: Pre-trade execution plans, Plan vs Actual execution comparison modal, and Playbook strategies library.
7. **Rules & Goals**: Active trading rules, severity matrix, behavior goals, and historical trade rule check evaluations.
8. **Growth & Coach**: Edge XP event telemetry, Mission progression, 10-Trade Improvement Experiments, and AI Coach weekly action checklists.
9. **Reports & Notes**: Weekly/monthly trading reports, calendar day reflection notes, and algorithmic leak radar insight snapshots.

Code paths:

- `src/app/admin/users/[id]/page.tsx`
- `src/components/admin/users/UserNarrativeSummary.tsx`
- `src/components/admin/users/AccountIbAttributionBadge.tsx`
- `src/app/admin/users/[id]/UserJournalTab.tsx`
- `src/app/admin/users/[id]/UserTradePlansTab.tsx`
- `src/app/admin/users/[id]/UserRulesGoalsTab.tsx`
- `src/app/admin/users/[id]/UserGrowthCoachTab.tsx`
- `src/app/admin/users/[id]/UserReportsNotesTab.tsx`

---

### `/admin/trading-systems/brokers` — Commission Rates & IB Revenue Pipeline

Purpose:

- Broker Commission Rates Configuration & IB Revenue Pipeline (Doc #3).
- Configure per-broker and per-symbol commission rates to compute accurate IB gross revenue and track Out-of-IB active traders.
...
- Tabbed management: Brokers list & **Commission Rates** tab.
- Add, edit, or delete `BrokerCommissionRate` records with audit logging.
- Wildcard `*` symbol support with default fallback.
- Dynamic IB gross revenue recalculation based on `CONFIRMED` IB accounts and eligible trade journals (excluding demo/paper/manual syncs).

Code paths:

- `src/app/admin/trading-systems/brokers/page.tsx`
- `src/app/admin/trading-systems/brokers/EABrokersTabsClient.tsx`
- `src/app/admin/trading-systems/brokers/CommissionRatesTab.tsx`
- `src/lib/admin/ib/eligible-volume.server.ts`
- `src/lib/services/ib-snapshot.service.ts`

---

### `/admin/ib/sync-requests` — Cloud Sync Requests & Manual Support Tickets

Purpose:
- Operations telemetry for worker-driven MT5 Cloud Sync jobs and trader manual sync support requests.
- Handle fallback Manual Sync Support Tickets requested by users or auto-generated by worker failures (e.g., MT5 error `-10005` broker server catalog missing).
- Enable Admins to securely view trader investor credentials (with strict `ADMIN_VIEW_CREDENTIAL` security audit logging) and manually sync accounts via local MT5 terminal.

Users:
- Admins and Superadmins.

Expected behavior:
- Two dedicated top-level tabs:
  1. `Support Tickets (Manual Sync)`: Search and filter tickets (All, Pending, Verified, Failed). Shows ticket details, masked investor password with secure click-to-reveal modal, 1-click "Copy All" formatted credentials, step-by-step MT5 `-10005` troubleshooting guide, "Mark Synced" (`VERIFIED`) and "Mark Failed" with admin explanation notes.
  2. `Cloud Sync Jobs (Worker Telemetry)`: Full worker telemetry table with real-time status badges, range filter, search, retry, cancel, delete, and diagnostics inspector modal with raw job telemetry.
- Fail-fast handling: Worker error `-10005` (`SERVERS_DAT_PROVISION_FAILED`) stops immediately after attempt 1, creates a `SupportSyncTicket`, alerts admins, and avoids 5x timeout loops.
- User visibility: Users can track their support tickets at `/dashboard/accounts` with status badges, cancel requests, and trigger manual sync requests directly from `CloudSyncModal` or `SupportSyncRequestModal`.

Code paths:
- `src/app/admin/ib/sync-requests/page.tsx`
- `src/app/admin/ib/sync-requests/client.tsx`
- `src/app/admin/ib/sync-requests/SupportTicketsTab.tsx`
- `src/components/trading-accounts/SupportSyncRequestModal.tsx`
- `src/components/trading-accounts/CloudSyncModal.tsx`
- `src/components/trading-accounts/AccountListClient.tsx`
- `src/actions/support-sync.ts`
- `src/actions/admin-sync-requests.ts`
- `src/lib/credentials/investor-password.server.ts`
- `src/app/api/worker/jobs/fail/route.ts`
- `tools/tnt-worker/worker.py`
