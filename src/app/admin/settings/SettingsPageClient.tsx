        ...
        <div className="space-y-0 pb-10">
            ...
            >
                ...
                <div className="pt-6">
                    ...
                    {activeTab === "system" && (
                        <SystemSettings initialConfig={initialConfig} />
                    )}
        ...
        <div className="bg-white dark:bg-[#151925] rounded-xl border border-gray-200 dark:border-white/10 shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-6">
                <div className="bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-500 p-4 rounded-xl text-sm font-medium">
                    Password change is currently managed via the Login provider
                    (Supabase Auth). Direct password update here requires
                    re-authentication logic which is in development.
                </div>
            </div>
        </div>
    );
...
const ToggleRow = ({
    ...
    <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-black/20 rounded-xl border border-gray-200 dark:border-white/10">
        <div>
            <h4 className="font-bold text-gray-700 dark:text-white text-sm">
                {label}
            </h4>
...
function SystemSettings({
    ...
    initialConfig: SettingsPageClientProps["initialConfig"];
}) {
    const [isPending, startTransition] = useTransition();
    ...
    const handleToggle = (key: keyof typeof config) => {
        ...
        startTransition(async () => {
            ...
            if (result.success) {
                toast.success(
                    `Setting ${updated[key] ? "enabled" : "disabled"}`
                );
            } else {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                <SectionCard icon={Globe} title="Feature Toggles">
                    ...
                    <ToggleRow
                        ...
                        desc="Allow users to submit bug reports and feature requests."
                ...
                <div className="flex flex-col gap-5">
                    <SectionCard icon={Send} title="Notifications">
                        <ToggleRow
                            label="Welcome Email"
                            desc="Send a welcome email when new users register."
                            checked={config.welcomeEmail}
                            onChange={() => handleToggle("welcomeEmail")}
                        />
                        <ToggleRow
                            label="New Article Alert"
                            desc="Notify users when a new article is published."
                            checked={config.newArticleAlert}
                            onChange={() => handleToggle("newArticleAlert")}
                        />
                    </SectionCard>
                </div>
                ...
                >
                    <div>
                        <p className="text-xs text-gray-600 mb-3">
                            Display a modal announcement to all users. Leave
                            empty to hide.
                        </p>
                        <textarea
