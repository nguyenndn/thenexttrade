...
import { Mail, ShieldAlert, CheckCircle, XCircle, Info, RefreshCw } from "lucide-react";
...
export function EmailLabClient({ defaultRecipient, allowCustomRecipient }: EmailLabClientProps) {
 ...
 try {
 const res = await sendEmailLabTest({ templateId, to: customTo || undefined });
 setResults(prev => ({
 ...prev,
 [templateId]: {
 success: res.success,
 ...
 description: "Sends a basic system validation email to verify credentials and SMTP relay host connectivity.",
 ...
 description: "Sends a weekly trading report summary with positive performance data and gold/green accent badges.",
 sourceFile: "src/lib/services/email.service.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Sends a monthly trading report summary displaying a negative performance scenario with red accents.",
 sourceFile: "src/lib/services/email.service.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Sends a weekly warning notification advising the user to stay consistent even if no trades are placed.",
 sourceFile: "src/lib/services/email.service.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Sends a monthly warning notification advising the user to stay consistent even if no trades are placed.",
 sourceFile: "src/lib/services/email.service.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Reminder notification sent to users who registered but haven't connected any accounts after 24 hours.",
 ...
 description: "Notification sent 24 hours after sync onboarding if the user connected an account but logged no trades.",
 sourceFile: "src/lib/emails/activation-reminders.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Final activation reminder prompting the user to explore copy trading, academy or link accounts.",
 sourceFile: "src/lib/emails/activation-reminders.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Sent immediately after registration. Highlights the first 3 steps to unlock dashboard metrics.",
 ...
 description: "Sent 24 hours after registration if the user has no trade data. Prompts MT5 connection details.",
 sourceFile: "src/lib/emails/welcome-sequence.ts",
 status: "active",
 statusLabel: "Active SMTP"
 },
 ...
 description: "Sent 72 hours after registration if the user has no trade data. Explains features they are missing.",
 sourceFile: "src/lib/emails/welcome-sequence.ts",
 status: "active",
 statusLabel: "Active SMTP"
 }
 ...
 description: "Verification code or confirmation link sent during initial user registration.",
 ...
 description: "One-click authentication login email generating a temporary session token.",
 sourceFile: "src/app/auth/actions.ts",
 status: "supabase",
 statusLabel: "Supabase Auth"
 },
 ...
 description: "Email containing recovery token to allow user to reset their credentials.",
 sourceFile: "src/app/auth/actions.ts",
 status: "supabase",
 statusLabel: "Supabase Auth"
 },
 ...
 description: "Admin panel trigger. Generates a password recovery link using Supabase Admin Auth API.",
 ...
 const getStatusBadge = (status: "active" | "supabase" | "needs_fix", label: string) => {
 switch (status) {
 case "active":
 return <span className="bg-emerald-500/10 text-emerald-500 px-2.5 py-1 rounded-md text-xs font-bold tracking-wide border border-emerald-500/20">{label}</span>;
 case "supabase":
 return <span className="bg-sky-500/10 text-sky-500 px-2.5 py-1 rounded-md text-xs font-bold tracking-wide border border-sky-500/20">{label}</span>;
 case "needs_fix":
 return <span className="bg-amber-500/10 text-amber-500 px-2.5 py-1 rounded-md text-xs font-bold tracking-wide border border-amber-500/20">{label}</span>;
 }
 };
 ...
 Test and verify system-generated transactional HTML email templates.
 </p>
 </div>
 </div>
 ...
 <p className="font-bold uppercase tracking-wider">Internal Testing Environment Only</p>
 <p className="leading-relaxed">Every active SMTP trigger will dispatch a real email through the configured host server. Ensure you use local sandbox environments (like Mailtrap) to prevent sending mock data to production clients.</p>
 </div>
 </div>
 ...
 Custom recipient is locked by environment config. Sending only to default: <code className="text-gray-700 dark:text-gray-300 font-bold bg-gray-100 dark:bg-white/5 px-1.5 py-0.5 rounded text-xs">{defaultRecipient}</code>.
 </p>
 ) : (
 <p className="text-xs text-gray-500 mt-2 flex items-center gap-1 font-medium">
 <Info size={14} className="text-emerald-500" />
 Custom recipient is enabled. You can direct emails to your custom inbox.
 </p>
 )}
 </div>
 </div>
 ...
 {getStatusBadge(template.status, template.statusLabel)}
 </div>
 <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed min-h-[48px]">{template.description}</p>
 ...
 isLoading={loading}
 onClick={() => handleSend(template.id as EmailLabTemplateId)}
 >
 <Mail size={15} />
 Send Test Email
 </Button>
 ...
 {res && (
 <div className={`p-2.5 rounded text-xs font-medium flex gap-1.5 items-start ${res.success ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-red-500/10 text-red-600 dark:text-red-400'}`}>
 {res.success ? (
 ...
 <p className="opacity-60 text-xs">{res.timestamp}</p>
 </div>
 </div>
 )}
 </div>
 ...
 Auth emails are managed and dispatched directly by the <strong>Supabase Auth microservice</strong>. In local development or staging, configure Supabase Custom SMTP settings to redirect auth emails to Mailtrap, or trigger them manually from the login forms.
 </p>
 </div>
 ...
 <p className="mt-1">The link generation works, but the system currently only generates a recovery URL and does not dispatch it through SMTP. Treat this as an unfinished flow.</p>
 </div>
 ) : (
 ...
 <p className="font-bold">💡 How to Trigger & Test:</p>
 <p className="mt-1">Trigger this flow by submitting the actual forms at <code className="bg-sky-500/10 px-1 py-0.5 rounded">/auth/login</code> or <code className="bg-sky-500/10 px-1 py-0.5 rounded">/auth/signup</code> views.</p>
 </div>
 )}
 </div>
 </div>
