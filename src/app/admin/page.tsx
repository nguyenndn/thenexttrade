...
const getStats = unstable_cache(
  async () => {
    ...
    const [
      ...
      prisma.tradingAccount.count({ where: { createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } }),
      prisma.vipRequest.count({ where: { createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } }),
      prisma.copyTradingRegistration.count({ where: { createdAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } } }),
      // Recent 30-day period
      prisma.tradingAccount.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
      prisma.vipRequest.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
      prisma.copyTradingRegistration.count({ where: { createdAt: { gte: thirtyDaysAgo } } }),
      // 7-day sparkline raw
      prisma.tradingAccount.findMany({ where: { createdAt: { gte: sevenDaysAgo } }, select: { createdAt: true } }),
      prisma.vipRequest.findMany({ where: { createdAt: { gte: sevenDaysAgo } }, select: { createdAt: true } }),
      prisma.copyTradingRegistration.findMany({ where: { createdAt: { gte: sevenDaysAgo } }, select: { createdAt: true } }),
    ]);
    ...
    return {
      ...
      tradingAccounts: { total: tradingAccountsCount, sparkline: tradingAccountSparkline, trendPercent: calcTrend(recentTradingAccountsCount, prevTradingAccountsCount) },
      vipRequests: { total: vipRequestsCount, sparkline: vipRequestSparkline, trendPercent: calcTrend(recentVipRequestsCount, prevVipRequestsCount) },
      copyTrading: { total: copyTradingCount, sparkline: copyTradingSparkline, trendPercent: calcTrend(recentCopyTradingCount, prevCopyTradingCount) },
      ibLeadsCount,
      proEntitlementsCount,
      lessonsCount,
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <AdminDashboardClient
        ...
        tradingAccounts={{ value: stats.tradingAccounts.total, sparkline: stats.tradingAccounts.sparkline, trendPercent: stats.tradingAccounts.trendPercent }}
        vipRequests={{ value: stats.vipRequests.total, sparkline: stats.vipRequests.sparkline, trendPercent: stats.vipRequests.trendPercent }}
        copyTrading={{ value: stats.copyTrading.total, sparkline: stats.copyTrading.sparkline, trendPercent: stats.copyTrading.trendPercent }}
