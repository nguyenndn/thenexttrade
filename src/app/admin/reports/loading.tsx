export default function AdminReportsLoading() {
  return (
    <div className="space-y-6 pb-10">
      ...
      {/* Panels skeleton */}
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] p-6 animate-pulse">
          <div className="h-4 w-32 bg-gray-200 dark:bg-white/10 rounded mb-4" />
          <div className="grid grid-cols-4 gap-3">
            {[1, 2, 3, 4].map((j) => (
              <div key={j} className="h-20 bg-gray-100 dark:bg-white/5 rounded-xl" />
            ))}
          </div>
        </div>
      ))}
