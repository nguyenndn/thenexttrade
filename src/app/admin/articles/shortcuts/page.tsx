...
export default function ShortcutsManagerPage() {
    const {
        ...
        isLoading,
        mutate,
    } = useSWR("/api/articles/shortcuts", fetcher, {
        onError: (err) => {
    ...
    const [shortcutToDelete, setShortcutToDelete] = useState<string | null>(
        null
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const url = editingId
                ? `/api/articles/shortcuts/${editingId}`
                : "/api/articles/shortcuts";
            ...
            toast.success(
                editingId ? "Shortcut updated!" : "Shortcut created!"
            );
            ...
            toast.error(
                error instanceof Error
                    ? error.message
                    : error?.message ||
                          "An error occurred preserving the shortcut."
            );
        } finally {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch(
                `/api/articles/shortcuts/${shortcutToDelete}`,
                { method: "DELETE" }
            );
            ...
            toast.error(
                error instanceof Error
                    ? error.message
                    : error?.message || "Failed to delete shortcut."
            );
        } finally {
            setIsDeleting(false);
        ...
        <div className="space-y-4 pb-10">
            ...
            >
                <Button
                    ...
                    variant="primary"
                    className="flex items-center gap-2 shadow-lg shadow-primary/30 active:scale-95 active:translate-y-0 transition-all"
                >
                    ...
                    {isLoading ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                            {[1, 2, 3, 4, 5, 6].map((i) => (
                                <div
                                    key={i}
                                    className="flex flex-col p-6 bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl h-[160px] animate-pulse"
                                >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                            {filteredShortcuts.map((shortcut) => (
                                <div
                                    key={shortcut.id}
                                    className="group relative flex flex-col p-6 bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl hover:shadow-md transition-shadow cursor-pointer"
                                >
                        ...
                        <div className="flex flex-col items-center justify-center text-center p-12 bg-gray-50 dark:bg-white/5 rounded-xl border border-dashed border-gray-200 dark:border-white/10 my-10 max-w-lg mx-auto">
                            ...
                            <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">
                                You haven't created any reusable snippets yet.
                                Create your first shortcut to start saving time
