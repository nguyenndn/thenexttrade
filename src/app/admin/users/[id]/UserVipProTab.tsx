...
import {
  KeyRound, ShieldCheck, ShieldAlert, CheckCircle2, XCircle, Clock,
  Image as ImageIcon, Loader2, AlertCircle, AlertTriangle, Trash2
} from "lucide-react";
...
export function UserVipProTab({ user }: UserVipProTabProps) {
  ...
  const [selectedAccountId, setSelectedAccountId] = useState<string>("user-level");
  const [grantType, setGrantType] = useState<"permanent" | "grace">("permanent");
  ...
  const handleApproveVip = async (requestId: string) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success("VIP verification request approved successfully!");
      } else {
  ...
  const handleGrantPro = async (e: React.FormEvent) => {
    ...
    try {
      const accountId = selectedAccountId === "user-level" ? undefined : selectedAccountId;
      let res;
  ...
  const handleRevokePro = async (entitlementId: string, tradingAccountId?: string) => {
    ...
    try {
      const res = await revokeProAccess(user.id, revokeReason, tradingAccountId);
      if (res.success) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden shadow-sm">
        <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
          <div>
            <h3 className="text-base font-bold text-gray-700 dark:text-white flex items-center gap-2">
              <ShieldCheck size={18} className="text-primary" /> VIP Verification Requests
            </h3>
            <p className="text-xs text-gray-500 mt-1">Review broker account verification requests submitted by the user</p>
          </div>
        ...
        {user.vipRequests.length > 0 ? (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-200 dark:border-white/10 bg-gray-50/50 dark:bg-white/[0.01] text-[10px] font-bold text-gray-500 uppercase tracking-wider">
                  <th className="p-4 sm:px-6">Account Details</th>
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-white/10 text-sm">
                {user.vipRequests.map((req) => (
                  <tr key={req.id} className="hover:bg-gray-50/30 dark:hover:bg-white/[0.01] transition-colors">
                    <td className="p-4 sm:px-6">
                      <p className="font-bold text-gray-700 dark:text-white">
                        {req.broker}
                      </p>
                      <p className="text-xs text-gray-600 mt-0.5 font-medium">
                        Account #: <span className="font-mono text-xs">{req.accountNumber}</span>
                      </p>
                      <p className="text-xs text-gray-500 mt-0.5">
                        Balance: <span className="font-bold text-gray-700 dark:text-white">${parseFloat(req.balance).toLocaleString()}</span>
                      </p>
                    </td>
                    <td className="p-4">
                      ...
                      <p className="text-xs text-gray-600 mt-0.5">
                        Telegram: {req.telegramId ? `@${req.telegramId.replace('@', '')}` : "N/A"}
                      </p>
                    ...
                    <td className="p-4">
                      {req.screenshotUrl ? (
                        ...
                        <span className="text-xs text-gray-600 flex items-center gap-1">
                          <AlertCircle size={14} className="text-gray-400" /> No attachment
                        </span>
                      )}
                    </td>
                    <td className="p-4">
                      <div className="space-y-1.5">
                        ...
                        {req.reviewedAt && (
                          <p className="text-[10px] text-gray-500">
                            Reviewed on {format(new Date(req.reviewedAt), "MMM d, yyyy")}
                          </p>
                        )}
                      </div>
                    </td>
                          ...
                          {rejectingId === req.id ? (
                            ...
                            <>
                              ...
                              <Button
                                ...
                                disabled={loading === `vip-approve-${req.id}`}
                                className="h-8 text-xs font-bold rounded-lg"
                              >
                                {loading === `vip-approve-${req.id}` ? (
          ...
          <div className="p-8 text-center bg-gray-50/30 dark:bg-transparent">
            <p className="text-sm text-gray-500">No VIP verification requests found for this user.</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden shadow-sm flex flex-col">
          <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-gray-700 dark:text-white flex items-center gap-2">
                <KeyRound size={18} className="text-amber-500" /> Pro Entitlement Subscriptions
              </h3>
              <p className="text-xs text-gray-500 mt-1">Status of this user's active or temporary Pro access benefits</p>
            </div>
          ...
          {user.proEntitlements.length > 0 ? (
            <div className="divide-y divide-gray-200 dark:divide-white/10 flex-1">
              {user.proEntitlements.map((ent) => (
                <div key={ent.id} className="p-5 space-y-4 hover:bg-gray-50/20 dark:hover:bg-white/[0.01] transition-colors">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-bold text-gray-700 dark:text-white">
                          {ent.broker ? `${ent.broker} · #${ent.accountNumberMasked || ent.accountNumber}` : "User-level Access (Global)"}
                        </p>
                        {ent.tradingAccount?.name && (
                          <span className="text-xs text-gray-600 font-medium">({ent.tradingAccount.name})</span>
                        )}
                      </div>
                      <p className="text-xs text-gray-500">
                        Granted by: {ent.source === "IB_VERIFIED" ? "IB Auto-Verify" : ent.source}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    <div>
                      ...
                      <p className="font-bold text-gray-700 dark:text-white mt-0.5">
                        {format(new Date(ent.startsAt), "MMM d, yyyy · HH:mm")}
                      </p>
                    </div>
                    <div>
                      <p className="text-gray-600">Expiration Date</p>
                      <p className="font-bold text-gray-700 dark:text-white mt-0.5">
                        {ent.expiresAt ? format(new Date(ent.expiresAt), "MMM d, yyyy · HH:mm") : "Never (Lifetime)"}
                      </p>
                    </div>
                  </div>
                  ...
                  {ent.adminNote && (
                    <div className="bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-3 text-xs text-gray-600 dark:text-gray-300">
                      <p className="font-bold uppercase text-[9px] tracking-wider text-gray-600 mb-1">Admin Notes</p>
                      {ent.adminNote}
                    </div>
                  )}

                  {ent.status !== "REVOKED" && ent.status !== "EXPIRED" && (
                      ...
                      {revokingId === ent.id ? (
                        <div className="flex items-center gap-2 w-full max-w-md bg-red-50/50 dark:bg-red-500/5 border border-red-200 dark:border-red-500/10 p-3 rounded-xl">
                          ...
                          <Button
                            variant="primary"
                            size="sm"
                            onClick={() => handleRevokePro(ent.id, ent.broker ? ent.id : undefined)} // Use actual accountId if applicable
                            disabled={loading === `revoke-pro-${ent.id}`}
                            className="h-8 text-xs font-bold bg-red-600 hover:bg-red-700 text-white rounded-lg"
                          >
                            {loading === `revoke-pro-${ent.id}` ? (
                              <Loader2 size={12} className="animate-spin mr-1" />
                            ) : null}
                            Revoke
                          </Button>
            ...
            <div className="p-8 text-center flex-1 flex flex-col items-center justify-center bg-gray-50/30 dark:bg-transparent">
              ...
              <p className="text-sm text-gray-500">No Pro entitlement privileges set for this user yet.</p>
              <p className="text-xs text-gray-600 mt-1">Use the panel on the right to manually grant Pro features.</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl p-6 shadow-sm">
          <h3 className="text-base font-bold text-gray-700 dark:text-white flex items-center gap-2 mb-4 border-b border-gray-200 dark:border-white/10 pb-3">
            <KeyRound size={18} className="text-primary" /> Grant Pro Access
          </h3>
          ...
          <form onSubmit={handleGrantPro} className="space-y-4">
            ...
            <div>
              ...
              >
                <option value="user-level">Global (User-level access)</option>
      ...
      {screenshotModal && (
        ...
        >
          <div className="relative max-w-5xl max-h-[90vh] bg-white dark:bg-[#151925] rounded-xl overflow-hidden p-2 shadow-2xl cursor-default" onClick={e => e.stopPropagation()}>
            <img
              src={screenshotModal}
            ...
            <div className="p-4 flex items-center justify-between">
              <p className="text-xs text-gray-600 font-medium">VIP verification proof attachment screenshot</p>
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setScreenshotModal(null)}
