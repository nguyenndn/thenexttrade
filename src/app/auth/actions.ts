...
export async function login(formData: FormData) {
    ...
    if (error) {
        logSecurityEvent({
            ...
            ip,
            path: "/auth/login",
            detail: `Email: ${data.email}. Error: ${error.message}`,
        }).catch(() => {});
        ...
        if (!dbUser) {
            ...
            if (existingUserByEmail) {
                ...
                logSecurityEvent({
                    ...
                    ip,
                    path: "/auth/login",
                    detail: `Identity conflict for email: ${user.email}`,
                }).catch(() => {});
                return {
                    error: "Account setup issue. Please contact support.",
                };
            } else {
    ...
    if (authedUser) {
        ...
        displayName =
            dbUser?.name || authedUser.user_metadata?.full_name || "Trader";
    }
...
export async function signup(formData: FormData) {
    ...
    const referralCode =
        (formData.get("referralCode") as string | null)?.trim() || null;
    ...
    if (error) {
        logSecurityEvent({
            ...
            ip,
            path: "/auth/signup",
            detail: `Email: ${email}. Error: ${error.message}`,
        }).catch(() => {});
...
export async function verifyOtpAction(formData: FormData) {
    ...
    if (data.user) {
        ...
        const referrerUserId =
            typeof metadata?.referrer_user_id === "string"
                ? metadata.referrer_user_id
                : null;
        const referralCode =
            typeof metadata?.referral_code === "string"
...
export async function resendOtpAction(formData: FormData) {
    ...
    if (error) {
        logSecurityEvent({
            type: SECURITY_EVENT_TYPES.AUTH_FAILED,
            ip,
            path: "/auth/actions/resend",
            detail: `Resend failed: ${error.message}`,
        }).catch(() => {});
    ...
    return {
        success: true,
        message: "A new 8-digit code has been sent to your email.",
    };
}
