...
import { Globe, Lock, Info, Loader2, BarChart2, Eye, EyeOff } from "lucide-react";
...
export function LeaderboardSidebar({ initialData }: LeaderboardSidebarProps) {
 const [showOnLeaderboard, setShowOnLeaderboard] = useState(initialData.showOnLeaderboard);
 const [accounts, setAccounts] = useState<UserTradingAccountInfo[]>(initialData.accounts);
 ...
 const handleAccountVisibilityToggle = (accountId: string, currentVal: boolean) => {
 setTogglingAccount(accountId);
 startTransition(async () => {
 try {
 const newVal = !currentVal;
 const result = await toggleAccountLeaderboardVisibility(accountId, newVal);
 setAccounts((prev) =>
 prev.map((acc) => (acc.id === accountId ? { ...acc, useForLeaderboard: result } : acc))
 );
 } catch (err) {
 ...
 Control how your profile and trading accounts appear on the leaderboard.
 </p>
 </div>
 ...
 showOnLeaderboard ? "bg-[#00C888]" : "bg-gray-200 dark:bg-white/10"
 )}
 >
 <span
 className={cn(
 ...
 <button
 onClick={() =>
 handleAccountVisibilityToggle(account.id, account.useForLeaderboard)
 }
 ...
 {!showOnLeaderboard && account.useForLeaderboard && (
 ...
 <span>
 Your profile is set to hidden. This account will not be visible on the
 leaderboard.
 </span>
 </div>
 )}
 ...
 <span className="block text-[10px] text-gray-500 font-medium">Daily</span>
 <span
 className={cn(
 ...
 >
 {formatCurrency(account.pnl.daily)}
 </span>
 </div>
 <div className="p-2 bg-white/40 dark:bg-white/5 rounded-lg">
 <span className="block text-[10px] text-gray-500 font-medium">Weekly</span>
 <span
 className={cn(
 "text-xs font-bold block mt-0.5",
 ...
 >
 {formatCurrency(account.pnl.weekly)}
 </span>
 </div>
 <div className="p-2 bg-white/40 dark:bg-white/5 rounded-lg">
 <span className="block text-[10px] text-gray-500 font-medium">Monthly</span>
 <span
 className={cn(
 "text-xs font-bold block mt-0.5",
 ...
 >
 {formatCurrency(account.pnl.monthly)}
 </span>
 </div>
 </div>
 ...
 <span>
 No trading data for this period. Place some trades or sync your account.
 </span>
 </div>
 )}
 </div>
