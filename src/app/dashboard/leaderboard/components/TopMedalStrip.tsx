...
const RANK_CONFIGS = [
  {
    ...
    hoverBorderColor: "hover:border-amber-500 dark:hover:border-amber-500/60",
  ...
  },
  {
    ...
    hoverBorderColor: "hover:border-slate-400 dark:hover:border-slate-600/70",
  ...
  },
  {
    ...
    hoverBorderColor: "hover:border-orange-400 dark:hover:border-orange-500/60",
...
const MedalBadge = ({ rank }: { rank: number }) => {
  ...
  return (
    <div className={cn("relative w-10 h-10 shrink-0 flex items-center justify-center bg-transparent", config.glowClass)}>
      <svg viewBox="0 0 40 40" className="w-10 h-10 select-none">
        ...
        {/* Inner ring */}
        <circle cx={20} cy={20} r={9.5} fill="none" stroke={config.strokeColor} strokeWidth="1.2" strokeOpacity="0.45" />
        {/* Number centered */}
        <text x="20" y="20" dy=".35em" textAnchor="middle" fill="#FFFFFF" className="font-extrabold text-sm font-sans select-none pointer-events-none">
          {rank}
        </text>
      </svg>
    </div>
...
function get2x2Stats(entry: LeaderboardEntry, type: LeaderboardType) {
  switch (type) {
    case "xp":
      return [
        { label: "Total Edge", value: `${entry.value.toLocaleString()} Edge` },
        { label: "Study Time", value: formatStudyTime(entry.studyTimeMinutes) },
        { label: "Lessons Completed", value: `${entry.lessonsCompleted}` },
        { label: "Level", value: `Level ${entry.level}` },
      ];
    case "streak":
      return [
        { label: "Daily Streak", value: `${entry.value} days` },
        { label: "Total Edge", value: `${entry.tier.minXp.toLocaleString()}+` },
    ...
    case "trading":
      return [
        { label: "Win Rate", value: `${entry.value}%` },
        { label: "Total Trades", value: `${entry.totalTrades}` },
        { label: "Total PnL", value: entry.pnl >= 0 ? `+$${entry.pnl.toFixed(0)}` : `-$${Math.abs(entry.pnl).toFixed(0)}` },
        { label: "Level", value: `Level ${entry.level}` },
      ];
    default:
      return [
        { label: "Total Edge", value: `${entry.value.toLocaleString()}` },
        { label: "Study Time", value: formatStudyTime(entry.studyTimeMinutes) },
...
export function TopMedalStrip({ entries, currentUserId, onUserClick, type }: TopMedalStripProps) {
      ...
      {padded.map((entry, index) => {
        ...
        if (!entry) {
          return (
            <div
              key={`empty-${rank}`}
              className="flex flex-col rounded-2xl border border-dashed border-gray-200 dark:border-white/10 bg-gray-50/20 dark:bg-white/[0.01] p-4 text-left select-none text-gray-400 dark:text-gray-500"
            >
              {/* Header placeholder */}
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className="w-14 h-14 rounded-full border-2 border-dashed border-gray-300 dark:border-gray-700 bg-transparent flex items-center justify-center text-sm font-bold text-gray-300 dark:text-gray-700 shrink-0">
                    ?
                  </div>
                  <div>
                    <p className="font-bold text-sm text-gray-400 dark:text-gray-500">Open spot</p>
                    <p className="text-xs text-gray-400 dark:text-gray-600">No occupant yet</p>
                  </div>
                </div>
                {/* Greyed out medal shape */}
                <div className="w-10 h-10 rounded-full border-2 border-dashed border-gray-300 dark:border-gray-700 flex items-center justify-center text-xs font-black text-gray-300 dark:text-gray-700 shrink-0">
                  #{rank}
                </div>
              </div>

              {/* Divider */}
              <div className="w-full border-b border-dashed border-gray-100 dark:border-white/5 my-3" />
        ...
        return (
          <button
            ...
            className={cn(
              ...
              isCurrentUser && "ring-2 ring-primary/30 ring-offset-2 dark:ring-offset-[#151925]"
            )}
          >
                  ...
                  <div className="flex items-center gap-1.5">
                    <p
                      className={cn(
                        ...
                        isCurrentUser ? "text-primary font-black" : "text-gray-800 dark:text-gray-200"
                      )}
                    >
                      {entry.name}
                    </p>
            ...
            {/* Divider */}
            <div className="w-full border-b border-gray-100 dark:border-white/5 my-3" />
