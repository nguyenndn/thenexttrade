...
export function LeaderboardTable({
    ...
    if (entries.length === 0 && !myEntry) {
        return (
            <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard shadow-sm p-12 text-center">
                ...
                <p className="text-gray-600 dark:text-gray-300 text-sm">
                    Be the first to claim the top spot! Start learning, trading,
                    ...
                    <thead className="bg-gray-50 dark:bg-white/5 text-[11px] uppercase text-gray-500 font-bold tracking-wider">
                        <tr>
                            ...
                            {headers.map((h) => (
                                <th
                                    key={h.label}
                                    className={cn("py-3 px-3", h.className)}
                                >
                                    {h.label}
                                </th>
                            ))}
                    ...
                    <tbody className="divide-y divide-dashboard">
                        ...
                        {!isUserInTop && currentUserId && myEntry && (
                            <>
                                <tr>
                                    <td
                                        colSpan={getColSpan(type)}
                                        className="py-2 px-6"
                                    >
                                        <div className="border-t-2 border-dashed border-dashboard" />
                                    </td>
                                </tr>
                                <LeaderboardRow
                                    entry={myEntry}
