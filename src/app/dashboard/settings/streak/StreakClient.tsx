    ...
    const fetchStreakData = async () => {
        try {
            ...
            if (res.ok) {
                ...
                setLastCheckIn(
                    data.lastCheckIn ? new Date(data.lastCheckIn) : null
                );
        ...
        try {
            ...
            if (res.ok) {
                ...
                await celebrateXP({
                    ...
                    leveledUp:
                        data.message?.includes("Badge") ||
                        data.message?.includes("LEGENDARY"),
                });
            } else {
    ...
    return (
        <div className="w-full grid grid-cols-1 xl:grid-cols-5 gap-6 items-stretch">
            ...
            <div className="xl:col-span-2 h-full">
                ...
                <div className="bg-white dark:bg-[#0B0E14] p-8 rounded-xl border border-dashboard shadow-sm flex flex-col relative overflow-hidden h-full">
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center w-full min-h-[300px]">
                        {" "}
                        {/* Added min-h to ensure space */}
                        <div className="relative mb-6">
                            <div className="absolute inset-0 bg-orange-500 blur-[50px] opacity-20 rounded-full"></div>
                            <Flame
                                size={80}
                                className="text-orange-500 relative z-10 animate-pulse"
                            />
                        </div>
                        <div className="mb-6 text-center">
                            <p className="text-gray-600 font-bold uppercase tracking-[0.2em] text-xs mb-2">
                                Current Login Streak
                            </p>
                            <h2 className="text-6xl font-black text-gray-700 dark:text-white mb-2">
                                {streak}
                            </h2>
                        ...
                        <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3 custom-scrollbar">
                            {MILESTONES.map((milestone, index) => {
                                ...
                                const isNext =
                                    !isAchieved &&
                                    (index === 0 ||
                                        streak >= MILESTONES[index - 1].days);
            ...
            <div className="bg-white dark:bg-[#0B0E14] p-6 rounded-xl border border-dashboard shadow-sm h-full flex flex-col xl:col-span-3">
                ...
                <div className="flex-1 w-full flex flex-col justify-center">
                    ...
                    <div className="grid grid-cols-7 mb-3 gap-1 md:gap-2">
                        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map(
                            (day) => (
                                <div
                                    key={day}
                                    className="text-center text-[10px] md:text-xs font-bold text-gray-500 uppercase tracking-wider"
                                >
                                    {day}
                                </div>
                            )
                    ...
                    <div className="grid grid-cols-7 gap-1 md:gap-2">
                        ...
                        {calendarDays.map((day) => {
                            const isCheckedIn = checkInHistory.some((h) =>
                                isSameDay(new Date(h), day)
                            );
