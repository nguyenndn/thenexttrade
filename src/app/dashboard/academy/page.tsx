...
import { GraduationCap, Trophy, ArrowRight, Zap, Target, BookOpen, Award, Clock } from "lucide-react";
...
export default async function UserAcademyDashboard() {
 ...
 <span>{completedLessons}/{totalLessons} Lessons</span>
  ...
  <h3 className="text-xl md:text-2xl font-black text-gray-800 dark:text-white leading-snug">{nextLesson.title}</h3>
  <p className="text-gray-500 text-sm font-medium">In module: {nextLessonModuleTitle}</p>
  </div>
  ...
  <span>You paused your learning path for {idleDays} days. Let&apos;s resume to keep your trading routine sharp and refine your edge!</span>
  </p>
  )}
  ...
  variant: 'primary',
  size: 'smd',
  className: 'group whitespace-nowrap shadow-lg shadow-primary/20 gap-1.5 font-bold rounded-xl'
  })}
  >
  ...
  Estimated: {nextLesson.duration} mins
  </span>
  )}
  </div>
  </div>
  ...
  <div id="onborda-academy-tree" className="bg-white dark:bg-[#151925] rounded-xl border border-dashboard shadow-sm overflow-hidden">
  ...
  <h3 className="font-extrabold text-sm tracking-wider uppercase text-white/90 dark:text-gray-400">Daily Streak</h3>
  </div>
  <p className="text-white/85 dark:text-gray-400 text-xs mb-4">Consistency is key to trading success.</p>
  <div className="flex items-baseline gap-2">
  <span className="text-4xl font-black text-white dark:bg-gradient-to-r dark:from-amber-400 dark:to-orange-500 dark:bg-clip-text dark:text-transparent">{currentStreak}</span>
  <span className="text-white/80 dark:text-gray-400 text-xs font-bold uppercase tracking-wider">Days Active</span>
  </div>
  </div>
  ...
  <p className="text-sm font-bold text-gray-700 dark:text-gray-200 truncate group-hover:text-primary transition-colors">{quiz.title}</p>
  ...
  {hasPassed ? (
  <span className="flex-shrink-0 text-[10px] font-black px-2.5 py-1 rounded-lg bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">✓ {bestAttempt.score}%</span>
  ) : bestAttempt ? (
  <span className="flex-shrink-0 text-[10px] font-black px-2.5 py-1 rounded-lg bg-red-500/10 text-red-600 dark:text-red-400 border border-red-500/20">{bestAttempt.score}%</span>
  ) : (
  <span className="flex-shrink-0 text-[10px] font-black px-2.5 py-1 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20">New</span>
  )}
  </a>
  );
  })}
  ...
  <span className="text-gray-400 text-xs font-bold uppercase tracking-wider">/ {totalLevels} levels</span>
  </div>
  ...
  {earnedCerts > 0 ? `${earnedCerts} certificate${earnedCerts > 1 ? 's' : ''} earned` : 'Pass all quizzes to earn certificates'}
  <ArrowRight size={14} className="group-hover:translate-x-0.5 transition-transform" />
  </p>
  </div>
  </Link>
