...
export async function GET(request: NextRequest) {
 ...
 const isDryRun = process.env.NODE_ENV === "development" && (!process.env.SMTP_USER || !process.env.SMTP_PASS);
 ...
 message = "Connect your trading account to start tracking trades and unlocking professional metrics.";
 link = "/dashboard/accounts?action=add&source=activation-reminder";
 icon = "PlusCircle";
 } else if (c.type === "NO_FIRST_DATA_24H") {
 title = "Sync your first trade 📊";
 message = c.preferredSyncMethod === "MANUAL"
 ...
 link = c.preferredSyncMethod === "MANUAL"
 ...
 message = "You're missing out on AI Trade Score, Psychology Trackers, and Weekly Performance reviews.";
 link = "/dashboard/accounts?setup=sync";
 icon = "Sparkles";
 } else if (c.type === "MOBILE_SYNC_FALLBACK") {
 title = "Finish MT5 setup on desktop 💻";
 message = "Auto-syncing requires local desktop setup. Open the desktop setup link or log manually for now.";
 link = "/dashboard/journal?action=log-trade&source=mobile-fallback";
 icon = "Laptop";
 }
 ...
 const methodParam = c.preferredSyncMethod === "EA_SYNC" ? "ea" : "tnt";
 ctaLink = `${appUrl}/dashboard/accounts?setup=sync&method=${methodParam}&source=desktop-link`;
 }

 const html = buildActivationEmailHtml(c.type, c.name, c.preferredSyncMethod, ctaLink);

 let emailSent = false;
 if (isDryRun) {
 console.log(`[DryRun Email] To: ${c.email} | Subject: ${subject}`);
 emailSent = true;
 dryRunCount++;
 } else {
