...
export async function GET(request: NextRequest) {
    try {
        ...
        const trades = await prisma.journalEntry.findMany({
            // Assuming JournalEntry is the model name
            where: whereClause,
            select: {
                id: true,
                pnl: true,
        ...
        const worstSession =
            sessionStats.length > 0
                ? sessionStats.reduce((worst, curr) =>
                      curr.totalPnL < worst.totalPnL ? curr : worst
                  ).session
                : null;

        const hourlyWithTrades = fullHourlyStats.filter(
            (h) => h.totalTrades >= 3
        );
        const bestHour =
            hourlyWithTrades.length > 0
                ? hourlyWithTrades.reduce((best, curr) =>
        ...
        const worstHour =
            hourlyWithTrades.length > 0
                ? hourlyWithTrades.reduce((worst, curr) =>
...
function detectSession(entryDate: Date): string {
    ...
    if (
        activeSessions.includes("LONDON") &&
        activeSessions.includes("NEW_YORK")
    ) {
