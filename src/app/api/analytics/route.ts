    ...
    try {
        ...
        if (reversedTrades.length > 0) {
            ...
            if (firstResult === "WIN" || firstResult === "LOSS") {
                currentStreak.type = firstResult.toLowerCase() as
                    "win" | "loss";
        ...
        for (const dateKey of sortedDates) {
            ...
            const growth =
                runningBalance !== 0 ? (dayPnL / runningBalance) * 100 : 0;
        ...
        const dailyPnL = Array.from(tradesByDate.entries()).map(
            ([date, pnl]) => {
                const tradesOnDate = trades.filter((t) => {
                    if (!t.exitDate) return false;
                    const brokerDate = addHours(
                        t.exitDate,
                        BROKER_OFFSET_HOURS
                    );
        ...
        >();
        for (const trade of trades) {
            const current = pairMap.get(trade.symbol) || {
                pnl: 0,
                count: 0,
                wins: 0,
            };
