...
export async function GET(request: NextRequest) {
    try {
        ...
        const emotionBeforeMap = new Map<
            string,
            { wins: number; losses: number; pnl: number }
        >();
        for (const trade of trades) {
            if (!trade.emotionBefore) continue;
            const current = emotionBeforeMap.get(trade.emotionBefore) || {
                wins: 0,
                losses: 0,
                pnl: 0,
            };
        ...
        const emotionBeforeStats: EmotionStats[] = Array.from(
            emotionBeforeMap.entries()
        )
            .map(([emotion, data]) => {
        ...
        const emotionAfterMap = new Map<
            string,
            { wins: number; losses: number; pnl: number }
        >();
        for (const trade of trades) {
            if (!trade.emotionAfter) continue;
            const current = emotionAfterMap.get(trade.emotionAfter) || {
                wins: 0,
                losses: 0,
                pnl: 0,
            };
        ...
        const emotionAfterStats: EmotionStats[] = Array.from(
            emotionAfterMap.entries()
        ).map(([emotion, data]) => {
            const total = data.wins + data.losses;
            return {
                emotion,
        ...
        for (const trade of trades) {
            ...
            const current = confidenceMap.get(trade.confidenceLevel) || {
                wins: 0,
                total: 0,
                pnl: 0,
            };
        ...
        const planAdherenceStats = {
            followed: {
                ...
                winRate:
                    followed.length > 0
                        ? (followed.filter((t) => t.result === "WIN").length /
                              followed.length) *
                          100
                        : 0,
            ...
            notFollowed: {
                ...
                winRate:
                    notFollowed.length > 0
                        ? (notFollowed.filter((t) => t.result === "WIN")
                              .length /
                              notFollowed.length) *
                          100
                        : 0,
        ...
        const totalLots = sortedTrades.reduce(
            (sum, t) => sum + (t.lotSize || 0),
            0
        );
        const avgLotSize =
            sortedTrades.length > 0 ? totalLots / sortedTrades.length : 0;

        // Overtrading detection: count days with > 5 trades
        const tradesByDay = new Map<string, number>();
        for (const trade of sortedTrades) {
            const dayKey = new Date(trade.entryDate)
                .toISOString()
                .split("T")[0];
        ...
        for (let i = 0; i < sortedTrades.length; i++) {
            ...
            if (trade.followedPlan === false) {
                ...
                maxNotFollowingPlanStreak = Math.max(
                    maxNotFollowingPlanStreak,
                    notFollowingPlanStreak
                );
            ...
            if (i > 0) {
                ...
                if (prevTrade.result === "LOSS") {
                    const sizeIncrease =
                        (trade.lotSize || 0) / (prevTrade.lotSize || 1);
                    // If size increased by > 50% AND is significantly above average
                    if (
                        sizeIncrease > 1.5 &&
                        (trade.lotSize || 0) > avgLotSize
                    ) {
                        sizingUpCount++;
                    }
                }
        ...
        const tiltIndicators = {
            ...
            avgPnLAfterLoss:
                afterLossCount > 0 ? afterLossPnL / afterLossCount : 0,
        ...
        for (const trade of sortedTrades) {
            ...
            if (trade.emotionBefore) {
                week.emotions.set(
                    trade.emotionBefore,
                    (week.emotions.get(trade.emotionBefore) || 0) + 1
                );
            }
        }
        const emotionTrend = Array.from(weekMap.values())
            .map((w) => {
                let dominant = "—";
                let maxCount = 0;
                w.emotions.forEach((count, emotion) => {
                    if (count > maxCount) {
                        maxCount = count;
                        dominant = emotion;
                    }
        ...
        const heatmapGrid: Record<
            string,
            { trades: number; wins: number; emotions: Map<string, number> }
        > = {};
        for (const day of DAYS)
            for (const slot of SLOTS) {
                heatmapGrid[`${day}-${slot}`] = {
                    trades: 0,
                    wins: 0,
                    emotions: new Map(),
                };
        ...
        for (const trade of sortedTrades) {
            ...
            if (trade.emotionBefore) {
                cell.emotions.set(
                    trade.emotionBefore,
                    (cell.emotions.get(trade.emotionBefore) || 0) + 1
                );
            }
        }
        const moodHeatmap = DAYS.map((day) => ({
            ...
            slots: SLOTS.map((slot) => {
                ...
                return {
                    ...
                    trades: cell.trades,
                    winRate:
                        cell.trades > 0 ? (cell.wins / cell.trades) * 100 : 0,
                    dominantEmotion: dominant,
                };
            }),
        }));
