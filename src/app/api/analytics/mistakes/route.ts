...
export async function GET(request: NextRequest) {
    try {
        ...
        // Calculate win rates
        const cleanWins = tradesWithoutMistakes.filter(
            (t) => t.result === "WIN"
        ).length;
        const cleanTradeWinRate =
            tradesWithoutMistakes.length > 0
                ? (cleanWins / tradesWithoutMistakes.length) * 100
                : 0;

        const mistakeWins = tradesWithMistakes.filter(
            (t) => t.result === "WIN"
        ).length;
        const mistakeTradeWinRate =
        ...
        const mistakeStats: MistakeStats[] = Array.from(mistakeMap.entries())
            .map(([code, data]) => {
                ...
                return {
                    ...
                    winRate:
                        totalTrades > 0 ? (data.wins / totalTrades) * 100 : 0,
                };
            })
        ...
        for (const stat of mistakeStats) {
            mistakesByCategory[stat.category] =
                (mistakesByCategory[stat.category] || 0) + stat.count;
        }

        // Finding most costly and frequent
        const mostCostlyMistake =
            mistakeStats.length > 0 ? mistakeStats[0].code : null;

        // Most frequent
        const sortedByFreq = [...mistakeStats].sort(
            (a, b) => b.count - a.count
        );
        const mostFrequentMistake =
            sortedByFreq.length > 0 ? sortedByFreq[0].code : null;
        ...
        const costOfMistakes = tradesWithMistakes.reduce(
            (sum, t) => sum + (t.pnl || 0),
            0
        );

        const totalMistakes = Array.from(mistakeMap.values()).reduce(
            (sum, m) => sum + m.count,
            0
        );
