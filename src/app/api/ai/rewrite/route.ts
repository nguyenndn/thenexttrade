...
async function getSystemPrompt(): Promise<string> {
    return loadFileContent(
        path.join(process.cwd(), "content", "rewrite-system-prompt.md")
    );
}

async function getWriterPersona(): Promise<string> {
    const content = await loadFileContent(
        path.join(process.cwd(), "content", "writer-persona.md")
    );
    return (
        content ||
        "Write in a friendly, professional tone. Use short paragraphs and bullet points."
    );
}
...
async function getToneInstructions(tone: string): Promise<string> {
    if (!tone) return "";
    const content = await loadFileContent(
        path.join(process.cwd(), "content", "tones", `${tone}.md`)
    );
    return content;
}
...
async function scrapeUrl(
    url: string
): Promise<{ content: string; images: ExtractedImage[]; title: string }> {
    if (!FIRECRAWL_API_KEY) {
        throw new Error(
            "FIRECRAWL_API_KEY is not configured. Add it to .env.local"
        );
    }
...
async function scrapeMultipleSources(
    ...
    if (sources.length === 0) {
        throw new Error(
            "Failed to scrape any of the provided URLs. Try selecting different sources."
        );
    }
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        for (const u of urlList) {
            if (!(await isSafeUrl(u))) {
                return NextResponse.json(
                    { error: `Forbidden - Invalid or unsafe URL: ${u}` },
                    { status: 400 }
                );
            }
        }
        ...
        if (urlList.length === 0 && !pastedContent) {
            return NextResponse.json(
                { error: "URL(s) or content is required" },
                { status: 400 }
            );
        }
        ...
        if (unusedImgs) {
            ...
            if (recapIndex > 0) {
                finalContent =
                    finalContent.slice(0, recapIndex) +
                    unusedImgs +
                    finalContent.slice(recapIndex);
            } else {
