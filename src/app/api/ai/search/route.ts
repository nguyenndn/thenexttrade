...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const [educationResults, forumResults, socialResults] =
            await Promise.all([
                ...
                serperSearch(
                    `${topic} forex site:reddit.com OR site:forexfactory.com`
                ).catch(() => []),
                // Tertiary: Quick insights from X/Twitter
                serperSearch(
                    `${topic} forex site:x.com OR site:twitter.com`
                ).catch(() => []),
            ]);
        ...
        const allResults: SearchResult[] = [
            ...educationResults.map((r) => ({
                ...r,
                source: detectSource(r.url, "google"),
            })),
            ...forumResults.map((r) => ({
                ...r,
                source: detectSource(r.url, "reddit"),
            })),
            ...socialResults.map((r) => ({
                ...r,
                source: detectSource(r.url, "twitter"),
            })),
        ];
        ...
        const unique = allResults.filter((r) => {
            ...
            if (BLOCKED_DOMAINS.some((d) => normalized.includes(d)))
                return false;
            if (seen.has(normalized)) return false;
            if (
                primaryUrl &&
                normalized.includes(primaryUrl.replace(/\/$/, "").toLowerCase())
            )
                return false;
            seen.add(normalized);
            return true;
