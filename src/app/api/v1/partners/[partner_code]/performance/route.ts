            ...
            registrations.map(async (reg) => {
                ...
                return {
                    accountInfo: {
                        ...
                        lastUpdated:
                            tradingAccount?.lastSync?.toISOString() || null,
                    },
                    ...stats,
                };
            })
...
function calculatePerformanceStats(
    ...
    const symbolMap = new Map<
        string,
        { profit: number; winRate: number; wins: number; total: number }
    >();

    trades.forEach((trade) => {
        const netPnl =
            (trade.pnl || 0) + (trade.commission || 0) + (trade.swap || 0);
    ...
    const initialBalance =
        reg.tradingCapital || (account?.balance || 0) - totalNetProfit;
    const growthPercent =
        initialBalance > 0 ? round((totalNetProfit / initialBalance) * 100) : 0;
    ...
    const winRatePercent =
        totalTrades > 0 ? round((winCount / totalTrades) * 100) : 0;
    ...
    const growthChartArray = Array.from(dailyMap.entries())
        ...
        .map(([date, day]) => {
            ...
            const ddPct =
                chartPeak > 0
                    ? round(((chartPeak - chartBalance) / chartPeak) * 100)
                    : 0;
            return {
                date,
    ...
    for (const [, month] of monthlyMap) {
        month.growthPct =
            initialBalance > 0
                ? round((month.profit / initialBalance) * 100)
                : 0;
    ...
    return {
        ...
        growthChartArray,
        instrumentDistribution: Array.from(symbolMap.entries()).map(
            ([symbol, data]) => ({
                symbol,
