...
export async function GET(request: Request) {
    ...
    try {
        ...
        entries.forEach((entry) => {
            ...
            const dateStr = new Date(entry.entryDate)
                .toISOString()
                .split("T")[0];
        ...
        const profitFactor =
            totalLossPnL === 0 ? totalWinPnL : totalWinPnL / totalLossPnL;
        ...
        const pairsArr = Object.entries(pairStatsMap).map(
            ([symbol, stats]) => ({
                symbol,
                ...stats,
        ...
        const bestPairObj = pairsArr.reduce(
            (prev, current) => (prev.pnl > current.pnl ? prev : current),
            pairsArr[0]
        );
        const worstPairObj = pairsArr.reduce(
            (prev, current) => (prev.pnl < current.pnl ? prev : current),
            pairsArr[0]
        );
