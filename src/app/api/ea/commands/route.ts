...
export async function POST(request: NextRequest) {
    try {
            ...
            return NextResponse.json(
                {
                    ...
                    details: (validated.error as any).errors,
                } as any,
                { status: 400 }
            );
        }
        ...
        if (!account) {
            return NextResponse.json(
                { error: "Trading account not found" } as any,
                { status: 404 }
            );
        }
        ...
        const isConnected =
            ...
            new Date().getTime() - new Date(account.lastHeartbeat).getTime() <
                300000; // 5 mins tolerance
