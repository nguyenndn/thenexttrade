...
export async function GET(request: Request) {
    ...
    try {
        ...
        const fetchDays = useHourly
            ? Math.max(period * 3, 10)
            : Math.ceil(period * 1.8);
        ...
        await Promise.all(
            pairs.map(async (pair) => {
                const yahooSymbol =
                    PAIR_TO_YAHOO[pair] || pair.replace("/", "") + "=X";
                try {
                    ...
                    console.log(
                        `[Correlation] ${pair}: ${closes.length} closes → ${historicalData[pair].length} returns`
                    );
                } catch (err: any) {
                    console.error(
                        `[Correlation] Failed ${pair}:`,
                        err?.message || err
                    );
                    historicalData[pair] = [];
                }
