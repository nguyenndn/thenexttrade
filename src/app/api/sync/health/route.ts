...
export async function GET() {
    ...
    const summary = {
        ...
        healthy: responseAccounts.filter((a) => a.health.status === "healthy")
            .length,
        needsAttention: responseAccounts.filter((a) =>
            [
                ...
                "missing_trade_data",
                "sync_error",
            ].includes(a.health.status)
        ).length,
        neverSynced: responseAccounts.filter(
            (a) => a.health.status === "no_trades_yet"
        ).length,
        stale: responseAccounts.filter((a) => a.health.status === "stale")
            .length,
        disconnected: responseAccounts.filter(
            (a) => a.health.status === "disconnected"
        ).length,
    };
