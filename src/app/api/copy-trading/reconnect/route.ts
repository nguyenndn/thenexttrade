...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!mt5AccountNumber?.trim()) {
            return NextResponse.json(
                { error: "MT5 account number is required" },
                { status: 400 }
            );
        }
        ...
        if (!registration) {
            return NextResponse.json(
                { error: "No disconnected account found for this MT5 number" },
                { status: 404 }
            );
        }
        ...
        const resolvedBroker = isCustomBroker
            ? registration.customBrokerName || registration.brokerName
            : registration.brokerName;
        const resolvedServer =
            registration.customServer || registration.mt5Server || "";
        ...
        if (PVSR_API_URL && PVSR_API_KEY) {
            ...
            if (!pvsrResponse.ok && pvsrResponse.status !== 409) {
                ...
                return NextResponse.json(
                    {
                        error:
                            pvsrData.error || "Failed to re-register with PVSR",
                    },
                    { status: 502 }
                );
            }
            ...
            console.log(
                `[Reconnect] ✅ PVSR re-register: ${pvsrResponse.status}`,
                pvsrData
            );
        }
        ...
        return NextResponse.json({
            success: true,
            message: "Reconnection request sent. Awaiting PVSR approval.",
        });
    } catch (error) {
