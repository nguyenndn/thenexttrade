    ...
    try {
        ...
        const updatedRegistrations = await Promise.all(
            registrations.map(async (reg) => {
                ...
                try {
                    const pvsrData = await getAccountDetail(
                        reg.mt5AccountNumber
                    );
                    if (!pvsrData) return reg;

                    const pvsrStatus =
                        pvsrData.status as keyof typeof CopyTradingStatus;

                    // If PVSR status changed from PENDING → update local DB
                    if (
                        pvsrStatus !== "PENDING" &&
                        pvsrStatus in CopyTradingStatus
                    ) {
                    ...
                    console.warn(
                        `[Sync] Failed to check PVSR for ${reg.mt5AccountNumber}:`,
                        err
                    );
                }
                return reg;
