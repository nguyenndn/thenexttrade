...
export async function POST(request: NextRequest) {
    try {
        ...
        if (
            ...
            return NextResponse.json(
                {
                    error: "Missing required fields: fullName, email, brokerName, mt5AccountNumber",
                },
                { status: 400 }
            );
        }
        ...
        if (isCustomBroker && !customBrokerName?.trim()) {
            return NextResponse.json(
                {
                    error: "Custom broker name is required when 'Any Broker' is selected",
                },
                { status: 400 }
            );
        }
        ...
        const existing = await prisma.copyTradingRegistration.findUnique({
            where: {
                mt5AccountNumber_brokerName: {
                    mt5AccountNumber,
                    brokerName: isCustomBroker ? customBrokerName : brokerName,
                },
            },
        });
        ...
        if (existing) {
            return NextResponse.json(
                {
                    error: "This MT5 account is already registered for copy trading",
                },
                { status: 409 }
            );
        }
        ...
        const resolvedBroker = isCustomBroker
        ...
        const registration = await prisma.copyTradingRegistration.create({
            data: {
                ...
                customBrokerName: isCustomBroker
                    ? customBrokerName.trim()
                    : null,
                mt5Server: resolvedServer || null,
                customServer:
                    isCustomBroker || isOtherServer
                        ? customServer?.trim() || null
                        : null,
...
async function syncToPVSR(payload: PVSRRegistrationPayload): Promise<void> {
    ...
    if (!apiUrl || !apiKey) {
        console.warn(
            "[PVSR Sync] PVSR_API_URL or PVSR_API_KEY not configured — skipping sync"
        );
        return;
    }
    ...
    if (response.ok) {
        console.log(
            `[PVSR Sync] ✅ Registration forwarded successfully — ID: ${data.registrationId}`
        );
    } else if (response.status === 409) {
        console.log(
            `[PVSR Sync] ⚠️ Account already exists on PVSR — ${payload.mt5Account}`
        );
    } else {
        console.error(
            `[PVSR Sync] ❌ Failed (${response.status}):`,
            data.error
        );
    }
}
