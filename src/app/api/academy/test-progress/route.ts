...
export async function POST(req: NextRequest) {
    ...
    if (!parsed.success) {
        return NextResponse.json(
            { error: "Invalid action. Must be: next, reset, or module" },
            { status: 400 }
        );
    }
    ...
    if (action === "next") {
        ...
        const completedCount =
            lessons.filter((l) => l.progress.some((p) => p.isCompleted))
                .length + 1;
