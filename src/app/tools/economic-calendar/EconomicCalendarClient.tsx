  ...
  return (
    <ToolsPageShell maxWidth="max-w-7xl">
      ...
      <div className="flex items-center gap-2.5 text-xs font-semibold bg-white/60 dark:bg-white/[0.02] border border-gold/15 rounded-xl px-4 py-2.5 mb-8 w-fit shadow-sm relative z-10 backdrop-blur-sm">
        ...
        <ChevronRight size={12} className="text-gray-400 dark:text-gray-600 shrink-0" />
        <span className="text-gray-850 dark:text-gray-200 font-bold truncate">Economic Calendar</span>
      </div>
          ...
          <div className="md:col-span-7 lg:col-span-8 text-left space-y-4">
            ...
            <div className="flex items-center gap-2.5 sm:gap-3">
              ...
              <h1 className="text-[20px] sm:text-3xl md:text-5xl font-black text-slate-800 dark:text-white tracking-tight leading-none font-heading">
                Economic <span className="text-gold">Calendar</span>
              </h1>
            </div>
      ...
      <div className="flex justify-end mb-6">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            title="Refresh Data"
          >
            <RefreshCw size={15} className={isLoading ? "animate-spin text-gold" : "text-gray-500"} />
            Refresh
          </Button>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        {/* Week Navigation Bar */}
        <div className={`rounded-xl border shadow-sm p-1.5 flex items-center justify-between gap-1 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-dashboard'}`}>
            ...
            {weekDays.map((day) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`text-sm md:text-lg font-black text-center ${isSelected ? 'text-white' : ''}`}>
                    {format(day, "d")}
                  </span>
                  {isToday && !isSelected && (
                    <span className={`absolute bottom-0.5 w-1 h-1 rounded-full ${isDark ? 'bg-emerald-400' : 'bg-emerald-500'}`}></span>
                  )}
                </Button>
              );
            })}
        ...
        <div className={`rounded-xl border p-4 flex items-start gap-3 bg-gold/5 border-gold/15`}>
          <TriangleAlert className="text-gold dark:text-gold shrink-0 mt-0.5" size={18} />
          <div>
            <h3 className="text-sm font-bold text-gold dark:text-gold mb-0.5">Trading During News</h3>
      ...
      {/* Calendar Table */}
      <div className={`rounded-xl border overflow-hidden shadow-sm ${isDark ? 'bg-[#151925]/90 border-gold/15 shadow-md' : 'bg-white border-gold/15 shadow-sm'}`}>
        {/* Card Header with Full Date */}
        <div className={`px-4 py-4 flex items-center justify-between border-b ${isDark ? 'border-gold/10 bg-[#151925]' : 'border-gold/10 bg-white'}`}>
        ...
        {/* Table Header */}
        <div className={`grid grid-cols-12 gap-2 md:gap-4 px-4 py-3 border-b text-xs font-bold uppercase tracking-wider ${isDark ? 'bg-slate-900/50 border-gold/10 text-gray-500' : 'bg-gray-50 border-gold/10 text-gray-600'}`}>
        ...
        {isLoading ? (
          ...
          <div className="py-16 text-center flex flex-col items-center justify-center">
            ...
            <Button variant="link" onClick={() => setSelectedDate(new Date())} className="mt-4 text-sm font-bold text-gold hover:text-gold/90 p-0">
              Go to Today
            </Button>
          </div>
        )}
