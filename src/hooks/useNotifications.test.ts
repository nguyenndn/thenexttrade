...
describe("localNotificationStore", () => {
    ...
    it("should add, remove, and mark local notifications as read", () => {
        ...
        expect(readSnapshot.find((n) => n.id === "test-nudge")?.isRead).toBe(
            true
        );

        // Verify localStorage has the title
        expect(localStorage.getItem("read_local_notification_test-nudge")).toBe(
            "Test Title"
        );
