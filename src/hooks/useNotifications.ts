...
export const localNotificationStore = {
  add: (notification: Notification) => {
    ...
    const isAlreadyRead = typeof window !== 'undefined' && 
      localStorage.getItem(`read_local_notification_${notification.id}`) === notification.title;
    ...
    const exists = localNotifications.some(n => n.id === finalNotification.id);
    if (!exists) {
      localNotifications = [finalNotification, ...localNotifications];
      notifyListeners();
    } else {
      // Update existing
      localNotifications = localNotifications.map(n => n.id === finalNotification.id ? finalNotification : n);
      notifyListeners();
    }
  },
  remove: (id: string) => {
      ...
      localNotifications = localNotifications.map(n => {
        if (n.id === id) {
          if (typeof window !== 'undefined') {
            localStorage.setItem(`read_local_notification_${n.id}`, n.title);
          }
...
export function useNotifications(limit = 5) {
  ...
  const [localNotifs, setLocalNotifs] = useState<Notification[]>(localNotifications);
