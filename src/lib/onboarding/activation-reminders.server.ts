 ...
 */
export async function getActivationReminderCandidates(): Promise<ReminderCandidate[]> {
  const now = new Date();
  ...
  for (const user of users) {
    ...
    const hasTrades = user._count.journalEntries > 0 || user.tradingAccounts.some(acc => acc.totalTrades > 0);
    ...
    if (activationReminders.dismissedUntil) {
      const dismissedUntilDate = new Date(activationReminders.dismissedUntil);
      if (dismissedUntilDate > now) {
        continue; // Skip snoozed users
      }
    }

    const sentList: ActivationReminderSend[] = activationReminders.sent || [];
    ...
    const emailsInLast7Days = sentList.filter(s => {
      ...
      return s.channel === "email" && (now.getTime() - sentDate.getTime()) <= 7 * 24 * 60 * 60 * 1000;
    });

    const isEmailPrefsEnabled = !settings.emailPreferences || (
      settings.emailPreferences.marketing !== false &&
      settings.emailPreferences.activation !== false
    );

    const canSendEmail = isEmailPrefsEnabled && emailsInLast7Days.length < 2;
    ...
    if (mobileSyncFallback.firstSeenAt && !hasTrades) {
      ...
      const hoursSinceFallback = (now.getTime() - fallbackSeenDate.getTime()) / (1000 * 60 * 60);
      
      const alreadySent = sentList.some(s => s.type === "MOBILE_SYNC_FALLBACK");
    ...
    if (hoursSinceCreation >= 72) {
      const alreadySent = sentList.some(s => s.type === "STILL_NO_FIRST_VALUE_72H");
      if (!alreadySent) {
        candidates.push({
          userId: user.id,
          email: user.email,
    ...
    if (hoursSinceCreation >= 24 && hasAccounts) {
      const alreadySent = sentList.some(s => s.type === "NO_FIRST_DATA_24H");
      if (!alreadySent) {
        candidates.push({
          userId: user.id,
          email: user.email,
    ...
    if (hoursSinceCreation >= 24 && !hasAccounts) {
      const alreadySent = sentList.some(s => s.type === "NO_ACCOUNT_24H");
      if (!alreadySent) {
        candidates.push({
          userId: user.id,
          email: user.email,
