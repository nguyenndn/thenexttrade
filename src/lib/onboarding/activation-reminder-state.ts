 ...
 */
export async function getActivationReminderState(
    userId: string
): Promise<ActivationReminderState> {
...
export async function appendActivationReminderSend(
    ...
    const existingOnboarding =
        (existingSettings.onboarding as Record<string, any>) || {};
    const existingReminders =
        (existingOnboarding.activationReminders as ActivationReminderState) ||
        {};
 ...
 */
export async function dismissActivationReminder(
    userId: string,
    until: string
): Promise<void> {
    const user = await prisma.user.findUnique({
