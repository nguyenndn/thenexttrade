 ...
 */
export async function getFirstInsightPayload(userId: string): Promise<FirstInsightPayload> {
 ...
 return { shouldShow: false, facts: [], primaryCta: "/dashboard", secondaryCta: "/dashboard" };
 }

 // Check if user already viewed the first insight
 if (firstSession.firstInsightViewedAt || firstSession.firstSyncCelebratedAt) {
 return { shouldShow: false, facts: [], primaryCta: "/dashboard", secondaryCta: "/dashboard" };
 }
 ...
 const totalAccountTrades = accounts.reduce((acc, a) => acc + a.totalTrades, 0);
 ...
 // Fact 1: Trade Count
 facts.push(`You successfully synced/logged your first ${totalTrades} trade(s).`);

 // Fact 2: Net P/L (if pnl values exist)
 const pnlList = entries.map(e => e.pnl).filter((p): p is number => p !== null);
 if (pnlList.length > 0) {
 const netPnL = pnlList.reduce((acc, p) => acc + p, 0);
 const sign = netPnL >= 0 ? "+" : "";
 facts.push(`Your net profit/loss for this period is ${sign}$${netPnL.toFixed(2)}.`);
 }
 ...
 facts.push(`Your initial win rate is ${winRate.toFixed(1)}% (${wins}/${closedEntries.length} wins).`);
 }

 // Fact 4: Top Symbol (if symbol values exist)
 const symbols = entries.map(e => e.symbol).filter((s): s is string => !!s);
 if (symbols.length > 0 && facts.length < 3) {
 const counts: Record<string, number> = {};
 symbols.forEach(s => counts[s] = (counts[s] || 0) + 1);
 const topSymbol = Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b);
 facts.push(`Your most active trading symbol is ${topSymbol}.`);
 }

 // Fact 5: Session Hint (if session values exist)
 const sessions = entries.map(e => e.tradingSession).filter((s): s is string => !!s);
 if (sessions.length > 0 && facts.length < 3) {
 const counts: Record<string, number> = {};
 sessions.forEach(s => counts[s] = (counts[s] || 0) + 1);
 const topSession = Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b);
 facts.push(`You executed most of your trades during the ${topSession} session.`);
 }
 ...
 const secondaryCta = reportCount === 0
