...
describe("Trade Classification", () => {
  ...
  it("should classify positive PnL with [sl] comment as BE_PLUS", () => {
    expect(classifyTradeResult({ pnl: 100, comment: "[sl]" })).toBe("BE_PLUS");
    expect(classifyTradeResult({ pnl: 50, comment: "trailing SL hit" })).toBe("BE_PLUS");
  });
  ...
  it("should classify < 0.2R as BE_PLUS", () => {
    expect(classifyTradeResult({ pnl: 10, riskAmount: 100 })).toBe("BE_PLUS");
  });
