...
import type { DateRange, UserLifecycleReport, LifecycleStage, LifecycleStageStat, LifecycleDropoff } from "./types";
import { pct } from "./date-range";

export async function getUserLifecycleReport(range: DateRange): Promise<UserLifecycleReport> {
 const { since } = range;
 ...
 tradingReports: { select: { id: true, type: true }, where: { type: "WEEKLY" }, take: 1 },
 ...
 const hasPro = user.proEntitlements.some((p) => p.status === "ACTIVE" || p.status === "GRACE");
 const hasAccount = user.tradingAccounts.length > 0;
 const hasTrades = user.journalEntries.length > 0 || user.tradingAccounts.some((a) => a.totalTrades > 0);
 const hasWeeklyReport = user.tradingReports.length > 0;
 const hasProfile = !!user.profile;
 const hasRealAccount = user.tradingAccounts.some((a) => a.balance > 0 || a.totalTrades > 5);
 const latestSync = user.tradingAccounts.reduce<Date | null>((max, a) => {
 ...
 else if (latestSync && latestSync < fourteenDaysAgo) stage = "At Risk";
 }
 stageCounts[stage]++;
 }
 ...
 let cum = 0;
 for (let i = funnelStages.length - 1; i >= 0; i--) { cum += stageCounts[funnelStages[i]]; cumulatives[i] = cum; }

 const dropoffs: LifecycleDropoff[] = [];
 for (let i = 0; i < funnelStages.length - 1; i++) {
 const from = cumulatives[i]; const to = cumulatives[i + 1]; const drop = from - to;
 if (from > 0) dropoffs.push({ fromStage: funnelStages[i], toStage: funnelStages[i + 1], dropCount: drop, dropRate: pct(drop, from), suggestedAction: dropoffActions[`${funnelStages[i]}→${funnelStages[i + 1]}`] || "Investigate" });
 }
