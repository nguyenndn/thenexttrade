...
export async function getDataQualityReport(): Promise<DataQualityReport> {
    ...
    const [
        ...
        prisma.proEntitlement.count({
            where: { tradingAccountId: null, status: "ACTIVE" },
        }),
    ...
    if (usersNoProfile > 0)
        issues.push({
            group: "User / Profile",
            issue: "User without profile",
            count: usersNoProfile,
            severity: usersNoProfile > 50 ? "high" : "medium",
            suggestedFix: "Run profile backfill",
            href: "/admin/users",
        });
    if (usersNoName > 0)
        issues.push({
            group: "User / Profile",
            issue: "Missing user name",
            count: usersNoName,
            severity: "low",
            suggestedFix: "Profile completion nudge",
            href: "/admin/users",
        });
    if (accountsNoBroker > 0)
        issues.push({
            group: "Trading",
            issue: "Account without broker",
    ...
    if (proNoAccount > 0)
        issues.push({
            group: "Pro / IB",
            issue: "Active Pro without linked account",
            count: proNoAccount,
            severity: "high",
            suggestedFix: "Review Pro entitlements",
            href: "/admin/ib",
        });
    if (articlesMissingMeta > 0)
        issues.push({
            group: "Content",
            issue: "Article missing meta title/desc",
            count: articlesMissingMeta,
            severity: "medium",
            suggestedFix: "Use Article Ops bulk SEO fix",
            href: "/admin/articles/ops",
        });
    if (articlesMissingImage > 0)
        issues.push({
            group: "Content",
            issue: "Article missing featured image",
            count: articlesMissingImage,
            severity: "medium",
            suggestedFix: "Generate images via Article Ops",
            href: "/admin/articles/ops",
        });
    ...
    for (const issue of issues) {
        ...
        else if (issue.severity === "high") penalty += 8;
