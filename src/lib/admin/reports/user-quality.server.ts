...
function getRecommendedAction(row: {
    ...
    if (row.qualityBand === "High Quality" && row.balance >= 10000)
        return "Review Pro opportunity";
...
export async function getUserQualityReport(
    range: DateRange
): Promise<UserQualityReport> {
    const { since } = range;
    ...
    for (const vr of vipRequests) {
        if (vr.country)
            countryByUser.set(vr.userId, normalizeCountryCode(vr.country));
    }
    ...
    const rows: UserQualityRow[] = users.map((user) => {
        ...
        const hasReal = user.tradingAccounts.some(
        ...
        const tradeJournalCount =
            user.journalEntries.length +
            user.tradingAccounts.reduce((s, a) => s + a.totalTrades, 0);
        if (tradeJournalCount > 0) score += 15;
        if (user.tradingReports.length > 0) score += 10;
        if (user.edgeEvents.length > 0 || user.missionProgress.length > 0)
            score += 5;
        ...
        const hoursSinceSignup =
            ...
            (1000 * 60 * 60);
        if (
            hoursSinceSignup >= INACTIVE_HOURS &&
            !hasAccount &&
    ...
    return {
        averageScore:
            users.length > 0 ? Math.round(totalScore / users.length) : 0,
        usersWithAccount,
        realAccountUsers,
        inactiveAfterSignup,
        newUsers: users.length,
        topUsers: rows.slice(0, TABLE_LIMIT),
    };
}
