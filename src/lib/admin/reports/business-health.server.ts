...
export async function getBusinessHealthReport(range: DateRange): Promise<BusinessHealthReport> {
  const { since } = range;
  ...
  const fundingRate = totalUsers > 0 ? (fundedAccounts / totalUsers) * 100 : 0;
