...
export async function getReleaseHealthData(): Promise<ReleaseHealthData> {
    ...
    // Reports
    const [weeklyReportsLast7Days, usersWithTrades, usersWithWeeklyReport] =
        await Promise.all([
    ...
    const userIdsWithWeeklyReport = new Set(
        usersWithWeeklyReport.map((u) => u.userId)
    );
    ...
    if (newUserIds.length > 0) {
        [
            ...
            prisma.tradingReport
                .groupBy({
                    by: ["userId"],
                    where: { userId: { in: newUserIds }, type: "WEEKLY" },
                })
                .then((r) => r.length),
    ...
    const eventCounts = await Promise.all([
        ...
        prisma.analyticsEvent.count({
            where: {
                name: "mission_report_cta_clicked",
                createdAt: { gte: sevenDaysAgo },
            },
        }),
        prisma.analyticsEvent.count({
            where: {
                name: "weekly_review_generate_clicked",
                createdAt: { gte: sevenDaysAgo },
            },
        }),
        prisma.analyticsEvent.count({
            where: {
                name: "weekly_review_generate_succeeded",
                createdAt: { gte: sevenDaysAgo },
            },
        }),
        prisma.analyticsEvent.count({
