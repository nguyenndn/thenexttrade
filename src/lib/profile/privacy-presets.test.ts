...
import { applyPrivacyPreset, sanitizePublicProfileData, canExposeSensitiveProfileField } from "./privacy-presets";
...
describe("privacy-presets", () => {
  ...
  describe("canExposeSensitiveProfileField", () => {
    it("should return correct boolean for money exposure", () => {
      expect(canExposeSensitiveProfileField({ showMoney: true }, "money")).toBe(true);
      expect(canExposeSensitiveProfileField({ showMoney: false }, "money")).toBe(false);
    });
  });
});
