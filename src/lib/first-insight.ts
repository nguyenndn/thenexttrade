...
export async function getFirstInsight(
    userId: string
): Promise<FirstInsight | null> {
    ...
    const topSymbol = [...symbolCounts.entries()].sort(
        (a, b) => b[1] - a[1]
    )[0];
    ...
    for (const t of trades) {
        if (t.tradingSession && t.pnl) {
            sessionPnl.set(
                t.tradingSession,
                (sessionPnl.get(t.tradingSession) || 0) + t.pnl
            );
        }
    }
    const bestSession = [...sessionPnl.entries()].sort(
        (a, b) => b[1] - a[1]
    )[0];
    if (bestSession && bestSession[1] > 0) {
        const sessionLabel =
            bestSession[0].charAt(0).toUpperCase() +
            bestSession[0].slice(1).toLowerCase();
        return {
            type: "best_session",
    ...
    const biggestLoss = trades.reduce(
        (worst, t) => (t.pnl !== null && t.pnl < (worst?.pnl ?? 0) ? t : worst),
    ...
    // 4. Best winning trade
    const bestWin = trades.reduce(
        (best, t) => (t.pnl !== null && t.pnl > (best?.pnl ?? 0) ? t : best),
        null as (typeof trades)[0] | null
    );
    ...
    // 5. Missing journal data
    const withoutNotes = trades.filter(
        (t) => !t.entryReason && !t.exitReason && !t.notes && !t.notesPsychology
    );
