...
export async function getAccountDetail(
    mt5Account: string
): Promise<PVSRAccountDetail | null> {
    try {
...
export async function disconnectAccount(
    mt5Account: string,
    reason?: string
): Promise<PVSRDisconnectResult> {
    try {
        const response = await fetch(
            `${PVSR_API_URL}/clients/${mt5Account}/disconnect`,
            {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    reason: reason || "User requested disconnect",
                }),
                signal: AbortSignal.timeout(10_000),
            }
        );
        ...
        return {
            success: false,
            message: "Failed to reach PVSR API",
            status: "error",
        };
    }
}
