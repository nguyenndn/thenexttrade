...
export async function getWeeklyReviewEligibility(params: {
  ...
  const getTradeDate = (t: typeof closedTrades[0]) => t.exitDate || t.entryDate;
  ...
  const diffTime = Math.abs(latestTradeDate.getTime() - firstTradeDate.getTime());
  ...
  if (isFirstWeeklyReview) {
    ...
    if (lastReportEnd) {
      ...
      const daysSince = Math.floor((now.getTime() - lastReportEnd.getTime()) / (1000 * 60 * 60 * 24));
      
      returningReviewReady = daysSince >= 7 && hasClosedTradesAfterLastReport;
