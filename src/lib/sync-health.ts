...
export function computeSyncHealth(input: SyncHealthInput): SyncHealth {
    ...
    const heartbeatDate = newerDate(
        input.lastHeartbeat,
        input.appLastHeartbeat
    );
    ...
    const lastHeartbeatAt = heartbeatDate
        ? typeof heartbeatDate === "string"
            ? heartbeatDate
            : heartbeatDate.toISOString()
        : null;
    const lastSyncAt = input.lastSync
        ? typeof input.lastSync === "string"
            ? input.lastSync
            : input.lastSync.toISOString()
        : null;
    ...
    if (input.totalTrades === 0) {
        ...
        if (heartbeatHours !== null && heartbeatHours < STALE_HEARTBEAT_HOURS) {
            return {
                ...
                description:
                    "Your account is connected. Close a trade in MT5 to sync your first data.",
                primaryAction: {
