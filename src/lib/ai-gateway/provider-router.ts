...
function redactProviderError(message: string | undefined, secret: string): string | undefined {
  if (!message) return undefined;
  let redacted = message.split(secret).join("[REDACTED]");
  redacted = redacted.replace(/(authorization|api[-_ ]?key)\s*[:=]\s*\S+/gi, "$1=[REDACTED]");
  redacted = redacted.replace(/([?&]key=)[^&\s]+/gi, "$1[REDACTED]");
  return redacted.slice(0, 500);
}

export async function executeAiGateway(input: GatewayExecutionInput): Promise<GatewayExecutionResult> {
  const plan = await resolveExecutionPlan();
  const attempts: ProviderAttemptResult[] = [];

  if (!plan.policy) {
    return { ok: false, attempts, error_code: "NO_ROUTING_POLICY", message: "No active AI routing policy found." };
  }
  ...
  for (const step of plan.steps) {
    if (step.kind === "skipped") {
      ...
      lastError = { error_code: step.diagnostic.errorCode, message: step.diagnostic.reason };
      continue;
    }

    const candidate = step.candidate;
    const { model, provider, credential, decryptedSecret, adapter } = candidate;
    ...
    try {
      ...
      attempts.push({
        ...
        errorMessageRedacted: redactProviderError(errorMessage, decryptedSecret),
      ...
      lastError = {
        ...
        message: redactProviderError(errorMessage, decryptedSecret) || "Provider request failed.",
      };
      if (["PROVIDER_AUTH_ERROR", "PROVIDER_CREDENTIAL_INVALID"].includes(lastError.error_code)) break;
    } catch (error) {
      const message = error instanceof Error ? error.message : "Provider adapter failed unexpectedly.";
      attempts.push({
        providerId: provider.id,
        modelId: model.id,
        credentialId: credential.id,
        ...
        errorMessageRedacted: redactProviderError(message, decryptedSecret),
        completedAt: new Date(),
      });
      lastError = { error_code: "PROVIDER_CONNECTION_ERROR", message: "Provider adapter failed unexpectedly." };
    }
  }
...
export async function executeCoachGateway(input: GatewayExecutionInput): Promise<CoachGatewayExecutionResult> {
  const plan = await resolveExecutionPlan();
  const attempts: ProviderAttemptResult[] = [];
  ...
  for (const step of plan.steps) {
    ...
    try {
      ...
      attempts.push({
        ...
        errorMessageRedacted: redactProviderError(providerResult.message, decryptedSecret),
        inputTokens: providerResult.inputTokens,
        outputTokens: providerResult.outputTokens,
        providerRequestId: providerResult.providerRequestId,
        finishReason: providerResult.finishReason,
      ...
      lastError = {
        ...
        message: redactProviderError(providerResult.message, decryptedSecret) || "Provider request failed.",
      };
      if (["PROVIDER_AUTH_ERROR", "PROVIDER_CREDENTIAL_INVALID"].includes(lastError.error_code)) break;
    } catch (error) {
      const message = error instanceof Error ? error.message : "Provider adapter failed unexpectedly.";
