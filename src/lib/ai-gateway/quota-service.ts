...
async function countConsumedRequests(db: Prisma.TransactionClient | typeof prisma, userId: string): Promise<number> {
...
export async function reserveAiRequest(input: ReserveAiRequestInput) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const duplicate = await tx.aiRequest.findUnique({ where: { requestId: input.requestId } });
    if (duplicate) {
