...
export const XAIAdapter: AiGatewayProviderAdapter = {
    ...
    async execute(
        ...
        try {
            res = await fetch(
                input.baseUrl || "https://api.x.ai/v1/chat/completions",
                {
                    method: "POST",
                    headers: {
        ...
        if (aiData.error) {
            return {
                ...
                message:
                    aiData.error.message || "xAI returned an error payload.",
                rawResponseText: responseText,
                httpStatus: res.status,
            };
        }
        ...
        try {
            ...
            return {
                ...
                error_code: "MODEL_RESPONSE_INVALID",
                message:
                    "xAI returned invalid JSON inside the completion content.",
                rawResponseText: responseText,
                httpStatus: res.status,
            };
        }
