...
export function buildGoogleUrl(
    ...
    if (url.includes("{secret}"))
        return url.replace("{secret}", encodeURIComponent(secret));
    if (/([?&]key=)[^&]*/i.test(url)) {
        return url.replace(
            /([?&]key=)[^&]*/i,
            `$1${encodeURIComponent(secret)}`
        );
    }
    ...
    async execute(
        ...
        if (aiData.error) {
            return {
                ...
                message:
                    aiData.error.message || "Google returned an error payload.",
                rawResponseText: responseText,
                httpStatus: res.status,
            };
        }
        ...
        try {
            ...
            return {
                ...
                error_code: "MODEL_RESPONSE_INVALID",
                message:
                    "Google returned invalid JSON inside the completion content.",
                rawResponseText: responseText,
                httpStatus: res.status,
            };
        }
