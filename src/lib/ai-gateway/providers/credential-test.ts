...
async function runCredentialProbe(
  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      return { ok: false, message: `Provider authentication failed (HTTP ${response.status})` };
    }
    return { ok: false, message: `Provider credential probe failed (HTTP ${response.status})` };
  } catch (error) {
...
export function testAnthropicCredential(input: CredentialTestInput): Promise<CredentialTestResult> {
  return runCredentialProbe(
