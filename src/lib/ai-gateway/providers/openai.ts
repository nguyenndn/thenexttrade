...
export const OpenAIAdapter: AiGatewayProviderAdapter = {
  providerCode: "openai",

  testCredential(input) {
    return testOpenAiCompatibleCredential(input, "https://api.openai.com/v1/chat/completions");
  },
  ...
  async execute(input: ProviderExecutionInput): Promise<ProviderExecutionResult> {
    ...
    try {
      res = await fetch(input.baseUrl || "https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: {
    ...
    try {
      ...
      return {
        ...
        message: "OpenAI returned malformed JSON (outer payload).",
        rawResponseText: responseText,
        httpStatus: res.status
      };
    }
    ...
    if (aiData.error) {
      return {
        ...
        message: aiData.error.message || "OpenAI returned an error payload.",
        rawResponseText: responseText,
        httpStatus: res.status
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        error_code: "MODEL_RESPONSE_INVALID",
        message: "OpenAI returned invalid JSON inside the completion content.",
        rawResponseText: responseText,
        httpStatus: res.status
      };
    }
