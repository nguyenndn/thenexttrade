...
describe("AI gateway provider router", () => {
  ...
  it("fails over when the primary provider returns an invalid schema", async () => {
    const primaryExecute = vi.fn().mockResolvedValue({ ok: true, data: { action: "SELL" } });
    const fallbackExecute = vi.fn().mockResolvedValue({ ok: true, data: validResult });
    (resolveExecutionPlan as any).mockResolvedValue({
      policy: { id: "policy-1", version: 3, timeoutMs: 30000 },
      steps: [
        { kind: "candidate", candidate: candidate("primary", primaryExecute) },
        { kind: "candidate", candidate: candidate("fallback", fallbackExecute) },
      ],
    });
  ...
  it("redacts a credential if an adapter includes it in an error", async () => {
    ...
    expect(result.attempts[0].errorMessageRedacted).not.toContain("secret-primary");
  });

  it("records an adapter exception without exposing its raw message", async () => {
    const execute = vi.fn().mockRejectedValue(new Error("network failed with secret-primary"));
    (resolveExecutionPlan as any).mockResolvedValue({
