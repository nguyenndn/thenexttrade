...
const SIGNAL_CONTENT_MAP: Record<string, { academyLessonSlugs: string[]; articleSlugs: string[]; reason: string }> = {
    LOSS_STREAK: {
...
export async function getLearningRecommendations(
    ...
    for (const sig of activeSignals) {
        ...
        const uncompletedAcademySlugs = mapping.academyLessonSlugs.filter(slug => !completedSlugs.has(slug));
