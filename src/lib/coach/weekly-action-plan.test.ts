...
vi.mock("./ai-coach-engine.server", () => ({
  generateWeeklyAIReview: vi.fn().mockResolvedValue({
    ...
    nextActions: [
      { label: "Wait for candle close before touching SL", detail: "Detail" },
      { label: "Wait for candle close before touching SL", detail: "Detail" },
...
describe("Weekly Action Plan Generator", () => {
  it("should generate a plan and integrate AI insights with a limit of 3 actions", async () => {
    ...
    expect(plan.keepDoing).toContain("Focusing on A+ setups in London session");
    expect(plan.fixNext).toContain("Moving stop loss to break even too early");
    ...
    expect(nextActions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: "Wait for candle close before touching SL" })
      ])
    );
    expect(new Set(nextActions.map((action) => `${action.label}|${action.detail}`)).size).toBe(nextActions.length);
