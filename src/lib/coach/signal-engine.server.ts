...
} from "@/lib/journal/trade-outcomes";
...
export async function computeTraderSignals(
    ...
    if (countAccounts === 0) {
        signals.push({
            ...
            summary:
                "Connect your trading account to unlock advanced AI coaching, automatic synchronization, and edge analytics.",
        ...
        if (allSyncedCount === 0 && trades.length === 0) {
            signals.push({
                ...
                summary:
                    "Your account is connected but has never synced trades. Setup Trade Manager EA to automatically track your edge.",
        ...
        if (sortedSyncs.length > 0) {
            ...
            if (now - sortedSyncs[0].getTime() > threeDaysInMs) {
                signals.push({
                    ...
                    summary:
                        "Your account hasn't successfully synchronized trades in over 3 days. Check your Trade Manager EA setup to keep analytics current.",
    ...
    if (countAccounts > 0 && trades.length === 0) {
        signals.push({
            ...
            summary:
                "Start tracking your journey. Sync your MT5 history or manually log your first trade today.",
                ...
                evidence: [
                    {
                        ...
                        label: "Trades Logged",
                        description:
                            "Number of trades currently available for AI analysis.",
                        count: trades.length,
                        sampleSize: 5,
                    },
                ],
    ...
    if (showFirstWeeklyReview) {
        signals.push({
            ...
            summary:
                "You have enough synced trade data to generate your first review.",
    ...
    if (countCompletedLessons === 0) {
        signals.push({
            ...
            summary:
                "Accelerate your trading consistency. Start with our structured lessons and claim Edge XP along the way.",
    ...
    if (trades.length >= 5) {
                    ...
                    evidence: [
                        {
                            ...
                            description:
                                "Number of losing trades in a row without a win.",
                            count: maxStreak,
                        },
                    ],
                },
        ...
        for (const t of trades) {
            if (isConfirmedStopLossExit(t)) {
                const dayStr = new Date(t.entryDate)
                    .toISOString()
                    .split("T")[0];
                    ...
                    evidence: [
                        {
                            ...
                            description:
                                "Maximum number of stop-loss hits in a single day.",
        ...
        for (let i = 0; i < chronologicalTrades.length - 1; i++) {
            ...
            if (classifyTradeOutcome(current) === "LOSS") {
                const timeDiff =
                    next.entryDate.getTime() -
                    (current.exitDate || current.entryDate).getTime();
                const oneDayMs = 24 * 60 * 60 * 1000;
                if (
                    timeDiff > 0 &&
                    timeDiff <= oneDayMs &&
                    next.lotSize >= current.lotSize * 1.5
                ) {
        ...
        if (hasRevengeSizeUp) {
            signals.push({
                ...
                summary:
                    "Your data shows you significantly increased lot size shortly after a losing trade. This is a common revenge-trading trap.",
                    ...
                    evidence: [
                        {
                            ...
                            description:
                                "You sized up significantly (>1.5x) immediately following a loss.",
                        },
                    ],
                },
            });
        ...
        if (reviewedTrades.length >= 5) {
            const nonCompliant = reviewedTrades.filter(
        ...
        const breakEvens = last15.filter(
            (t) => classifyTradeOutcome(t) === "BREAK_EVEN"
        ).length;
                    ...
                    evidence: [
                        {
                            ...
                            description:
                                "Trades exited with near $0 profit/loss out of the last 15.",
                            count: breakEvens,
        ...
        for (const t of trades) {
            if (t.pnl && t.tradingSession) {
                sessionStats[t.tradingSession] = sessionStats[
                    t.tradingSession
                ] || { pnl: 0, count: 0 };
                    ...
                    evidence: [
                        {
                            ...
                            description:
                                "Number of recent trades tagged with this specific mistake.",
                            count: topMistake[1],
                        },
                    ],
                },
