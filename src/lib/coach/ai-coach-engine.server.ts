  ...
  "nextActions": [
    ...
    try {
        ...
        if (!gatewayResult.ok || !gatewayResult.result) {
            console.error(
                `[AI Coach] Gateway failed: ${gatewayResult.error_code} - ${gatewayResult.message}`
            );
            return null;
        }
        ...
        if (
            typeof parsed.keepDoing !== "string" ||
            typeof parsed.fixNext !== "string" ||
            !Array.isArray(parsed.nextActions)
        ) {
            throw new Error("Invalid response format from AI");
        }

        parsed.nextActions = parsed.nextActions.filter(
            (action) =>
                action &&
