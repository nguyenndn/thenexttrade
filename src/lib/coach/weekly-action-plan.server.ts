...
export async function generateWeeklyActionPlan(
    ...
    const followedCount = allPeriodChecks.filter(
        (c) => c.status === "FOLLOWED"
    ).length;
    const brokenCount = allPeriodChecks.filter(
        (c) => c.status === "BROKEN"
    ).length;
    const complianceRate =
        followedCount + brokenCount > 0
    ...
    const mostFollowedRule =
        Object.values(followedMap).sort((a, b) => b.count - a.count)[0] || null;
    const mostBrokenRule =
        Object.values(brokenMap).sort((a, b) => b.count - a.count)[0] || null;
    ...
    const reviewedPlansCount = periodPlans.filter(
    ...
    const actualTradesWithoutPlanCount = Math.max(
        0,
        actualTradesCount - actualTradesWithPlanCount
    );
    ...
    let keepDoing =
        "You logged your trades consistently and tracked key psychology tags.";
    ...
    if (totalUserRulesCount === 0) {
        ...
        keepDoing =
            "You have trading rules set up. Make sure to check them off inside your journal to track compliance.";
        nextActions.push({
    ...
    if (!mostBrokenRule) {
        ...
        if (activeWeaknesses.length > 0) {
            ...
            if (topWeak.signalType === "REVENGE_SIZE_UP") {
                ...
                topWeak.signalType === "LOSS_STREAK" ||
                topWeak.signalType === "SL_CLUSTER"
            ) {
                nextActions.unshift({
    ...
    const plan = await prisma.$transaction(async (tx) => {
        ...
        const existingKeyMap = new Map(
            existingItems.map((i) => [i.stableKey, i])
        );
        ...
        for (let i = 0; i < uniqueActions.length; i++) {
            ...
            await tx.coachActionPlanItem.upsert({
                where: {
                    planId_stableKey: { planId, stableKey: action.stableKey },
                },
                update: {
                    label: action.label,
        ...
        const obsoleteKeys = Array.from(existingKeyMap.keys()).filter(
            (k) => !newKeys.has(k)
        );
