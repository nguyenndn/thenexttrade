...
export function computePlanMatchConfidence(
  ...
  const symbolMatch = plan.symbol.toUpperCase() === entry.symbol.toUpperCase();
  if (!symbolMatch) {
    return { plan, confidence: "LOW", reason: "Symbol does not match" };
  }

  const directionMatch = plan.type === entry.type;
  const accountMatch = plan.accountId && entry.accountId ? plan.accountId === entry.accountId : false;

  const planDate = new Date(plan.plannedAt || plan.createdAt);
  const entryDate = new Date(entry.entryDate);
  const diffHours = Math.abs(planDate.getTime() - entryDate.getTime()) / (1000 * 60 * 60);
  const isCloseDate = diffHours <= 48; // within 48 hours

  if (accountMatch && directionMatch && isCloseDate) {
    return { plan, confidence: "HIGH", reason: "Matches symbol, direction, account, and planned within 48h." };
  }

  if (directionMatch && (!plan.accountId || !entry.accountId)) {
    return { plan, confidence: "MEDIUM", reason: "Matches symbol and direction." };
  }
...
export function getBestPlanMatch(
  ...
  const matches = plans
    ...
    .filter((m) => m.plan.symbol.toUpperCase() === entry.symbol.toUpperCase());
