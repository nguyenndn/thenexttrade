...
function readReferralMetadata(metadata: Prisma.JsonValue | null): ReferralMetadata {
...
export async function resolveReferrerByCode(referralCode: string | null, newUserEmail?: string | null) {
...
export async function getReferralDashboardData(userId: string) {
 ...
 qualifiedDefinition: "A referral is counted after the invited trader verifies their email and their account is created.",
 referrals: events.map((event) => {
 const metadata = readReferralMetadata(event.metadata);

 return {
 id: event.id,
 displayName: metadata.referredName || maskEmail(metadata.referredEmail),
...
export type ReferralDashboardData = Awaited<ReturnType<typeof getReferralDashboardData>>;
