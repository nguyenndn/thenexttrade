...
afterEach(() => {
    ...
    if (ORIGINAL_LEGACY_SECRET === undefined)
        delete process.env.AI_LEGACY_ENCRYPTION_SECRET;
...
describe("credential encryption", () => {
    it("round-trips with the per-record salt", () => {
        process.env.ENCRYPTION_SECRET =
            "primary-secret-that-is-at-least-32-bytes-long";
    ...
    it("decrypts a legacy per-record-salt payload only with explicit migration secret", () => {
        process.env.ENCRYPTION_SECRET =
            "new-primary-secret-that-is-at-least-32-bytes";
        process.env.AI_LEGACY_ENCRYPTION_SECRET =
            "legacy-secret-that-is-at-least-32-bytes";
