...
export function buildAdvancedFiltersSql(filters?: AdvancedFilters) {
  ...
  if (filters.direction) {
    conditions.push(Prisma.sql`AND "type" = ${filters.direction}::"TradeType"`);
  }

  if (filters.source) {
    if (filters.source === "MANUAL") {
      conditions.push(Prisma.sql`AND ("syncSource" = 'MANUAL' OR ("syncSource" = 'APP' AND ("magicNumber" = 0 OR "magicNumber" IS NULL)))`);
    } else if (filters.source === "AUTO") {
      conditions.push(Prisma.sql`AND "syncSource" = 'APP' AND "magicNumber" > 0`);
    }
  }

  if (filters.comment) {
    conditions.push(Prisma.sql`AND "notes" ILIKE ${'%' + filters.comment + '%'}`);
  }
  ...
  if (filters.symbol) {
    conditions.push(Prisma.sql`AND "symbol" ILIKE ${'%' + filters.symbol + '%'}`);
  }

  if (filters.result) {
    conditions.push(Prisma.sql`AND "result" = ${filters.result}::"TradeResult"`);
  }
 ...
 */
export async function getMonthlyAnalytics(userId: string, accountId?: string, timezone?: string, filters?: AdvancedFilters) {
 ...
 */
export async function getKeyStats(userId: string, accountId?: string, startDate?: Date, endDate?: Date, filters?: AdvancedFilters) {
  // Build date filter
...
export const getCachedDashboardStats = unstable_cache(
  async (userId: string, accountId?: string, startDate?: string, endDate?: string, filtersStr?: string) => {
  const start = startDate ? new Date(startDate) : undefined;
  const end = endDate ? new Date(endDate) : undefined;
  const filters = filtersStr ? JSON.parse(filtersStr) as AdvancedFilters : undefined;
 ...
 */
export async function getDailyPerformance(userId: string, accountId?: string, startDate?: Date, endDate?: Date, timezone?: string, filters?: AdvancedFilters) {
  const tz = timezone || 'Etc/UTC';
  // Build date filter
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
 ...
 */
export async function getIntradayPerformance(userId: string, accountId?: string, startDate?: Date, endDate?: Date, filters?: AdvancedFilters) {
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
  : Prisma.empty;
  const advancedFiltersSql = buildAdvancedFiltersSql(filters);
 ...
 */
export async function getSymbolPerformance(userId: string, accountId?: string, startDate?: Date, endDate?: Date, filters?: AdvancedFilters) {
  // Build date filter
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
  : Prisma.empty;
 ...
 */
export async function getTopTrades(userId: string, accountId?: string, startDate?: Date, endDate?: Date, filters?: AdvancedFilters) {
  // Build date filter
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
  : Prisma.empty;
 ...
 */
export async function getLotDistribution(userId: string, accountId?: string, startDate?: Date, endDate?: Date, filters?: AdvancedFilters) {
  // Build date filter
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
  : Prisma.empty;
 ...
 */
export async function getDayOfWeekPerformance(userId: string, accountId?: string, startDate?: Date, endDate?: Date, timezone?: string, filters?: AdvancedFilters) {
  const tz = timezone || 'Etc/UTC';
  // Build date filter
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
 ...
 */
export async function getSessionPerformance(userId: string, accountId?: string, startDate?: Date, endDate?: Date, filters?: AdvancedFilters) {
  const dateFilter = startDate && endDate
  ? Prisma.sql`AND "exitDate" >= ${startDate} AND "exitDate" <= ${endDate}`
  : Prisma.empty;
  const advancedFiltersSql = buildAdvancedFiltersSql(filters);
