    ...
    try {
        ...
        if (process.env.INTERNAL_SECURITY_SECRET) {
            headers["Authorization"] =
                `Bearer ${process.env.INTERNAL_SECURITY_SECRET}`;
        }

        const res = await fetch(
            `${baseUrl}/api/internal/security-log?action=blocked-ips`,
            {
                headers,
            }
        );
...
export async function middleware(request: NextRequest) {
    ...
    if (rateLimitResult && rateLimitCategory) {
        response.headers.set(
            "X-RateLimit-Limit",
            String(RATE_LIMITS[rateLimitCategory].max)
        );
        response.headers.set(
    ...
    if (isRealPageview && !isMaliciousBot(userAgent)) {
        ...
        if (!shouldDedupePageview(sessionId, pathname)) {
            ...
            const utmSource =
                request.nextUrl.searchParams.get("utm_source") || null;
            const utmMedium =
                request.nextUrl.searchParams.get("utm_medium") || null;
            const utmCampaign =
