...
import { addAiCredential, testAiCredential, activateAiCredential } from "./ai-gateway";
...
vi.mock("@/lib/prisma", () => ({
  prisma: {
    ...
    aiProviderCredential: { create: vi.fn(), findUnique: vi.fn(), update: vi.fn(), updateMany: vi.fn() },
...
describe("AI Gateway Admin Actions - Credential Lifecycle", () => {
  ...
  it("adds a new credential in DRAFT status", async () => {
    vi.mocked(prisma.aiProviderCredential.create).mockResolvedValue({ id: "cred_1" } as any);

    const res = await addAiCredential("provider_1", "My Key", "sk_1234");
    
    expect(res.success).toBe(true);
    expect(prisma.aiProviderCredential.create).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("tests a credential and moves it to TESTED if successful", async () => {
    ...
    vi.mocked(getAdapter).mockReturnValue({ testCredential: mockTest } as any);

    const res = await testAiCredential("cred_1");
    expect(res.success).toBe(true);
    expect(mockTest).toHaveBeenCalledWith(expect.objectContaining({ decryptedSecret: "sk_1234" }));
    
    expect(prisma.aiProviderCredential.update).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: "cred_1" },
  ...
  it("moves a credential to INVALID if test fails", async () => {
    ...
    const mockTest = vi.fn().mockResolvedValue({ ok: false, message: "Invalid key" });
    vi.mocked(getAdapter).mockReturnValue({ testCredential: mockTest } as any);
  ...
  it("activates a TESTED credential and revokes others", async () => {
    ...
    // Revokes others
    expect(prisma.aiProviderCredential.updateMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({ status: "ACTIVE", id: { not: "cred_2" } }),
  ...
  it("throws if trying to activate a non-TESTED credential", async () => {
    ...
    await expect(activateAiCredential("cred_3")).rejects.toThrow("Credential must be TESTED before activation");
  });
});
