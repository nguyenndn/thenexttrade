...
export async function applyArticleSeoFixAction(
    articleId: string,
...
export async function generateBulkArticleSeoFixSuggestions(
    articleIds: string[]
) {
    await requireAdminUser();
        ...
        try {
            ...
            if (changed) {
                skipped.push({
                    articleId: entry.articleId,
                    title: entry.title,
                    message: "Article changed after bulk apply; skipped undo.",
                });
                continue;
            }
