...
export interface RuleViolation {
 type: "max_daily_loss" | "max_daily_trades" | "max_risk_percent" | "cooldown_after_losses";
 label: string;
 count: number;
...
export async function getRuleViolations(
 ...
 const { maxDailyLoss, maxDailyTrades, maxRiskPercent, cooldownAfterLosses } = account;
 ...
 const gap = nextEntry.getTime() - currentExit.getTime();
 if (gap < 60 * 60 * 1000) {
 const day = nextEntry.toISOString().split("T")[0];
 if (!violationDates.includes(day)) violationDates.push(day);
 }
 }
 }
 ...
 const complianceRate = tradingDays > 0
