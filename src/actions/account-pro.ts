...
export async function createPartnerProAccount(
 ...
 const { broker, accountNumber, balance, email, telegramId, fullName, country } = parsed.data;
 ...
 if (otherUserAccount) {
 return { error: "This account number is already registered by another user." };
 }
...
function normalizeBroker(raw: string | null | undefined): string {
 ...
 if (upper.startsWith("VTMARKET") || upper.startsWith("VT")) return "VTMARKETS";
...
export async function upgradeToPartnerPro(
 ...
 const balanceStr = account.balance != null ? String(account.balance) : (formData.get("balance") as string) || "0";
