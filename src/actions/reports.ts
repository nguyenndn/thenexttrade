    ...
    for (const [index, r] of reports.entries()) {
        ...
        if (
            ...
            try {
                ...
                console.error(
                    `Auto generate weekly action plan error [type=${type}, userId=${user.id}, reportId=${r.id}]:`,
                    err
                );
                if (
                    process.env.NODE_ENV === "development" ||
                    process.env.NODE_ENV === "test" ||
                    process.env.USER_QA_EMAIL
                ) {
                    throw err;
                }
            }
...
export async function generateMyWeeklyReview(accountId?: string) {
    ...
    const { generateWeeklyReportForUser } =
        await import("@/lib/services/report-generator.service");
    ...
    try {
        ...
        if (result.skipped) {
            ...
            const { getUserMissions } =
                await import("@/lib/services/edge-missions.service");
            const missions = await getUserMissions(user.id);
            const claimable = missions.filter(
                (m) =>
                    ["FIRST_WEEKLY_REVIEW", "WEEKLY_REVIEW"].includes(
                        m.missionId
                    ) &&
                    m.completed &&
                    !m.claimed
                ...
                missionReward:
                    claimable.length > 0
                        ? {
                              ...
                              totalEdge: claimable.reduce(
                                  (sum, m) => sum + m.def.xpReward,
                                  0
                              ),
...
export async function generateMyMonthlyReview(accountId?: string) {
    ...
    const { generateMonthlyReportForUser } =
        await import("@/lib/services/report-generator.service");
