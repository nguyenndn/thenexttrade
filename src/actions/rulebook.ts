...
const ruleSchema = z.object({
  ...
  category: z.enum(["RISK", "ENTRY", "EXIT", "PSYCHOLOGY", "SESSION", "MANAGEMENT"]),
...
export async function updateTradingRule(id: string, data: Partial<z.infer<typeof ruleSchema>>) {
  const user = await getAuthUser();
  if (!user) return { error: "Unauthorized" };
  ...
  try {
    const rule = await prisma.tradingRule.update({
      ...
      data: {
        ...
        strategyId: data.strategyId === undefined ? undefined : data.strategyId || null,
      },
    });
...
export async function updateTraderGoalStatus(id: string, status: "ACTIVE" | "COMPLETED" | "CANCELLED") {
  const user = await getAuthUser();
  if (!user) return { error: "Unauthorized" };
...
export async function saveTradeRuleChecks(
  ...
  checks: Array<{ tradingRuleId: string; status: "FOLLOWED" | "BROKEN" | "SKIPPED"; note?: string }>
) {
  const user = await getAuthUser();
  if (!user) return { error: "Unauthorized" };
...
export async function getRulesComplianceStats() {
  ...
  return rules.map((rule) => {
    const ruleChecks = checkGroups.filter((g) => g.tradingRuleId === rule.id);
    const followed = ruleChecks.find((g) => g.status === "FOLLOWED")?._count._all || 0;
    const broken = ruleChecks.find((g) => g.status === "BROKEN")?._count._all || 0;
    const skipped = ruleChecks.find((g) => g.status === "SKIPPED")?._count._all || 0;
...
export async function addStarterRules() {
  ...
  const starters = [
    { title: "Risk max 1-2% per trade", category: "RISK", severity: "HIGH" as const },
    { title: "Stop after 2 consecutive losses", category: "RISK", severity: "MEDIUM" as const },
    { title: "Only trade confirmed setups", category: "ENTRY", severity: "HIGH" as const },
    { title: "No trading during high-impact news unless planned", category: "ENTRY", severity: "MEDIUM" as const },
    { title: "No revenge entry after a loss", category: "PSYCHOLOGY", severity: "HIGH" as const },
    { title: "Do not increase lot size after a loss", category: "PSYCHOLOGY", severity: "HIGH" as const },
  ];
