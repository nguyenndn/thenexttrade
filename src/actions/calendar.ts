...
export async function getCalendarPerformance(
    monthStr: string,
    accountId?: string
): Promise<{ success: boolean; data?: DailyPerformance[]; error?: string }> {
    try {
        ...
        const result = Array.from(performanceMap.values()).sort((a, b) =>
