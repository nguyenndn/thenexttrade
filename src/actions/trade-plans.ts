...
export async function updateTradePlanStatus(
    id: string,
    status: "PLANNED" | "ACTIVE" | "MATCHED" | "REVIEWED" | "CANCELLED"
) {
    const user = await getAuthUser();
    if (!user) return { error: "Unauthorized" };
...
export async function cancelTradePlan(id: string, reason: string) {
    ...
    try {
        ...
        if (
            ...
            return {
                error: `Cannot cancel trade plan in ${currentPlan.status} state`,
            };
        }
...
export async function linkPlanToTrade(planId: string, journalEntryId: string) {
    ...
    try {
        ...
        if (
            ...
            return {
                error: `Cannot link trade plan in ${currentPlan.status} state`,
            };
        }
