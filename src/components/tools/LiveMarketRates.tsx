    ...
    const fetchRates = useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(
                `/api/tools/rates?symbols=${allSymbols.join(",")}`
            );
            const data = await res.json();
            if (data.success) {
                const map: Record<string, RateData> = {};
                data.rates.forEach((r: RateData) => {
                    map[r.symbol] = r;
                });
                setRates(map);
                ...
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <div className="relative flex-1 min-w-[200px]">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-white/5 border border-dashboard rounded-xl text-sm font-medium text-gray-700 dark:text-white outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
                        />
                    </div>
            ...
            <div className="flex items-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                ...
                <span>
                    ...
                    <span className="font-semibold text-gray-600 dark:text-gray-300">
                        24-hour daily change
                    </span>
                    . Some pairs may not update when markets are closed.
                </span>
            </div>
        </div>
    );
