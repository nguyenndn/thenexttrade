...
function getCellColor(val: number, isDiagonal: boolean): string {
    ...
    if (val >= 0.3)
        return "bg-green-100 dark:bg-teal-700/70 text-green-800 dark:text-teal-100";
    // Neutral
    if (abs < 0.3)
        return "bg-slate-200 dark:bg-slate-700/50 text-slate-600 dark:text-gray-300";
    // Weak negative: lighter red
    if (val > -0.7)
        return "bg-red-100 dark:bg-red-800/70 text-red-800 dark:text-red-100";
    ...
    const fetchCorrelation = useCallback(async () => {
        ...
        try {
            const res = await fetch(
                `/api/tools/correlation?pairs=${PAIRS.join(",")}&period=${period}`
            );
        ...
        <div className="space-y-6">
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                period === p.value
                                    ...
                                    : "bg-white dark:bg-white/5 text-gray-600 dark:text-gray-500 border-dashboard hover:border-primary/50"
                            )}
                        >
                            {period === p.value && (
                                <motion.div
            ...
            {!matrix && !loading && (
                <div className="text-center py-16 text-gray-500">
                    ...
                    <p className="text-sm">
                        Fetches historical data for {PAIRS.length} pairs and
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                !isDiagonal &&
                                                    "hover:brightness-125 hover:scale-[1.03] hover:z-10 cursor-default"
                                            )}
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-center gap-2 text-xs text-gray-600 dark:text-gray-300">
                        ...
                        <span>
                            ...
                            historical price data using Pearson correlation.
                        </span>
                    </div>
                </div>
            )}
