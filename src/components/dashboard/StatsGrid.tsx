    ...
    return (
        <div className={vertical ? "flex flex-col gap-4" : "grid grid-cols-1 md:grid-cols-3 gap-4"}>
            {stats.map((stat, i) => {
                ...
                return (
                    <div key={i} className={`bg-white dark:bg-[#0B0E14] p-5 rounded-xl border border-gray-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow border-t-4 ${borderColor}`}>
