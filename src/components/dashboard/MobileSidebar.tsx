...
function MobileSidebarItem({
    ...
    const isChildActive =
        hasSubItems && item.items.some((sub: any) => sub.href === activeHref);
    ...
    return (
        <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div className="absolute left-0 top-0 bottom-0 w-[272px] bg-white dark:bg-[#0B0E14] shadow-2xl flex flex-col">
                        ...
                        navItems.forEach((item: any) => {
                            ...
                            if (item.items) {
                                item.items.forEach((sub: any) => {
                                    if (
                                        sub.href &&
                                        (effectivePath === sub.href ||
                                            effectivePath.startsWith(
                                                `${sub.href}/`
                                            ))
                                    ) {
                                        if (sub.href.length > bestMatch.length)
                                            bestMatch = sub.href;
                                    }
                                });
                            }
                ...
                {!isAdmin && (
                    <div className="px-3 py-2 border-t border-dashboard">
                        <VipStatusWidget />
                    </div>
                )}

                {/* ── Bottom: Logout ── */}
                <div className="px-3 py-3 border-t border-dashboard">
                    <Button
                        variant="ghost"
                        onClick={() => {
                            onClose();
