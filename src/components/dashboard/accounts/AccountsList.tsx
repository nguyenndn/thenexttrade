...
export function AccountsList({ licenses, eaBrokers }: AccountsListProps) {
    ...
    const [accountToRemove, setAccountToRemove] = useState<EALicense | null>(
        null
    );
    ...
    return (
        <div className="w-full bg-white dark:bg-[#0B0E14] p-6 rounded-xl border border-dashboard overflow-hidden">
            {/* Header: Clean & Flat */}
            ...
            <ConfirmDialog
                ...
                title={
                    accountToRemove?.status === AccountStatus.PENDING
                        ? "Cancel Request"
                        : "Remove Account"
                }
                description={
