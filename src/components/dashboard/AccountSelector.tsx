    ...
    return (
        <span className="inline-flex items-center gap-1.5 truncate">
            ...
            <span className="text-gray-600 dark:text-gray-300">
                - {broker} <span className="text-gray-500">{accNumber}</span>
            </span>
        </span>
    );
}
...
export function AccountSelector({
            ...
            .catch((error) => {
                ...
                toast.error(
                    error instanceof Error
                        ? error.message
                        : error?.message || "Could not load trading accounts"
                );
            })
            .finally(() => {
                setIsLoading(false);
    ...
    useEffect(() => {
        ...
        if (active) {
            ...
            if (typeof window !== "undefined") {
                const event = new CustomEvent("tnt_account_changed", {
                    detail: active.id,
                });
            ...
            if (
                (!urlAccountId || active.id !== urlAccountId) &&
                !hasSetAccount.current
            ) {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                ...
                >
                    ...
                    <span className="flex-1 text-left font-medium overflow-hidden">
                        {selectedAccount ? (
                            <AccountLabelDisplay account={selectedAccount} />
                        ) : isLoading ? (
                            "Loading..."
                        ) : (
                            "Select Account"
            ...
            <PopoverContent
                className="w-[calc(100vw-32px)] sm:w-[360px] p-0"
                align="start"
                sideOffset={6}
            >
                <Command>
                            ...
                            {accounts.map((account) => (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col text-left overflow-hidden">
                                        ...
                                        <span className="text-xs text-gray-600 dark:text-gray-300 mt-0.5 truncate">
                                            {account.name}
                                        </span>
                                    </div>
                                </CommandItem>
