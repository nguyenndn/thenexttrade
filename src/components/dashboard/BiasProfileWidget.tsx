...
export function BiasProfileWidget() {
    ...
    const [profile, setProfile] = useState<{
        biases: BiasData;
        dominantBias: "LOSS_AVERSION" | "FOMO" | "OVERCONFIDENCE" | "EMOTIONAL_CONTAGION";
        assessment: string;
        actionPlan: string;
    ...
    const loadBiasProfile = async () => {
        ...
        try {
            ...
            if (res.success && res.biases) {
                setProfile({
                    biases: res.biases,
                    dominantBias: res.dominantBias as "LOSS_AVERSION" | "FOMO" | "OVERCONFIDENCE" | "EMOTIONAL_CONTAGION",
            ...
            console.error(e);
            setError("Failed to load AI Cognitive Bias Profile. Please try again.");
        } finally {
            setLoading(false);
        }
    };
    ...
    const getBiasDisplayName = (bias: string) => {
        switch (bias) {
            case "LOSS_AVERSION": return "Loss Aversion";
            case "FOMO": return "FOMO (Fear Of Missing Out)";
            case "OVERCONFIDENCE": return "Overconfidence";
            case "EMOTIONAL_CONTAGION": return "Emotional Contagion";
            default: return bias;
        }
    };
            ...
            return (
                <div className="bg-[#1E2028] border border-amber-500/20 px-3 py-1.5 rounded-xl shadow-xl">
                    <p className="text-xs font-bold text-gray-200">
                        {payload[0].name}: <span className="text-amber-500 font-extrabold">{payload[0].value}%</span>
                    </p>
                </div>
            );
        }
    ...
    if (loading) {
        return (
            <div className="relative overflow-hidden rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] p-6 min-h-[460px] lg:h-full flex flex-col items-center justify-center space-y-4">
                <Loader2 className="h-8 w-8 animate-spin text-amber-500" />
                <div className="text-center space-y-1">
                    <p className="text-sm font-bold text-gray-700 dark:text-gray-200">AI is scanning journal logs...</p>
                    <p className="text-xs text-gray-400 dark:text-gray-500">Analyzing emotional tags and metrics to map cognitive biases...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="relative overflow-hidden rounded-2xl bg-white dark:bg-[#141721] p-6 min-h-[460px] lg:h-full flex flex-col justify-between group transition-all duration-300">
                ...
                <div className="space-y-4 relative">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">AI Trading Psychology Bias Map</p>
                        </div>
                    </div>
                    ...
                    <div className="p-4 rounded-xl bg-gray-50 dark:bg-white/[0.02] border border-gray-100 dark:border-white/5 space-y-3">
                        <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
                            <strong>Upgrade to Pro to unlock:</strong> AI will automatically analyze your closed trades combined with before/after entry emotions to identify psychological blind spots causing you to lose money (Loss Aversion, FOMO, Overconfidence).
                        </p>
                        <ul className="space-y-1.5 text-xs text-gray-500 dark:text-gray-400">
                            ...
                            <li className="flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                                Actionable "Tough Love" discipline coaching feedback
                            </li>
                            <li className="flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-amber-500" />
                                Closed-loop learning: Automatically recommended Academy lessons
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div className="space-y-2 relative pt-4">
                    <Link href="/dashboard/accounts?action=add&intent=unlock-pro" className="w-full">
                    ...
                    <p className="text-[10px] text-gray-400 text-center">Or link your broker account via our partner IB referral link.</p>
                </div>
            </div>
        );
    }
    ...
    if (!hasEnoughData) {
        return (
            <div className="relative overflow-hidden rounded-2xl border border-dashed border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] p-6 min-h-[460px] lg:h-full flex flex-col justify-between group transition-all duration-300">
                <div className="space-y-4">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">Cognitive Bias Map</p>
                        </div>
                    </div>
                    ...
                    <div className="p-5 rounded-xl bg-amber-500/[0.01] border border-amber-500/10 text-center space-y-3 my-4">
                        ...
                        <h4 className="text-xs font-bold text-gray-700 dark:text-gray-200">Insufficient Data for Analysis</h4>
                        <p className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed max-w-xs mx-auto">
                            The AI system requires at least **5 closed trades** with emotional tags in your journal to accurately identify your psychological biases.
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-2">
                    <Link href="/dashboard/journal">
                        <Button variant="outline" className="w-full border-amber-500/20 hover:bg-amber-500/10 text-amber-600 font-bold text-xs py-2.5 rounded-xl">
    ...
    if (error || !profile) {
        return (
            <div className="relative overflow-hidden rounded-2xl border border-red-500/20 bg-white dark:bg-[#151925] p-6 min-h-[460px] lg:h-full flex flex-col items-center justify-center space-y-4 text-center">
                ...
                <div>
                    <h3 className="text-sm font-bold text-gray-800 dark:text-white">Unable to Display Bias Map</h3>
                    <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{error || "Unknown error occurred"}</p>
                </div>
                <Button onClick={loadBiasProfile} variant="outline" size="sm" className="mt-2">Try Again</Button>
            </div>
        );
    }
    ...
    const biasData = [
        ...
        { name: "Emotional Contagion", value: profile.biases.emotionalContagion },
    ];
    ...
    return (
        <div className="relative overflow-hidden rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] shadow-lg flex flex-col justify-between min-h-[460px] lg:h-full group transition-all duration-300">
            ...
            <div className="p-5 flex-1 flex flex-col justify-between">
                ...
                <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Dominant Bias</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-1.5 mt-1">
                    <div className="text-center px-1 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-100/50 dark:border-white/5">
                        <p className="text-base font-black text-amber-600 dark:text-amber-400">{profile.biases.lossAversion}%</p>
                        <p className="text-[9px] font-bold text-gray-500 uppercase tracking-wider leading-tight mt-0.5">Loss Aversion</p>
                    </div>
                    <div className="text-center px-1 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-100/50 dark:border-white/5">
                        <p className="text-base font-black text-amber-600 dark:text-amber-400">{profile.biases.fomo}%</p>
                        <p className="text-[9px] font-bold text-gray-500 uppercase tracking-wider leading-tight mt-0.5">FOMO</p>
                    </div>
                    <div className="text-center px-1 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-100/50 dark:border-white/5">
                        <p className="text-base font-black text-amber-600 dark:text-amber-400">{profile.biases.overconfidence}%</p>
                        <p className="text-[9px] font-bold text-gray-500 uppercase tracking-wider leading-tight mt-0.5">Overconfidence</p>
                    </div>
                    <div className="text-center px-1 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.03] border border-gray-100/50 dark:border-white/5">
                        <p className="text-base font-black text-amber-600 dark:text-amber-400">{profile.biases.emotionalContagion}%</p>
                        <p className="text-[9px] font-bold text-gray-500 uppercase tracking-wider leading-tight mt-0.5">Emotional Contagion</p>
                    </div>
                </div>
                ...
                <div className="space-y-2.5 mt-3">
                    ...
                    <div className="p-3 rounded-xl bg-amber-500/[0.02] dark:bg-amber-500/[0.03] border border-amber-500/10">
                        <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest">AI Mentor Assessment:</p>
                    ...
                    <div className="p-3 rounded-xl bg-emerald-500/[0.02] dark:bg-emerald-500/[0.03] border border-emerald-500/10">
                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-widest">Mindset Action Plan:</p>
            ...
            <div className="p-4 border-t border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/[0.01]">
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 overflow-hidden">
                        ...
                        <div className="overflow-hidden">
                            <p className="text-[9px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">Mindset Correction Lesson</p>
                    ...
                    <Link href={profile.recommendedLesson.path} className="shrink-0">
                ...
                <div className="flex items-center gap-1.5 mt-2 justify-center text-[10px] text-gray-400 dark:text-gray-500">
                    ...
                    <span>Complete the recommended lesson to earn **+100 Edge** & a Special Badge!</span>
                </div>
            </div>
        </div>
    );
