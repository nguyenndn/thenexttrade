...
export function DayOfWeekCard({ data }: DayOfWeekCardProps) {
    ...
    const allDays = [
        ...
        (d) =>
            tradingDays.find((td) => td.dayIndex === d.dayIndex) || {
                ...d,
                pnl: 0,
                tradeCount: 0,
                winRate: 0,
            ...
            <div className="flex-1 flex flex-col px-5 pb-5 min-h-0">
                {!hasTrades ? (
                    ...
                    <div className="flex flex-col flex-1 gap-2 justify-between">
                        {allDays.map((d) => {
                            const isBest =
                                bestDay.day === d.day && d.tradeCount > 0;
                            const isWorst =
                                worstDay.day === d.day &&
                                d.pnl < 0 &&
                                d.tradeCount > 0;
                                ...
                                >
                                    ...
                                    <div className="flex-1 h-2 bg-gray-100 dark:bg-white/5 rounded-full overflow-hidden">
                                        <div
                                            ...
                                            style={{
                                                width: `${Math.max(barWidth, d.tradeCount > 0 ? 3 : 0)}%`,
                                            }}
                                        />
                                    </div>

                                    <span
                                        className={`text-xs font-black min-w-[65px] text-right ${
                                            d.pnl >= 0
                                                ? "text-primary"
                                                : "text-red-500"
                                        }`}
                                    ...
                                    <span className="text-[11px] text-gray-500 font-semibold min-w-[70px] text-right shrink-0">
                                        {d.tradeCount > 0
                                            ? `${d.tradeCount}t · ${d.winRate.toFixed(0)}%`
                                            : "--"}
                                    </span>
                                </div>
                            );
