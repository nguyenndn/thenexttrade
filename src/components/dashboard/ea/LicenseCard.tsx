...
import {
    ...
    removeAccount,
} from "@/app/dashboard/trading-systems/actions";
...
export function LicenseCard({ license }: LicenseCardProps) {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            if (res.success) {
                toast.success(
                    isPending ? "Request cancelled" : "Account removed"
                );
                router.refresh();
            } else {
    ...
    const config =
        statusConfig[license.status as keyof typeof statusConfig] ||
    ...
    return (
        <Card
            className={cn(
                "p-6 transition-shadow hover:shadow-md relative overflow-hidden",
                config.border,
                "border-l-4"
            )}
        >
            ...
            <div
                className={cn(
                    "absolute top-4 right-4 px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1.5",
                    config.bg,
                    config.color
                )}
            >
            ...
            <div className="space-y-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-xs text-gray-600 font-medium tracking-wider uppercase mb-0.5">
                            Account Number
                        </p>
                        <h3 className="text-xl font-bold text-gray-700 dark:text-white font-mono">
                            {license.accountNumber}
                        </h3>
                    </div>
                </div>

                <div className="pt-4 border-t border-dashboard space-y-2 text-sm">
