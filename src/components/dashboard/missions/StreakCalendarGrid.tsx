...
import { Flame, CheckCircle, TrendingUp, HelpCircle, Loader2 } from "lucide-react";
...
export function StreakCalendarGrid() {
  const [data, setData] = useState<{ checkInHistory: string[]; tradeHistory: string[] } | null>(null);
  ...
  if (isLoading) {
    return (
      <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] p-6 flex flex-col items-center justify-center min-h-[220px]">
        <Loader2 className="animate-spin text-amber-500" size={32} />
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 font-medium">Loading streak history...</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] p-6 shadow-sm">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">
            Your daily trading and check-in consistency over the past 365 days.
          </p>
        </div>

        {/* Mini stats */}
        <div className="flex items-center gap-6 text-xs font-bold text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-white/5 px-4 py-2.5 rounded-xl border border-gray-100 dark:border-white/5">
          <div className="flex items-center gap-1.5">
            <CheckCircle className="text-amber-500" size={14} />
            <span>Check-in Days: <span className="text-amber-500">{totalCheckIns}</span></span>
          </div>
          <div className="h-4 w-px bg-gray-200 dark:bg-white/10" />
          <div className="flex items-center gap-1.5">
            <TrendingUp className="text-amber-500" size={14} />
            <span>Trading Days: <span className="text-amber-500">{totalTradeDays}</span></span>
          </div>
        </div>
      </div>
              ...
              {weeks.map((week, wIndex) => (
                <div key={wIndex} className="flex flex-col gap-1">
                  {week.map((day) => {
                    ...
                    const hasTraded = trades.includes(dateStr);
                    const hasCheckedIn = checkIns.includes(dateStr);

                    let title = format(day, "MMM dd, yyyy");
                    let subtitle = "No Activity";
                    let colorClass = "bg-gray-100 dark:bg-white/5 border-transparent";

                    if (hasTraded) {
                      subtitle = "Trading Day (Trades Logged)";
                      colorClass =
                      ...
                      colorClass = "bg-amber-500/25 border border-amber-500/30 text-amber-500 dark:bg-amber-500/20";
                    }

                    if (isTodayDay && !hasCheckedIn && !hasTraded) {
                      colorClass = "border border-dashed border-amber-500/50 bg-gray-50 dark:bg-white/5 animate-pulse";
                    }
                    ...
                    return (
                      <Tooltip key={day.toISOString()}>
                        ...
                        <TooltipContent side="top" className="bg-[#121620] border-white/10 text-white font-bold p-3 rounded-lg shadow-xl text-xs space-y-1 z-50">
