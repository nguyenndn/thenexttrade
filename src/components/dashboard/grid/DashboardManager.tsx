...
export function DashboardManager({
    ...
    const [heroWidgets, setHeroWidgets] = useState<HeroWidgetType[]>([
        "TOTAL_BALANCE",
        "PERIOD_PNL",
        "WIN_RATE",
        "TRADE_SCORE",
    ...
    const [freeStyleBackup, setFreeStyleBackup] = useState<LayoutItem[] | null>(
        null
    );
    ...
    useEffect(() => {
        if (activeDashboard && activeDashboard.layout) {
            ...
            const parsed =
                typeof activeDashboard.layout === "string"
            ...
            if (parsed && !Array.isArray(parsed) && parsed.grid) {
                ...
                const validLayout = migratedGrid
                    .filter(
                        (item: any) => WIDGET_REGISTRY[item.type as WidgetType]
                    )
                    .map((item: any) => {
    ...
    const applySpecialLayout = (
        type: "focus-left" | "focus-right" | "split" | "single-col" | "4-cols"
    ) => {
        if (layout.length === 0) return;
    ...
    const submitCreateDashboard = async () => {
        ...
        startTransition(async () => {
            try {
                ...
                const newDb = await createDashboard(
            ...
            {isEditable ? (
                <>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent
                            align="end"
                            className="w-56 rounded-xl bg-white dark:bg-[#1E2028] border border-gray-200 dark:border-[#382F1D] max-h-[380px] overflow-y-auto scrollbar-thin"
                        >
                            <DropdownMenuItem
                            ...
                            <DropdownMenuItem
                                onClick={() =>
                                    applySpecialLayout("focus-right")
                                }
                                className="cursor-pointer rounded-lg px-3 py-2 my-0.5 text-slate-700 dark:text-slate-200 hover:bg-gray-100 dark:hover:bg-white/5"
                            >
                                Focus Right (1/3 : 2/3)
                ...
                <>
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent
                            align="start"
                            className="w-56 rounded-xl bg-white dark:bg-[#1E2028] border border-gray-200 dark:border-[#382F1D]"
                        >
                            {dashboards.map((db) => (
                                <DropdownMenuItem
                                    key={db.id}
                                    ...
                                    <div className="flex items-center gap-2 text-gray-400">
                                        <Pin
                                            className={cn(
                                                "w-4 h-4",
                                                activeId === db.id &&
                                                    "text-amber-500 fill-amber-500 dark:fill-amber-400"
                                            )}
                                        />
                                        <div
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col 2xl:flex-row items-stretch 2xl:items-center justify-between gap-4 w-full">
                <div className="w-full 2xl:w-auto">{controls}</div>
            ...
            <div
                className={cn(
                    "min-h-[500px] transition-[background-color,border-color] duration-200 -mx-2",
                    isEditable &&
                        "bg-gray-50 dark:bg-gray-800/30 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 p-0"
                )}
            >
                {layout.length === 0 ? (
                    ...
                    <WidgetGrid
                        layout={layout.map((item: any) => {
                            const widgetConfig =
                                WIDGET_REGISTRY[item.type as WidgetType];
                            return {
                                ...item,
                    ...
                    >
                        {layout.map((item: any) => {
                            const widgetConfig =
                                WIDGET_REGISTRY[item.type as WidgetType];
                            if (!widgetConfig)
                                return <div key={item.i}>Invalid Widget</div>;
                ...
                <DialogContent>
                    ...
                    <div className="py-4">
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            ...
                            {activeDashboard?.name}"? This action cannot be
                            undone.
                        </p>
                    </div>
                    <DialogFooter className="gap-2 sm:gap-2">
...
function getWidgetProps(
    ...
    switch (type) {
        ...
        case "DAILY_WIN_RATE":
            return { data: data.dailyWinRates, dates: data.selectedDates };
        case "PROFIT_DIST":
            return { data: data.symbolPerformance };
        case "LOT_DIST":
            return { data: data.lotDistribution };
        case "MONTHLY_ANALYTICS":
            return { data: data.monthlyAnalytics };
        case "TOP_TRADES":
            return {
                bestTrades: data.bestTrades,
                worstTrades: data.worstTrades,
            };
        case "SYMBOL_PERF":
            return { data: data.symbolAnalytics };
        case "RECENT_TRADES":
            return { trades: data.recentTrades, onTradeClick, onAddTrade };
        case "TRADING_SESSIONS":
            return { data: data.sessionPerformance };
        case "DAY_OF_WEEK":
            return { data: data.dayOfWeekPerformance };
        case "TRADING_CALENDAR":
            return {
                data: data.dailyPerformance || [],
                selectedDates: data.selectedDates,
            };
        case "WIN_LOSS_COMPARISON":
            return {
                avgWin: data.dashboardData?.avgWin || 0,
                avgLoss: data.dashboardData?.avgLoss || 0,
            };
        case "PL_HEATMAP":
            return {
                data: data.dailyPerformance || [],
