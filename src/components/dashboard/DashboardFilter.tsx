...
export function DashboardFilter({ currentAccountId, className, hideDateFilter, equalWidth, defaultToAllTime }: DashboardFilterProps) {
  ...
  const [magicNumber, setMagicNumber] = useState(searchParams.get("magicNumber") || "");
  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success("Sync request sent! The Trade Manager EA will process it shortly.");
      } else {
  ...
  const handleApplyFilters = () => {
    const params = new URLSearchParams(searchParams.toString());
    if (direction) params.set("direction", direction); else params.delete("direction");
    if (source) params.set("source", source); else params.delete("source");
    if (comment) params.set("comment", comment); else params.delete("comment");
    if (magicNumber) params.set("magicNumber", magicNumber); else params.delete("magicNumber");
  ...
  return (
    <div className={cn("flex flex-col md:flex-row md:flex-nowrap items-stretch md:items-center gap-3 w-full md:w-auto", className)}>
      <div className="flex items-center gap-2 w-full md:w-auto flex-1 md:flex-initial">
        <Button
          onClick={handleSyncNow}
          disabled={isSyncing || !currentAccountId || currentAccountId === "all"}
          variant="outline"
        ...
        >
          <RefreshCw size={14} className={cn("shrink-0", isSyncing && "animate-spin text-amber-500")} />
          <span>Sync Now</span>
        </Button>
        <AccountSelector currentAccountId={currentAccountId} className="flex-1 md:flex-initial min-w-[150px]" />
      </div>
      ...
      {!hideDateFilter && (
        <div className="flex items-center gap-2 w-full md:w-auto flex-1 md:flex-initial">
          ...
          <Popover open={isOpen} onOpenChange={setIsOpen}>
            ...
            <PopoverContent align="end" className="w-[300px] border border-gray-200 dark:border-[#382F1D] bg-white dark:bg-[#1C1E24] shadow-2xl rounded-2xl p-4">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white mb-3">Advanced Filters</h3>
              
              <div className="space-y-3">
                {/* Direction Filter */}
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Direction</label>
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Trade Outcome</label>
                  <div className="grid grid-cols-4 gap-1 bg-slate-100 dark:bg-slate-900 p-0.5 rounded-lg">
                    {[
                      { label: "ALL", value: "" },
                      { label: "WIN", value: "WIN" },
                      { label: "LOSS", value: "LOSS" },
                      { label: "BE", value: "BREAK_EVEN" }
                    ].map((item) => (
                      <button
                        key={item.value}
                        type="button"
                ...
                {/* Source Filter */}
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Trade Source</label>
                  <div className="grid grid-cols-3 gap-1 bg-slate-100 dark:bg-slate-900 p-0.5 rounded-lg">
                    {["", "MANUAL", "AUTO"].map((val) => (
                      <button
                        key={val}
                ...
                {/* Symbol Filter */}
                <div className="space-y-1.5">
                  <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Symbol</label>
                  <Input
                    value={symbol}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Magic Number</label>
                    <Input
                      type="number"
                      value={magicNumber}
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Trade Comment</label>
                    <Input
                      value={comment}
