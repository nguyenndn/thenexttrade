...
export function MonthlyAnalyticsChart({ data }: MonthlyAnalyticsChartProps) {
    const yearGroups = useMemo(() => {
        ...
        data.forEach((item) => {
            ...
            const existing = groups[year].find(
                (d: any) => d.monthIndex === monthIndex
            );
            if (existing) {
    ...
    const CustomTooltip = ({ active, payload }: any) => {
        ...
        return (
            <div className="bg-white dark:bg-[#1E2028] p-3 border border-dashboard rounded-xl shadow-xl backdrop-blur-sm">
                <p className="text-xs font-bold text-gray-700 dark:text-gray-100 mb-1">
                    {d.month}
                </p>
                <div className="flex items-center gap-2">
                    <div
                        className={`w-2 h-2 rounded-full ${d.value >= 0 ? "bg-emerald-500" : "bg-red-500"}`}
                    />
                    <p
                        className={`text-sm font-black ${d.value >= 0 ? "text-primary" : "text-red-500"}`}
                    >
                ...
                <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                    ...
                    >
                        ...
                        <Tooltip
                            ...
                            cursor={{ fill: "var(--hover-bg)", opacity: 0.06 }}
                        />
                        <Bar
                            dataKey="value"
                            radius={[6, 6, 0, 0]}
                        ...
                        >
                            {chartData.map((entry: any, index: number) => (
                                <Cell
                                    ...
                                    fill={
                                        entry.value >= 0
                                            ? "url(#monthProfitGradient)"
                                            : "url(#monthLossGradient)"
                                    }
                                />
                            ))}
                        </Bar>
