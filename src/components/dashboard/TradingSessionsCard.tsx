...
const SESSION_CONFIG: Record<
    string,
    { icon: typeof Globe; color: string; bgColor: string; hours: string }
> = {
    Sydney: {
        icon: Sunset,
...
export function TradingSessionsCard({ data }: TradingSessionsCardProps) {
    const bestSession =
        data.length > 0
            ? data.reduce((best, s) => (s.pnl > best.pnl ? s : best), data[0])
            : null;
                ...
                {data.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {data.map((session) => {
                            ...
                            const tradePercent =
                                totalTrades > 0
                                    ? (session.trades / totalTrades) * 100
                                    : 0;
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={`p-1.5 rounded-lg ${config.bgColor} ${config.color} shrink-0`}
                                        >
                                            <Icon size={14} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between mb-1">
                                                ...
                                                <span
                                                    className={`text-xs font-black ${session.pnl >= 0 ? "text-primary" : "text-red-500"}`}
                                                >
                                                    {session.pnl >= 0
                                                        ? "+"
                                                        : ""}
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="flex-1 h-2 bg-gray-200 dark:bg-white/10 rounded-full overflow-hidden">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${Math.min(tradePercent, 100)}%`,
                                                        }}
                                                    />
                                                </div>
