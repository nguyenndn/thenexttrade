...
export function WeeklyCoachPlan({ plan }: WeeklyCoachPlanProps) {
    ...
    const handleToggle = (itemId: string, currentStatus: string) => {
        ...
        startTransition(async () => {
            const result = await toggleCoachActionPlanItem(
                plan.id,
                itemId,
                !isCompleted
            );
                        ...
                        return (
                            ...
                            >
                                <button
                                    type="button"
                                    onClick={() =>
                                        handleToggle(action.id, action.status)
                                    }
                                    disabled={isLegacy || isPending}
                                    aria-label={`Toggle completion for ${action.label}`}
                                    title={
                                        isLegacy
                                            ? "Legacy action items cannot be toggled"
                                ...
                                <div className="space-y-0.5 flex-1 min-w-0">
                                    <p
                                        className={`text-sm font-black text-gray-800 dark:text-white leading-tight ${isChecked ? "line-through" : ""}`}
                                    >
                                        {action.label}
                                    </p>
