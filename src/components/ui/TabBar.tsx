...
export function TabBar({ tabs, className, equalWidth }: TabBarProps) {
    ...
    return (
        <div
            className={cn(
                "bg-[#F1F3F5] dark:bg-[#1A1D27] p-1 rounded-xl border border-dashboard h-auto overflow-x-auto scrollbar-hide",
                equalWidth
            ...
            {tabs.map((tab) => {
                ...
                return (
                    <Link
                        ...
                        href={tab.href}
                        className={cn(
                            "rounded-lg px-3 sm:px-4 py-1.5 text-sm font-bold transition-all duration-300 flex items-center justify-center gap-1.5 sm:gap-2 border whitespace-nowrap shrink-0",
