...
export function TabsList({ children, className }: TabsListProps) {
 return (
 <div className={cn("flex items-center gap-1 bg-[#F1F3F5] dark:bg-[#1A1D27] p-1 rounded-xl border border-dashboard w-fit", className)}>
 {children}
 </div>
 );
}
...
export function TabsTrigger({ value, children, className, activeIndicatorClassName, activeTextClassName }: TabsTriggerProps) {
 ...
 isActive
 ? (activeTextClassName || "text-gray-700 dark:text-white")
 : "text-gray-600 hover:text-gray-700 dark:text-gray-500 dark:hover:text-gray-200 border-transparent",
 className
 )}
 >
 {isActive && (
 ...
 className={cn(
 "absolute inset-0 bg-white dark:bg-[#262A36] shadow-sm rounded-lg -z-10 border border-dashboard ",
