...
import { Cpu, Shield, ShieldAlert, Sparkles, Bot, SlidersHorizontal } from "lucide-react";
...
export function TradingSystemsDetailTabs({ system }: DetailTabsProps) {
  ...
  const gradientActive = {
    ...
    emerald: "!bg-gradient-to-r from-emerald-500 to-teal-600 shadow-md border-0",
  }[colorTheme] || "!bg-gradient-to-r from-gold to-amber-600 shadow-md border-0";
  ...
  return (
    ...
    >
      <div className="relative z-10">
        <Tabs value={activeTab} onValueChange={setActiveTab} tabsId={`system-detail-${system.slug}`}>
          <div className="mb-8 flex flex-col gap-5 border-b border-slate-200/70 dark:border-white/5 pb-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <h3 className="text-base font-black text-gray-850 dark:text-white uppercase tracking-wider flex items-center gap-2 mt-1">
                {activeTab === "overview" && "Key Features & Functions"}
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <TabsTrigger
                    ...
                    activeIndicatorClassName={gradientActive}
                ...
                {system.features.map((item, idx) => (
                  <div
                    ...
                    className={cn(
                      ...
                      colorTheme === "gold" && "hover:border-gold/30 hover:shadow-md",
                      colorTheme === "blue" && "hover:border-blue-500/30 hover:shadow-md",
                      colorTheme === "emerald" && "hover:border-emerald-500/30 hover:shadow-md"
                    )}
                  >
                    ...
                    <div
                      className={cn(
                        ...
                        colorTheme === "gold" && "bg-gold/10 text-gold border-gold/15",
                        colorTheme === "blue" && "bg-blue-500/10 text-blue-500 border-blue-500/15",
                        colorTheme === "emerald" && "bg-emerald-500/10 text-emerald-500 border-emerald-500/15"
                      )}
                    >
                      <TabIcon size={18} />
                    </div>
