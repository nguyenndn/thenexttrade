    ...
    if (loading) {
        return (
            <div className="p-8 text-center text-gray-500">
                <Loader2 className="mx-auto animate-spin" /> Loading sessions...
            </div>
        );
    }
            ...
            <div className="space-y-4">
                ...
                {sessions.map((session) => {
                    ...
                    return (
                        <div
                            key={session.id}
                            className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isCurrent ? "bg-green-50/50 dark:bg-green-500/5 border-green-200 dark:border-green-500/30" : "bg-gray-50 dark:bg-white/5 border-dashboard "}`}
                        >
                            <div className="flex items-center gap-4">
                                <div
                                    className={`w-10 h-10 rounded-full flex items-center justify-center ${isCurrent ? "bg-green-100 dark:bg-green-500/20" : "bg-white dark:bg-white/10"}`}
                                >
                            ...
                            {isCurrent && (
                                <div className="text-green-500 text-xs font-bold px-3">
                                    Active Now
                                </div>
                            )}
                        </div>
