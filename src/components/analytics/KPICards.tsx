...
export function KPICards({ summary }: KPICardsProps) {
    ...
    const cards = [
        ...
        },
        {
            ...
            description:
                (summary.avgRRR || 0) >= 2 ? "Excellent" : "Review exits",
        },
        {
            title: "Total Trades",
    ...
    return (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-5">
            {cards.map((card, index) => {
                ...
                else if (card.color.includes("red"))
                    colorClass = "text-red-500";

                return (
                    <div
                        key={index}
                        className={`bg-white dark:bg-[#1E2028] p-6 rounded-xl border border-dashboard shadow-sm hover:shadow-md transition-shadow duration-200 group border-t-4 ${card.color.replace("text-", "border-t-")}`}
                    >
                        <div className="flex items-center gap-3 mb-4">
                            <div
                                className={`p-3 rounded-xl ${card.bg} text-opacity-80 group-hover:bg-opacity-100 transition-colors shadow-sm relative z-10`}
                            >
                                <Icon
                                    size={20}
                        ...
                        <p
                            className={`text-[28px] font-bold tracking-tight ${colorClass} group-hover:scale-[1.02] transform origin-left transition-transform relative z-10`}
                        >
                        ...
                        {card.description && (
                            <p className="text-xs text-gray-500 font-medium mt-1.5 flex items-center gap-1.5 relative z-10">
                                <span
                                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${card.color.replace("text-", "bg-")}`}
                                ></span>
                                {card.description}
                            </p>
                        )}
