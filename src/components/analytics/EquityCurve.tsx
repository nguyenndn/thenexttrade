...
export function EquityCurve({ data }: EquityCurveProps) {
    ...
    const { isEmpty, chartData, firstBalance, totalNetProfit, splitOffset } =
        processEquityCurveData(data);
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] p-6 rounded-xl border border-dashboard shadow-sm hover:shadow-md transition-shadow duration-200 group/card">
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="text-right">
                    ...
                    <p
                        className={`text-lg font-black ${totalNetProfit >= 0 ? "text-primary" : "text-red-500"}`}
                    >
                        {totalNetProfit >= 0 ? "+" : ""}$
                        {Math.abs(totalNetProfit).toLocaleString("en-US", {
                            ...
                            <Tooltip
                                content={({ active, payload, label }) => {
                                    if (active && payload && payload.length) {
                                        ...
                                        const pnlVal = payload[0].payload
                                            .pnl as number;
                                        return (
                                            <div className="bg-white dark:bg-[#1E2028] p-3 border border-dashboard rounded-xl shadow-xl">
                                                ...
                                                <p
                                                    className={`text-xs font-bold ${pnlVal >= 0 ? "text-primary" : "text-red-500"}`}
                                                >
                                                    PnL:{" "}
                                                    {pnlVal >= 0 ? "+" : "-"}$
                                                    {Math.abs(
