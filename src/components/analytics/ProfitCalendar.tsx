...
export function ProfitCalendar({
        ...
        try {
            ...
            if (res && res.trades) {
                ...
                if (equityCurve && equityCurve.length > 0) {
                    const dayIdx = equityCurve.findIndex(
                        (e: any) => e.date === dateKey
                    );
                    if (dayIdx >= 0) {
            ...
            toast.error(
                err instanceof Error
                    ? err.message
                    : err?.message || "Failed to load details for this day."
            );
        } finally {
            setLoadingDay(null);
        }
    ...
    return (
        <div
            ref={calendarRef}
            className="bg-white dark:bg-[#1E2028] p-6 rounded-xl border border-dashboard shadow-sm hover:shadow-md transition-shadow duration-200 group"
        >
            {/* Header */}
            <div className="flex items-center justify-between mb-6">
                ...
                <p className="text-base sm:text-lg font-bold text-gray-700 dark:text-gray-200">
                    ...
                    <span
                        className={
                            monthlyPnL >= 0 ? "text-primary" : "text-red-500"
                        }
                    >
                        {monthlyPnL >= 0 ? "" : "-"}$
                        {Math.abs(monthlyPnL).toLocaleString("en-US", {
 ...
 `}
                                                title={
                                                    hasTrades
                                                        ? `$${pnl.toFixed(2)} (${dayData?.tradeCount} trades)`
                                                        : "No trades"
                                                }
                                            >
                                                <span
                                                    className={`text-[10px] font-bold absolute top-1.5 left-2 ${hasTrades ? "text-gray-700 dark:text-gray-100" : "text-gray-700 dark:text-gray-500"}`}
                                                >
                                                    {format(day, "d")}
                                                </span>
                                    ...
                                    <div className="min-h-[80px] p-2 rounded-xl flex flex-col items-center justify-center bg-white dark:bg-[#1E2028] border border-dashboard relative shadow-sm hover:shadow-md transition-all">
                                        <span
                                            className={`text-[10px] font-bold absolute top-1.5 left-2 ${weeklyPnL >= 0 ? "text-primary" : "text-red-500"}`}
                                        >
                                            W{weekIndex + 1}
                                        </span>
