...
function computeRadarData(data: IntelligenceData): RadarDimension[] {
    ...
    return [
        ...
        },
        {
            ...
            value: Math.round(
                Math.min(100, Math.max(0, quickStats.avgRR * 33))
            ),
            fullMark: 100,
        ...
        },
        {
            dimension: "Risk Mgmt",
            value: Math.round(
                Math.min(100, Math.max(0, quickStats.slUsageRate))
            ),
            fullMark: 100,
        ...
        },
        {
            ...
            value: Math.round(
                Math.min(
                    ...
                    Math.max(
                        0,
                        quickStats.planComplianceRate >= 0
                            ? quickStats.planComplianceRate
                            : 50
                    )
                )
            ),
        ...
        },
        {
            ...
            value: Math.round(
                Math.min(100, Math.max(0, 100 - quickStats.revengeCount * 12))
            ),
            fullMark: 100,
            description:
                quickStats.revengeCount > 0
...
function CustomTooltip({ active, payload }: any) {
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] border border-dashboard rounded-xl px-3 py-2 shadow-xl">
            <p className="text-sm font-bold text-gray-700 dark:text-white mb-0.5">
                {d.dimension}:{" "}
                <span style={{ color: getScoreColor(d.value) }}>{d.value}</span>
                /100
            </p>
...
export function BehavioralRadarChart({ data }: BehavioralRadarChartProps) {
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard shadow-sm p-5 min-h-[460px] lg:h-full flex flex-col justify-between">
            {/* Header */}
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500">
                            Behavioral analysis across 6 dimensions
                        </p>
                    </div>
                </div>
                <div className="text-right">
                    <p className={`text-lg font-black ${grade.color}`}>
                        {grade.label}
                    </p>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 mt-1">
                {radarData.map((d) => (
                    <div
                        key={d.dimension}
                        className="text-center px-1 py-2 rounded-lg bg-gray-50 dark:bg-white/[0.03]"
                    >
                        <p
                            className="text-lg font-black"
                            style={{ color: getScoreColor(d.value) }}
                        >
                            {d.value}
                        </p>
