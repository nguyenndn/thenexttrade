...
import { Brain, Sparkles, AlertCircle, Target, ChevronRight, Loader2, Bot, RefreshCw } from "lucide-react";
...
export function DeepSeekCoachCard({ accountId, timezone, dateFrom, dateTo }: DeepSeekCoachCardProps) {
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-indigo-200 dark:border-indigo-500/30 shadow-[0_4px_20px_-4px_rgba(99,102,241,0.1)] overflow-hidden">
            {/* Header */}
            <div className="bg-gradient-to-r from-indigo-50 to-transparent dark:from-indigo-500/10 dark:to-transparent border-b border-indigo-100 dark:border-indigo-500/20 px-5 py-3 flex items-center justify-between">
            ...
            <div className="p-5 sm:p-6">
                {isLoading ? (
                    ...
                    <div className="flex items-center gap-3 p-4 bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 rounded-xl">
                        ...
                        <Button variant="outline" size="sm" onClick={handleGenerate} className="ml-auto">Retry</Button>
                    </div>
                ) : insight ? (
                    <div className="space-y-6">
                        ...
                        <div className="pt-2 flex justify-end">
                            <Button 
                                ...
                                size="sm" 
                                onClick={handleGenerate}
                                className="text-xs font-semibold text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 border-gray-200 dark:border-white/10 transition-colors"
                            >
