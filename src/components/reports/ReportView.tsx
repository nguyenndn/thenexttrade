...
function StatCard({ label, value, icon: Icon, color, iconBg, delta, previous, isPercent, borderColor }: {
 ...
 return (
 <div className={`bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard p-5 shadow-sm hover:shadow-md transition-shadow duration-200 border-t-4 ${borderColor || "border-t-transparent"}`}>
...
function BreakdownTable({ data, title, icon: Icon, iconColor }: { data: any[] | null; title: string; icon: any; iconColor: string }) {
 ...
 <div>
 {data.map((item, i) => (
 <div key={i} className="grid grid-cols-12 gap-2 items-center px-5 h-[52px] hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors border-b border-gray-50 last:border-0">
