...
export function DraggablePreviewTable({ initialData, onChange }: DraggablePreviewTableProps) {
  ...
  const restoreColumn = (hidden: {name: string}) => {
    ...
    const originalIndex = initialData.headers.findIndex(h => h === hidden.name);
        ...
        <div className="flex justify-end">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm" className="h-8 text-xs font-bold gap-1.5 shadow-sm bg-white dark:bg-[#1E2028]">
                <Columns size={14} className="text-primary" /> Add Column
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56 bg-white dark:bg-[#1E2028] border-dashboard rounded-xl p-2 shadow-xl z-50">
              ...
              {hiddenHeaders.map((hidden) => (
                ...
                >
                  <PlusCircle size={14} className="text-primary" /> {hidden.name}
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
        ...
        <table className="w-full text-left border-collapse">
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <GripHorizontal size={14} className="text-gray-400 group-hover:text-primary transition-colors" />
                      <span>{header}</span>
                    </div>
                    <Button
                      variant="ghost"
          ...
          <tbody className="divide-y divide-dashboard">
            {rows.slice(0, 50).map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                {row.map((cell, cellIndex) => (
                  <td key={`${rowIndex}-${cellIndex}`} className="p-4 text-sm font-medium text-gray-700 dark:text-gray-300 whitespace-nowrap truncate max-w-[200px]" title={String(cell)}>
                    {cell}
                  </td>
                ))}
              </tr>
            ...
            {rows.length > 50 && (
              <tr>
                <td colSpan={headers.length} className="p-4 text-center text-sm font-medium text-gray-600 dark:text-gray-300 italic bg-gray-50/50 dark:bg-white/5">
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={headers.length} className="p-8 text-center text-sm font-medium text-gray-500">
                  No data available.
                </td>
              </tr>
            )}
