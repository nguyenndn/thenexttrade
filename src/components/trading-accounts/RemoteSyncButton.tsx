    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                ...
                if (data.command) {
                    setStatus(
                        data.command.status.toLowerCase() as CommandStatus
                    );
        ...
        try {
            ...
            if (!isAll) {
                ...
                const brokerNow = new Date(
                    utcMs + BROKER_OFFSET_HOURS * 60 * 60 * 1000
                );

                const toDate = format(brokerNow, "yyyy.MM.dd");
                const fromDate = format(
                    subDays(brokerNow, days - 1),
                    "yyyy.MM.dd"
                );
    ...
    const getStatusText = () => {
        switch (status) {
            ...
            case "completed":
                return (
                    result?.message ||
                    `Synced ${result?.syncedCount || 0} trades`
                );
            case "failed":
