...
export function SyncHealthAccountRow({ account, onActionTrigger }: SyncHealthAccountRowProps) {
    ...
    <div className={`p-4 rounded-xl border transition-all ${getStatusBgColor(account.health.status)}`}>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-[10px] text-gray-400 dark:text-gray-500 font-medium">
              {account.broker && (
                <div>
                  Broker: <span className="font-semibold text-gray-600 dark:text-gray-300">{account.broker}</span>
                </div>
              )}
              {account.source !== "MANUAL" && (
                <>
                  <div>
                    Last Heartbeat: <span className="font-semibold text-gray-600 dark:text-gray-300">{formattedDate(account.health.lastHeartbeatAt)}</span>
                  </div>
                  <div>
                    Last Sync: <span className="font-semibold text-gray-600 dark:text-gray-300">{formattedDate(account.health.lastSyncAt)}</span>
                  </div>
                </>
              )}
            </div>
        ...
        <div className="shrink-0 flex items-center">
          <SyncRecoveryAction action={account.nextAction} onActionTrigger={onActionTrigger} />
        </div>
      </div>
    </div>
  );
