...
const mockAccounts: AccountPerformance[] = [
    {
        ...
        swap: 0.0,
        commission: 0.0,
        maxDrawdown: { percent: 100.0, amount: 1143.01 },
        deposit: 10000.0,
        withdrawal: 0.0,
        equityData: [
    ...
    },
    {
        ...
        swap: -12.5,
        commission: -45.8,
        maxDrawdown: { percent: 8.42, amount: 589.0 },
        deposit: 7000.0,
        withdrawal: 0.0,
        equityData: [
    ...
    return (
        <div className="space-y-4">
            ...
            {hasMultiple && (
                <div ref={dropdownRef} className="relative">
                    <button
                        ...
                        className="w-full sm:w-auto flex items-center justify-between gap-3 px-4 py-2.5 rounded-xl border border-dashboard dark:border-white/[0.06] bg-white dark:bg-[#1A1D27] hover:bg-gray-50 dark:hover:bg-white/[0.04] transition-colors"
                    >
                        <div className="flex items-center gap-3">
                            <div
                                className={`w-2 h-2 rounded-full shrink-0 ${data.status === "ONLINE" ? "bg-emerald-500" : "bg-gray-400"}`}
                            />
                    ...
                    {dropdownOpen && (
                        <div className="absolute top-full left-0 mt-1 w-full sm:w-80 bg-white dark:bg-[#1A1D27] rounded-xl border border-dashboard dark:border-white/[0.06] shadow-xl z-20 overflow-hidden">
                            {mockAccounts.map((acc) => (
                                ...
                                >
                                    <div
                                        className={`w-2 h-2 rounded-full shrink-0 ${acc.status === "ONLINE" ? "bg-emerald-500" : "bg-gray-400"}`}
                                    />
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[10px] text-gray-400">
                                            {acc.broker} · {acc.accountId}
                                        </p>
                                    </div>
                                    <div className="text-right shrink-0">
                                        <p
                                            className={`text-xs font-black ${acc.growth >= 0 ? "text-primary" : "text-red-500"}`}
                                        >
                                            +{acc.growth}%
                                        </p>
                                        <p className="text-[10px] text-gray-400">
            ...
            <div className="grid lg:grid-cols-5 gap-4">
                ...
                <div className="lg:col-span-3 bg-white dark:bg-[#1A1D27] rounded-xl border border-dashboard dark:border-white/[0.06] p-5">
                    {/* Top row */}
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <p className="text-lg font-black text-primary mt-0.5">
                                {formatCurrency(data.highestBalance)}
                            </p>
                        </div>
                        <div>
                            <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                ...
                {/* Statistics — 2 cols */}
                <div className="lg:col-span-2 bg-white dark:bg-[#1A1D27] rounded-xl border border-dashboard dark:border-white/[0.06] p-5">
                    ...
                    <div className="space-y-2 text-xs">
                        {[
                            ...
                            {
                                label: "Swap",
                                value: formatCurrency(data.swap, true),
                                color: "text-primary",
                            },
                            {
                                label: "Commission",
                            ...
                            {
                                label: "Max Drawdown",
                                value: `${data.maxDrawdown.percent.toFixed(2)}% / ${formatCurrency(data.maxDrawdown.amount)}`,
                                color: "text-red-500",
                            },
                        ].map((stat) => (
                            <div
                                key={stat.label}
                                className="flex items-center justify-between py-1 border-b border-dashboard last:border-0"
                            >
                                <span className="text-gray-500 dark:text-gray-400 font-bold uppercase tracking-wider text-[10px]">
                                    {stat.label}
                                </span>
                                <span
                                    className={`font-black text-gray-700 dark:text-white ${stat.color || ""}`}
                                >
                                    {stat.value}
                                </span>
                            </div>
            ...
            <div className="bg-white dark:bg-[#1A1D27] rounded-xl border border-dashboard dark:border-white/[0.06] p-5">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        ...
                        <p className="text-[10px] text-gray-400 mt-0.5">
                            6 Trading Days / 9 Days Lifespan
                        </p>
                    </div>
            ...
            <div className="grid lg:grid-cols-5 gap-4">
                ...
                <div
                    ref={calendarRef}
                    className="lg:col-span-3 bg-white dark:bg-[#1A1D27] rounded-xl border border-dashboard dark:border-white/[0.06] p-5"
                >
                    <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="grid grid-cols-8 gap-0.5 text-center">
                        ...
                        {[
                            ...
                            <div
                                key={d}
                                className="py-1.5 text-[9px] font-black text-gray-400 uppercase tracking-widest"
                            >
                                {d}
                            </div>
                        ))}
                        ...
                        {weeks.map((week, wi) => {
                            ...
                            return paddedWeek.concat([null]).map((day, di) => {
                                ...
                                return (
                                    <div
                                        key={`w${wi}-d${di}`}
                                        className={`py-2 rounded-lg text-[10px] transition-colors ${
                                            pnl !== undefined
                                                ? pnl >= 0
                    ...
                    <div className="mt-3 pt-3 border-t border-dashboard flex items-center justify-between">
                        ...
                        <span
                            className={`text-sm font-black ${monthlyTotal >= 0 ? "text-primary" : "text-red-500"}`}
                        >
                            {formatCurrency(monthlyTotal, true)}
                        </span>
                    </div>
                ...
                {/* Account Info — 2 cols */}
                <div className="lg:col-span-2 bg-white dark:bg-[#1A1D27] rounded-xl border border-dashboard dark:border-white/[0.06] p-5 flex flex-col">
                    <h3 className="text-sm font-black text-gray-700 dark:text-white uppercase tracking-wider mb-4">
                        Account Info
                    </h3>
                    ...
                    <div className="space-y-3 flex-1">
                        {[
                            ...
                            <div
                                key={item.label}
                                className="flex items-center justify-between py-1.5 border-b border-dashboard last:border-0"
                            >
                                ...
                                <span
                                    className={`text-xs text-gray-700 dark:text-white ${item.bold ? "font-black" : "font-bold"}`}
                                >
                                    {item.value}
                                </span>
                            </div>
                        ...
                        <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-primary/5 dark:bg-primary/10 border border-primary/20">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-gray-500 dark:text-gray-400">
                                    Data synced directly from MT5 terminal
                                </p>
                            </div>
                        </div>
                    </div>
