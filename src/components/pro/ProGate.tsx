 ...
 */
export function ProGate({ children, feature, accountId, fallback, previewCount }: ProGateProps) {
...
function ProTeaser({ feature }: { feature?: string }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#1E2028]">
      ...
      <div className="relative flex flex-col items-center px-6 py-10 text-center">
        ...
        <p className="mt-2 max-w-sm text-sm text-gray-500 dark:text-gray-400">
          Unlock all Pro features for free by verifying as a Gold Scalper Ninja VIP trader.
        ...
        <p className="mt-3 text-xs text-gray-400 dark:text-gray-500">
          Open a broker account via our partner link → Verify → Get instant Pro access
        </p>
      </div>
    </div>
  );
