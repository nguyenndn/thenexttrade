...
function InsightCard({
    ...
    return (
        <div
            className={`rounded-xl border p-4 ${severityColors[insight.severity]}`}
        >
            <div className="flex items-start gap-3">
                <div
                    className={`shrink-0 rounded-lg p-2 ${isIssue ? "bg-red-100 dark:bg-red-500/10 text-red-500 dark:text-red-400" : "bg-emerald-100 dark:bg-emerald-500/10 text-emerald-500 dark:text-emerald-400"}`}
                >
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                        ...
                        <span
                            className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${severityBadge[insight.severity]}`}
                        >
                            {insight.severity}
                        </span>
                    </div>
...
function EdgeLeakDetectorContent({
    issues,
    strengths,
    loading,
}: EdgeLeakDetectorContentProps) {
    if (loading) {
        return (
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">
                        Patterns costing you money — detected from your trade
                        data
                    </p>
                </div>
            </div>
            ...
            {issues.length > 0 ? (
                ...
                <div className="rounded-xl border border-emerald-200 dark:border-emerald-500/20 bg-emerald-50 dark:bg-emerald-500/5 p-6 text-center">
                    ...
                    <p className="mt-2 text-sm text-emerald-600 dark:text-emerald-400">
                        No edge leaks detected. Keep it up!
                    </p>
                </div>
            )}
