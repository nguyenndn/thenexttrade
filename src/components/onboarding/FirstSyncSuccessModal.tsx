...
import { Sparkles, ArrowRight, FileText, Sun, X, CheckCircle2 } from "lucide-react";
...
export function FirstSyncSuccessModal({ open, onClose, hasReports, firstInsight }: FirstSyncSuccessModalProps) {
  ...
  const handlePrimary = () => {
    trackEvent("first_insight_cta_clicked", { action: "primary", target: firstInsight?.primaryCta || "/dashboard/analytics" });
    onClose();
    window.location.href = firstInsight?.primaryCta || "/dashboard/analytics";
  };

  const handleSecondary = () => {
    trackEvent("first_insight_cta_clicked", { action: "secondary", target: firstInsight?.secondaryCta || "/dashboard/reports" });
    onClose();
    window.location.href = firstInsight?.secondaryCta || "/dashboard/reports";
  };
        ...
        <div className="relative px-6 pt-8 pb-4 text-center overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 leading-relaxed max-w-sm mx-auto">
              Your first trade data is synced. Here is what we found:
            </p>
          </div>
        </div>
