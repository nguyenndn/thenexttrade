...
import { Bell, Trophy, Zap, PieChart, Check, Inbox, ChevronLeft, ChevronRight } from "lucide-react";
...
const getNotificationIcon = (type: string) => {
  switch (type) {
    ...
    case 'MONTHLY_REPORT':
    case 'REPORT_NUDGE':
      return { Icon: PieChart, bg: 'bg-emerald-100 dark:bg-emerald-500/20', color: 'text-emerald-600 dark:text-emerald-400' };
    case 'FEATURE_UPDATE':
    case 'SYNC_STALE':
    case 'NO_TRADES_NUDGE':
    case 'COACH_NUDGE':
      return { Icon: Zap, bg: 'bg-orange-100 dark:bg-orange-500/20', color: 'text-orange-600 dark:text-orange-400' };
    case 'MILESTONE':
      return { Icon: Trophy, bg: 'bg-yellow-100 dark:bg-yellow-500/20', color: 'text-yellow-600 dark:text-yellow-400' };
    case 'SYSTEM':
    case 'ONBOARDING':
    default:
      return { Icon: Bell, bg: 'bg-blue-100 dark:bg-blue-500/20', color: 'text-blue-600 dark:text-blue-400' };
  }
};
...
export function NotificationBell() {
  ...
  const normalizedNotifications = useMemo(() => {
    return notifications.map(n => {
      ...
      if (n.type === "FEATURE_UPDATE" && milestoneTitles.some(title => n.title.includes(title))) {
  ...
  const unreadCounts = useMemo(() => {
    return {
      ...
      Coach: normalizedNotifications.filter(n => !n.isRead && ["FEATURE_UPDATE", "SYNC_STALE", "NO_TRADES_NUDGE", "COACH_NUDGE"].includes(n.type)).length,
      Reports: normalizedNotifications.filter(n => !n.isRead && ["WEEKLY_REPORT", "MONTHLY_REPORT", "REPORT_NUDGE"].includes(n.type)).length,
      System: normalizedNotifications.filter(n => !n.isRead && ["SYSTEM", "ONBOARDING", "MILESTONE"].includes(n.type)).length,
    };
  ...
  const filteredNotifications = useMemo(() => {
    return normalizedNotifications.filter(n => {
      ...
      if (activeTab === "Coach") return ["FEATURE_UPDATE", "SYNC_STALE", "NO_TRADES_NUDGE", "COACH_NUDGE"].includes(n.type);
      if (activeTab === "Reports") return ["WEEKLY_REPORT", "MONTHLY_REPORT", "REPORT_NUDGE"].includes(n.type);
      if (activeTab === "System") return ["SYSTEM", "ONBOARDING", "MILESTONE"].includes(n.type);
      return true;
    });
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="w-10 h-10 rounded-full border border-dashboard relative text-gray-600 hover:text-gray-700 dark:text-gray-500 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5 transition-all duration-100">
          <Bell size={20} />
          {unreadCount > 0 && (
      ...
      <PopoverContent className="w-[calc(100vw-2rem)] sm:w-[420px] p-0 rounded-2xl bg-white dark:bg-[#1E2028] border-dashboard shadow-2xl" align="end" sideOffset={8}>
                ...
                return (
                  <div
                    ...
                    onClick={() => {
                      ...
                      if (n.onClick) {
                        n.onClick();
                      } else if (n.link && n.link !== "/dashboard") {
                        router.push(n.link);
                      } else {
                  ...
                  >
                    ...
                    {/* Left Icon */}
                    <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm", bg, color)}>
                    ...
                    <div className="flex-1 min-w-0 pr-1">
                      <div className="flex justify-between items-start mb-1 gap-2">
                        <h3 className={cn("text-sm font-semibold truncate", !n.isRead ? "text-gray-900 dark:text-white" : "text-gray-700 dark:text-gray-300")}>
                          {n.title}
                        </h3>
