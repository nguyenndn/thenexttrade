...
export function TradePlanList({ plans, onRefresh, onLogTrade, onViewActual }: TradePlanListProps) {
  const [filter, setFilter] = useState<"ALL" | "PLANNED" | "ACTIVE" | "MATCHED" | "CANCELLED">("ALL");
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-1.5 p-1 bg-gray-50 dark:bg-black/20 border border-dashboard rounded-xl">
          {(["ALL", "PLANNED", "ACTIVE", "MATCHED", "CANCELLED"] as const).map((status) => {
            const isActive = filter === status;
            const count = status === "ALL" ? plans.length : plans.filter((p) => p.status === status).length;
