...
export function CalendarHeatmap({
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard overflow-hidden">
            ...
            <div className="p-4 sm:p-5 border-b border-dashboard">
                    ...
                    <div className="hidden sm:flex items-center gap-4 text-sm">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-gray-600 dark:text-gray-300">
                                {monthSummary.winDays} win
                            </span>
                        </div>
                        <div className="flex items-center gap-1.5">
                            <TrendingDown size={14} className="text-red-500" />
                            <span className="text-gray-600 dark:text-gray-300">
                                {monthSummary.lossDays} loss
                            </span>
                        </div>
                        <div className="flex items-center gap-1.5">
                            <span
                                className={`font-bold ${monthSummary.totalPnl >= 0 ? "text-emerald-500" : "text-red-500"}`}
                            >
                ...
                <div className="flex sm:hidden items-center justify-between text-sm">
                    <div className="flex items-center gap-3">
                        <span className="text-emerald-500 font-medium">
                            {monthSummary.winDays}W
                        </span>
                        <span className="text-red-500 font-medium">
                            {monthSummary.lossDays}L
                        </span>
                    </div>
                    <span
                        className={`font-bold ${monthSummary.totalPnl >= 0 ? "text-emerald-500" : "text-red-500"}`}
            ...
            <div className="p-3 sm:p-5">
                ...
                <div className="grid grid-cols-7 gap-1 sm:gap-2 mb-2">
                    {weekDays.map((day) => (
                        <div
                            key={day}
                            className="text-center text-xs font-medium text-gray-600 dark:text-gray-300 py-1"
                        >
                            {day}
                        </div>
                    ))}
