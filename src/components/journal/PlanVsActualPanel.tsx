...
import { CheckCircle, AlertOctagon, Scale, BookOpen, AlertTriangle } from "lucide-react";
...
export function PlanVsActualPanel({ plan, entry }: PlanVsActualPanelProps) {
  // Calculations
  const entryDiff = plan.plannedEntry && entry.entryPrice ? entry.entryPrice - plan.plannedEntry : 0;
  const entrySlippagePct = plan.plannedEntry ? (entryDiff / plan.plannedEntry) * 100 : 0;
  ...
  const slDeviation = plan.plannedStopLoss && entry.stopLoss ? entry.stopLoss - plan.plannedStopLoss : 0;
  const tpDeviation = plan.plannedTakeProfit && entry.takeProfit ? entry.takeProfit - plan.plannedTakeProfit : 0;

  const lotDiff = plan.plannedLotSize && entry.lotSize ? entry.lotSize - plan.plannedLotSize : 0;
      ...
      <div className="flex items-center gap-2 p-3.5 rounded-xl border border-dashboard bg-purple-500/5">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-0.5">
            This trade was matched to the planned setup: <strong className="text-gray-700 dark:text-white">{plan.setupName || "Unnamed Plan"}</strong>.
          </p>
        </div>
      </div>
          ...
          <div className="space-y-2.5 text-xs">
            <div className="flex justify-between">
              ...
              <span className={cn(
                ...
                plan.type === "BUY" ? "bg-blue-500/10 text-blue-500" : "bg-red-500/10 text-red-500"
              )}>
                {plan.type}
              </span>
            </div>
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono font-bold text-gray-700 dark:text-white">
                {plan.plannedEntry ? plan.plannedEntry.toFixed(5) : "Market"}
              </span>
            </div>

            <div className="flex justify-between">
              <span className="text-red-500/80 font-medium">Stop Loss</span>
            ...
            <div className="flex justify-between">
              <span className="text-emerald-500/80 font-medium">Take Profit</span>
            ...
            <div className="flex justify-between">
              <span className="text-gray-400 font-medium">Risk Amount</span>
              <span className="font-bold text-gray-700 dark:text-white">
        ...
        <div className="bg-white dark:bg-black/10 p-4 rounded-xl border border-dashboard space-y-3">
          ...
          <div className="space-y-2.5 text-xs">
            <div className="flex justify-between">
              ...
              <span className={cn(
                "font-black uppercase px-2 py-0.5 rounded text-[10px]",
                entry.type.toUpperCase() === "BUY" ? "bg-blue-500/10 text-blue-500" : "bg-red-500/10 text-red-500"
              )}>
                {entry.type}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center">
              <span className="text-gray-400 font-medium">Execution Entry</span>
              <div className="text-right">
                ...
                {plan.plannedEntry && entryDiffAbs > 0 && (
                  <span className={cn(
                    ...
                    isBetterEntry ? "text-emerald-500" : "text-red-500"
                  )}>
                    {isBetterEntry ? "Better Entry" : "Slippage"}: {isBetterEntry ? "-" : "+"}{entryDiffAbs.toFixed(5)} ({entrySlippagePct.toFixed(2)}%)
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                <span className="font-mono font-bold text-red-500 block">
                  {entry.stopLoss ? entry.stopLoss.toFixed(5) : "-"}
                </span>
                {plan.plannedStopLoss && entry.stopLoss && slDeviation !== 0 && (
                  <span className={cn(
                    "text-[10px] font-bold block mt-0.5",
                    slDeviation > 0 ? "text-emerald-500" : "text-red-500"
                  )}>
                    Deviaton: {slDeviation > 0 ? "+" : ""}{slDeviation.toFixed(5)}
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                <span className="font-mono font-bold text-emerald-500 block">
                  {entry.takeProfit ? entry.takeProfit.toFixed(5) : "-"}
                </span>
                {plan.plannedTakeProfit && entry.takeProfit && tpDeviation !== 0 && (
                  <span className={cn(
                    "text-[10px] font-bold block mt-0.5",
                    tpDeviation > 0 ? "text-emerald-500" : "text-red-500"
                  )}>
                    Deviation: {tpDeviation > 0 ? "+" : ""}{tpDeviation.toFixed(5)}
                  </span>
                )}
              </div>
            </div>
              ...
              <div className="text-right">
                ...
                {lotDiff !== 0 && (
                  <span className="text-[10px] font-bold text-amber-500 block mt-0.5">
                    Size change: {lotDiff > 0 ? "+" : ""}{lotDiff.toFixed(2)} lots
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-between">
              <span className="text-gray-400 font-medium">Actual Result</span>
              <span className={cn(
                "font-bold",
                entry.pnl && entry.pnl >= 0 ? "text-emerald-500" : "text-red-500"
              )}>
        ...
        <div className="p-4 rounded-xl border border-dashboard bg-white dark:bg-black/10 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            {entry.ruleChecks.map((check: any) => {
              const statusColors = {
                FOLLOWED: "bg-emerald-50 text-emerald-600 border border-emerald-100 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20",
                BROKEN: "bg-red-50 text-red-600 border border-red-100 dark:bg-red-500/10 dark:text-red-400 dark:border-red-500/20",
                SKIPPED: "bg-gray-50 text-gray-600 border border-gray-100 dark:bg-white/5 dark:text-gray-400 dark:border-white/10"
              };

              return (
                <div key={check.id} className="p-3 rounded-lg border border-dashboard bg-gray-50/50 dark:bg-black/20 flex items-start gap-2 justify-between">
                  <div className="min-w-0">
