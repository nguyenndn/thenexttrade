...
export function MistakesCell({ entry, onUpdate }: MistakesCellProps) {
    ...
    const triggerContent = (
        <div className="flex gap-1 flex-wrap justify-center cursor-pointer min-h-[24px] min-w-[24px] items-center">
            {mistakes.length > 0 ? (
                mistakes.map((mCode: string, idx: number) => {
                    ...
                    if (mistake) {
                        return (
                            <span
                                key={idx}
                                className={`px-2 py-1 rounded-full text-[10px] font-bold border flex items-center gap-1 whitespace-nowrap ${getMistakeSeverityColor(mistake.severity)}`}
                            >
