...
export function TradePlanModal({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const sl = formData.plannedStopLoss
            ? parseFloat(formData.plannedStopLoss as string)
            : null;
        const tp = formData.plannedTakeProfit
            ? parseFloat(formData.plannedTakeProfit as string)
            : null;
        ...
        if (!isNaN(entry)) {
            if (formData.type === "BUY") {
                if (sl !== null && sl >= entry) {
                    toast.error(
                        "Stop Loss must be below Entry Price for BUY setup."
                    );
                    return;
                }
                if (tp !== null && tp <= entry) {
                    toast.error(
                        "Take Profit must be above Entry Price for BUY setup."
                    );
                    return;
                }
            } else {
                if (sl !== null && sl <= entry) {
                    toast.error(
                        "Stop Loss must be above Entry Price for SELL setup."
                    );
                    return;
                }
                if (tp !== null && tp >= entry) {
                    toast.error(
                        "Take Profit must be below Entry Price for SELL setup."
                    );
                    return;
                }
            }
        ...
        try {
            const payload: any = {
                ...formData,
                plannedEntry: formData.plannedEntry
                    ? parseFloat(formData.plannedEntry as string)
                    : null,
                plannedStopLoss: formData.plannedStopLoss
                    ? parseFloat(formData.plannedStopLoss as string)
                    : null,
                plannedTakeProfit: formData.plannedTakeProfit
                    ? parseFloat(formData.plannedTakeProfit as string)
                    : null,
                plannedLotSize: formData.plannedLotSize
                    ? parseFloat(formData.plannedLotSize as string)
                    : null,
                riskAmount: formData.riskAmount
                    ? parseFloat(formData.riskAmount as string)
                    : null,
    ...
    return (
        <form
            ...
            className="space-y-5 max-h-[80vh] overflow-y-auto custom-scrollbar p-1"
        >
            ...
            <div className="space-y-2 pt-2 border-t border-dashboard">
                <EmotionSelector
                    ...
                    onChange={(val) =>
                        setFormData((prev) => ({ ...prev, emotionBefore: val }))
                    }
                ...
                {!skipChecklist && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {[
                            {
                                id: "setupSelected",
                                label: "Setup selected & clear",
                            },
                            {
                                id: "riskDefined",
                                label: "Risk and stop-loss defined",
                            },
                            {
                                id: "sizeReviewed",
                            ...
                            },
                            {
                                ...
                                label: "Session/time window acceptable",
                            },
                            {
                                id: "emotionAcknowledged",
                                label: "Emotional state acknowledged",
                            },
                            {
                                id: "rulebookAcknowledged",
                                label: "Active rulebook acknowledged",
                            },
                        ].map((check) => (
                            <button
                                key={check.id}
