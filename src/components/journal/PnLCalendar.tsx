...
import { ChevronLeft, ChevronRight, FileText, Calendar as CalendarIcon, Edit2, Check, Loader2 } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { cn } from "@/lib/utils";
import { getCalendarPerformance, DailyPerformance, saveDailyNote } from "@/actions/calendar";
...
export function PnLCalendar({ 
  ...
  const fetchMonthData = async (month: Date) => {
    setIsLoading(true);
    const result = await getCalendarPerformance(month.toISOString(), accountId);
    ...
    <div className="bg-white dark:bg-[#1E2028] p-6 rounded-xl border border-dashboard shadow-sm hover:shadow-md transition-shadow duration-200 group mt-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="font-bold text-gray-700 dark:text-white text-sm">P&L Calendar</h3>
            <p className="text-xs text-gray-500">Daily session notes & overview</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <Button variant="outline" size="icon" onClick={handlePrevMonth} className="rounded-lg border-0 bg-gray-50 dark:bg-white/5" aria-label="Previous month">
            <ChevronLeft size={18} />
          </Button>
          <span className="text-[15px] font-medium text-gray-700 dark:text-gray-300 min-w-[120px] text-center">
            {format(currentMonth, "MMMM yyyy")}
          </span>
          <Button variant="outline" size="icon" onClick={handleNextMonth} className="rounded-lg border-0 bg-gray-50 dark:bg-white/5" aria-label="Next month">
        ...
        <div className="min-w-[700px] relative">
          {isLoading && (
            <div className="absolute inset-0 bg-white/50 dark:bg-[#1E2028]/50 flex items-center justify-center z-20 backdrop-blur-sm rounded-xl">
                <span className="font-bold text-gray-500 flex items-center gap-2"><Loader2 size={16} className="animate-spin" /> Loading...</span>
            </div>
          )}
          ...
          <div className="grid grid-cols-7 gap-2">
            {daysInMonth.map((date, i) => {
              ...
              let bgClass = "bg-gray-50/50 dark:bg-[#1A1C23] border-transparent text-gray-500 dark:text-gray-400";
              if (dayData && dayData.pnl > 0) {
                bgClass = "bg-emerald-50/80 dark:bg-primary/10 text-emerald-600 dark:text-primary border border-emerald-400 dark:border-primary/60";
              } else if (dayData && dayData.pnl < 0) {
                bgClass = "bg-red-50/80 dark:bg-red-500/10 text-red-600 dark:text-red-400 border border-red-400 dark:border-red-500/60";
              } else if (isCurrentMonth) {
                bgClass = "bg-white dark:bg-[#23252E] border border-dashboard text-gray-700 dark:text-gray-300";
              }
              
              if (!isCurrentMonth) bgClass = "bg-transparent border-dashed border-gray-200 dark:border-gray-800 text-gray-300 dark:text-gray-600 opacity-40";

              return (
                <div 
                  key={date.toISOString()} 
                  className={cn("rounded-xl p-3 min-h-[90px] relative group transition-all duration-200 flex flex-col", bgClass)}
                >
                ...
                {dayData?.note && (
                  <div className="text-blue-500" title="Has Note">
              ...
              {dayData && dayData.tradesCount > 0 ? (
                <div className="flex flex-col gap-1 mt-auto">
                  <div className="font-semibold text-sm">
                    <PnLDisplay value={dayData.pnl} />
                  </div>
                  <div className="text-[10px] text-gray-500 dark:text-gray-400 flex items-center gap-1.5 font-medium">
                    <span className="text-green-600 dark:text-green-400">{dayData.wins}W</span>
                    <span className="text-red-600 dark:text-red-400">{dayData.losses}L</span>
                    {dayData.breakEvens > 0 && <span>{dayData.breakEvens}BE</span>}
                  </div>
              ...
              {isCurrentMonth && (
                <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                  ...
                  {onSelectDateRange && (
                    <button
                      onClick={() => onSelectDateRange(format(startOfWeek(date), "yyyy-MM-dd"), format(endOfWeek(date), "yyyy-MM-dd"))}
                      className="p-1 rounded bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm text-gray-500 hover:text-primary"
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            Add context or reflections for this trading day. What worked well? What mistakes were made?
          </p>
          <textarea
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="outline" onClick={() => setIsNoteModalOpen(false)} disabled={isSavingNote}>
              Cancel
            </Button>
