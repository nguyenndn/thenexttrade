        ...
        <div className="relative min-h-[600px] w-full py-20 px-4 overflow-hidden bg-slate-50 dark:bg-[#0B0E14] transition-colors duration-300">
            ...
            <div className="max-w-4xl mx-auto relative z-10 space-y-24">
                {PHASES.map((phase, index) => {
                    ...
                    return (
                        <div key={index} className={cn("relative flex items-center gap-8",
                            index % 2 === 0 ? "flex-row text-right" : "flex-row-reverse text-left"
                        )}>
                            {/* Connector Line */}
                            {index < PHASES.length - 1 && (
                                <div className="absolute top-24 left-1/2 -translate-x-1/2 w-0.5 h-24 bg-gradient-to-b from-gray-200 to-transparent dark:from-gray-800 dark:to-white/5" />
                            )}
                            ...
                            <motion.div
                                ...
                                className={cn("w-1/2 flex flex-col gap-4", index % 2 === 0 ? "items-end" : "items-start")}
                            >
                                <div>
                            ...
                            {/* Modules */}
                            <div className={cn("w-1/2", index % 2 === 0 ? "pl-8 text-left" : "pr-8 text-right", isLocked && "opacity-30 blur-[1px]")}>
                                {levelData && levelData.modules.length > 0 ? (
                                    <div className={cn("space-y-3 flex flex-col", index % 2 === 0 ? "items-start" : "items-end")}>
                                        {levelData.modules.map((module) => (
                                            <Link
                                                key={module.id}
                                                href={module.lessons[0] ? `${basePath}/lessons/${module.lessons[0].slug}` : '#'}
                                                className={cn("block group w-full", !module.lessons[0] && "pointer-events-none opacity-50")}
                                            >
                                                <div className={cn(
                                                    "inline-flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/5 hover:border-[#00C888] dark:hover:border-white/20 transition-all cursor-pointer shadow-sm hover:shadow-md max-w-sm",
