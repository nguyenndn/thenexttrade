...
const ICON_ANIMATIONS: Record<
    number,
    { animate: Record<string, number[]>; transition: Record<string, any> }
> = {
    1: {
    ...
    7: {
        animate: { x: [0, 4, 0, -4, 0], rotate: [0, 5, 0, -5, 0] },
        transition: { duration: 3, repeat: Infinity, ease: "easeInOut" },
    },
    8: {
    ...
    10: {
        animate: { scale: [1, 1.05, 1.1, 1.05, 1], rotate: [0, 5, 0, -5, 0] },
        transition: { duration: 3, repeat: Infinity, ease: "easeInOut" },
    },
    11: {
...
export function AcademyTree({
    ...
    const [justUnlockedIds, setJustUnlockedIds] = useState<Set<string>>(
        new Set()
    );
    ...
    const { lessonStates, levelStates } = useMemo(() => {
        if (!hasProgress) {
            ...
            return {
                lessonStates: emptyLessonStates,
                levelStates: emptyLevelStates,
            };
        }
    ...
    return (
        <>
            <div className="relative min-h-[600px] w-full py-16 px-4 overflow-hidden bg-[#fbfaf6] dark:bg-[#0B0E14] transition-colors duration-300">
                ...
                <svg
                    className="absolute left-1/2 top-[10%] -translate-x-1/2 w-[800px] h-[80%] max-w-none opacity-[0.07] dark:opacity-[0.03] pointer-events-none"
                    viewBox="0 0 800 600"
                    fill="none"
                    aria-hidden="true"
                >
                    <path
                        d="M400 0 C380 80 420 160 400 240 C380 320 420 400 400 480 C380 560 400 600 400 600"
                        stroke="#F59E0B"
                        strokeWidth="1.5"
                        strokeDasharray="6 4"
                    />
                    <path
                        d="M380 0 C340 100 460 200 380 300 C300 400 460 500 380 600"
                        stroke="#10b981"
                        strokeWidth="1"
                        strokeDasharray="4 6"
                    />
                    <path
                        d="M420 0 C460 100 340 200 420 300 C500 400 340 500 420 600"
                        stroke="#10b981"
                        strokeWidth="1"
                        strokeDasharray="4 6"
                    />
                ...
                <div className="max-w-4xl mx-auto relative z-10">
                    {levels.map((level, index) => {
                        ...
                        const colors =
                            LEVEL_COLORS[level.order] || LEVEL_COLORS[1];
                        ...
                        const lvlState = hasProgress
                            ? levelStates[level.id] || "locked"
                            : null;
                        const isLevelLocked =
                            hasProgress && lvlState === "locked";
                        const isLevelCompleted =
                            hasProgress && lvlState === "completed";
                        ...
                        const nextColors = nextLevel
                            ? LEVEL_COLORS[nextLevel.order] || LEVEL_COLORS[1]
                            : null;
                        const nextLvlState =
                            nextLevel && hasProgress
                                ? levelStates[nextLevel.id] || "locked"
                                : null;
                            ...
                            <div key={level.id}>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <p
                                            className={cn(
                                                "text-sm text-gray-600 dark:text-gray-300 max-w-xs leading-relaxed",
                                                (!hasContent ||
                                                    isLevelLocked) &&
                                                    "opacity-50"
                                            )}
                                    ...
                                    >
                                        {isLevelCompleted && hasContent ? (
                                            ...
                                            <motion.div
                                                ...
                                                className="w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center border-[3px] border-dashed border-dashboard bg-gray-50 dark:bg-gray-900/50 opacity-50"
                                            >
                                                <Lock
                                                    size={24}
                                            ...
                                            <Link
                                                href={`${isGuest ? "/academy/lesson" : "/dashboard/academy/lessons"}/${currentSlug}`}
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    />
                                                    <div
                                                        className="absolute inset-0 rounded-full border border-current opacity-10 animate-spin"
                                                        style={{
                                                            animationDuration:
                                                                "12s",
                                                        }}
                ...
                <div className="fixed bottom-4 right-4 z-50">
                    ...
                    {devOpen ? (
                        <motion.div
                            ...
                            className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-dashboard p-3 space-y-2 min-w-[200px]"
                        >
                            <div className="flex items-center justify-between mb-1">
                                <span className="text-xs font-bold text-gray-600 flex items-center gap-1">
                                    <FlaskConical size={12} /> DEV TEST
                                </span>
                                <button
                            ...
                            <div className="border-t border-dashboard pt-2 mt-1">
                                <button
                                    onClick={() => {
                                        const titles = levels.reduce(
                                            (acc, l) => {
                                                acc[l.order] = l.title;
                                                return acc;
                                            },
