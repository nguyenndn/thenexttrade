...
export default function LessonClientView({
    ...
    const [lockedDialogLesson, setLockedDialogLesson] = useState<string | null>(
        null
    );

    const lessonPath = (slug: string) =>
        isDashboard
            ? `/dashboard/academy/lessons/${slug}`
            : `/academy/lesson/${slug}`;
    ...
    const completedInModule = courseLessons.filter((l: any) =>
    ...
    const unlockedIndexes = useMemo(() => {
        ...
        for (let i = 1; i < courseLessons.length; i++) {
            const allPrevDone = courseLessons
                .slice(0, i)
            ...
            if (res.ok) {
                ...
                setTimeout(() => {
                    if (nextLesson) router.push(lessonPath(nextLesson.slug));
                        ...
                        router.push(
                            isDashboard
                                ? `/dashboard/academy/quiz/${quiz.id}`
                                : `/academy/quiz/${quiz.id}`
                        );
                }, 1500);
            }
        } catch (error: any) {
        ...
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-2 text-base font-semibold">
                    ...
                    <span className="text-gray-600 dark:text-gray-300">
                        {lesson.module.level?.title || "Level"}
                    </span>
                    <ChevronRight
                        size={14}
                        className="text-gray-300 dark:text-gray-600"
            ...
            <div className="grid lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2 space-y-4">
                    {/* Video Player */}
                    {videoId && (
                        <div className="relative aspect-video rounded-xl overflow-hidden bg-gray-900 border border-dashboard shadow-lg">
                            <iframe
                    ...
                    <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard shadow-sm p-6 flex items-center justify-between gap-4">
                        {prevLesson ? (
                            <Link
                                ...
                                className="hover:text-primary flex items-center gap-2 transition-colors text-xs font-bold text-gray-500 uppercase tracking-wider"
                            >
                        ...
                        {nextLesson ? (
                            <Link
                                href={lessonPath(nextLesson.slug)}
                                className="hover:text-primary flex items-center gap-2 transition-colors text-xs font-bold text-gray-500 uppercase tracking-wider"
                            >
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard shadow-sm p-5 space-y-3">
                        ...
                        <h2 className="text-lg font-bold text-gray-700 dark:text-white leading-tight">
                            {lesson.module.title}
                        </h2>
                        <p className="text-xs text-gray-600 leading-relaxed line-clamp-3">
                            {lesson.module.description ||
                                "Master this module to advance your trading skills."}
                        </p>
                        {/* Module Progress */}
                        <div className="pt-2 border-t border-dashboard">
                            <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
                                <span>
                                    {completedInModule}/{courseLessons.length}{" "}
                                    completed
                                </span>
                                <span>
                            ...
                            <div className="w-full bg-gray-100 dark:bg-white/5 rounded-full h-1.5 overflow-hidden">
                                <div
                                    ...
                                    style={{
                                        width: `${courseLessons.length > 0 ? (completedInModule / courseLessons.length) * 100 : 0}%`,
                                    }}
                                />
                            </div>
                        </div>
                    ...
                    <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-dashboard shadow-sm overflow-hidden">
                        ...
                        {quiz &&
                            (() => {
                                const allLessonsCompleted = courseLessons.every(
                                    (l: any) =>
                                        completedLessonIds.includes(l.id)
                                );
                                return (
                                    <div className="p-4 border-t border-dashboard">
                                        {allLessonsCompleted ? (
                                            ...
                                            <div className="flex items-center gap-2 text-sm font-bold text-gray-400 dark:text-gray-600 cursor-not-allowed">
                                                ...
                                                <span className="text-[10px] font-normal text-gray-400 dark:text-gray-600 ml-auto">
                                ...
                                if (mobileLocked) {
                                    return (
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 py-2.5 px-3 text-sm rounded-lg text-gray-500 opacity-50"
                                        >
                                            <Lock
                                                size={10}
                                    ...
                                    <Link
                                        ...
                                        className={cn(
                                            ...
                                            l.id === lesson.id
                                                ...
                                                : "text-gray-600 dark:text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5"
                                        )}
                                    >
                    ...
                    >
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-1">
                            <strong className="text-gray-700 dark:text-gray-300">
                                &ldquo;{lockedDialogLesson}&rdquo;
                            </strong>
                        </p>
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-5">
                            You need to complete the previous lessons first
                            before accessing this one. Keep up the great work!
                        </p>
