...
export function HourlyHeatmap({ data }: HourlyHeatmapProps) {
    ...
    const getColor = (hour: (typeof fullHours)[0]) => {
        ...
        if (pnl > 0) {
            if (intensity > 0.8)
                return "bg-primary dark:bg-primary/90 text-white";
            if (intensity > 0.6) return "bg-primary/70 text-white";
            if (intensity > 0.3)
                return "bg-primary/40 text-gray-700 dark:text-white";
            return "bg-primary/20 text-gray-700 dark:text-gray-300";
        } else if (pnl < 0) {
            if (intensity > 0.8)
                return "bg-red-500 dark:bg-red-500/90 text-white";
            if (intensity > 0.6)
                return "bg-red-400 dark:bg-red-500/70 text-white";
            if (intensity > 0.3)
                return "bg-red-300 dark:bg-red-500/40 text-gray-700 dark:text-white";
 ...
 `}
                            >
                                <span className="font-bold text-[13px] opacity-100">
                                    {hour.totalTrades > 0
                                        ? hour.totalTrades
                                        : ""}
                                </span>
                        ...
                        <TooltipContent
                            ...
                            sideOffset={8}
                            className="w-36 bg-gray-900 dark:bg-gray-900 border-gray-800 text-white p-3 shadow-xl rounded-lg z-[100]"
                        >
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex justify-between items-center gap-2">
                                    ...
                                    <span
                                        className={`font-medium ${hour.totalPnL > 0 ? "text-primary dark:text-[#00C888]" : hour.totalPnL < 0 ? "text-red-400" : "text-gray-300"}`}
                                    >
                                        {hour.totalPnL > 0 ? "+" : ""}
                                        {hour.totalPnL.toFixed(2)}
                                    </span>
                                </div>
                            </div>
                        </TooltipContent>
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] p-6 rounded-xl border border-dashboard shadow-sm transition-shadow hover:shadow-md">
