...
export function SessionClock({ data }: SessionClockProps) {
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] p-6 rounded-xl border border-dashboard shadow-sm h-full transition-shadow hover:shadow-md">
            ...
            <div className="space-y-4">
                {sessions.map((session) => {
                    ...
                    const stats = data.find((d) =>
                        d.session.includes(session.id)
                    ); // Loose match for overlaps
                    ...
                    return (
                        <div
                            ...
                            className={`p-4 rounded-xl border transition-all ${
                                isActive
                                    ? "bg-gray-50 dark:bg-white/5 border-dashboard ring-1 ring-primary/20"
                        ...
                        >
                            <div className="flex justify-between items-center mb-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    ></div>
                                    <span
                                        className={`font-bold text-sm ${isActive ? "text-gray-700 dark:text-white" : "text-gray-600"}`}
                                    >
                                        {session.label}
                                    </span>
                                    {isActive && (
                            ...
                            {stats && (
                                <div className="flex items-center justify-between text-xs pl-4 border-l-2 border-dashboard ml-1">
                                    <span className="text-gray-600">
                                        Your PnL:
                                    </span>
                                    <span
                                        className={`font-bold ${stats.totalPnL > 0 ? "text-green-500" : stats.totalPnL < 0 ? "text-red-500" : "text-gray-500"}`}
                                    >
                                        {stats.totalPnL > 0 ? "+" : ""}
