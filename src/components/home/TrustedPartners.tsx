...
export function TrustedPartners() {
  return (
    <section className="pt-10 pb-12 relative overflow-hidden bg-gradient-to-br from-slate-50/50 via-white to-slate-50/50 dark:from-[#0B0E14] dark:via-[#0F1117] dark:to-[#0B0E14] border-t border-gray-200/60 dark:border-white/10">
      {/* Background pattern */}
          ...
          {SECTIONS.map((section, sectionIdx) => (
            <FadeIn key={sectionIdx} delay={sectionIdx * 0.1} direction="up">
              ...
              <div className="flex flex-col h-full bg-white/80 dark:bg-white/[0.02] border border-amber-200/70 dark:border-amber-500/15 rounded-2xl p-5 sm:p-6 backdrop-blur-md shadow-sm hover:border-amber-400 dark:hover:border-amber-500/30 hover:shadow-md transition-all duration-300">
                {/* Section Header */}
                <div className="flex items-center justify-between gap-3 mb-5 pb-4 border-b border-gray-200/60 dark:border-white/5">
                  ...
                  {section.viewAllHref && (
                    <Link
                      ...
                      className={`text-xs font-bold ${section.accentColor} hover:opacity-80 transition-colors flex items-center gap-1 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 px-2.5 py-1 rounded-lg flex-shrink-0`}
                    >
                      All →
                    </Link>
                  )}
                ...
                <div className="flex flex-col gap-3 flex-1">
                  {section.items
                    ...
                    .map((item, idx) => (
                      ...
                      >
                        {/* Logo */}
                        <div className="w-10 h-10 rounded-xl bg-white border border-gray-100 dark:border-white/15 flex items-center justify-center flex-shrink-0 overflow-hidden shadow-sm">
                          {item.logo ? (
                            <Image
                              src={item.logo}
                              alt={item.name}
