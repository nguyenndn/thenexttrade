...
export function MarketTickerSection({
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (result.success && result.data.length > 0) {
                ...
                console.warn(
                    "Using Mock Data for Market Ticker (Fetch Failed)"
                );
    ...
    const getDecimals = (symbol: string) => {
        ...
        if (["USOIL", "XAU/USD", "Gold", "Oil"].some((s) => symbol.includes(s)))
            return 2;
    ...
    const getIconColor = (symbol: string) => {
        ...
        if (symbol.includes("Oil") || symbol.includes("CL=F"))
                    ...
                    {displayData.map((item, idx) => {
                        ...
                        return (
                            ...
                            >
                                {/* Symbol Badge */}
                                <div
                                    className={`flex items-center gap-2 px-2.5 py-1 rounded-full ${iconClass} border border-transparent group-hover:border-current transition-colors`}
                                >
                                    {/* Simple Dot or Icon */}
                                    <Activity
                                        size={12}
                                ...
                                <div className="flex flex-col">
                                    <div className="flex items-baseline gap-2">
                                        ...
                                        <span
                                            className={`flex items-center text-[10px] font-bold ${isPositive ? "text-green-500" : "text-red-500"}`}
                                        >
                                            {isPositive ? (
                                                <TrendingUp
                                                    size={10}
