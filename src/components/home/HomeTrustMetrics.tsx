...
export function HomeTrustMetrics({ metrics }: TrustMetricsProps) {
    const items = [
        ...
        },
        {
            label: "Synced Trades",
            value:
                metrics.syncedTrades > 10000
                    ? `${(metrics.syncedTrades / 1000).toFixed(0)}k+`
        ...
        <div className="relative w-full overflow-hidden bg-white dark:bg-transparent">
            <section className="mx-auto max-w-6xl px-4 pt-2 pb-10 sm:px-6 sm:pb-12 lg:px-8">
                <div className="relative overflow-hidden rounded-3xl border border-gray-200/80 bg-white/80 p-2 shadow-[0_18px_60px_-40px_rgba(15,23,42,0.45)] dark:border-white/[0.07] dark:bg-white/[0.018] sm:p-0">
                    ...
                    <div
                        className={`relative z-10 grid grid-cols-2 gap-2 sm:grid-cols-2 sm:gap-0 sm:divide-x sm:divide-y-0 sm:divide-gray-200/70 sm:[&>*:nth-child(2n+1)]:border-l-0 ${gridColsClass} md:divide-x dark:sm:divide-white/[0.07]`}
                    >
                        {activeItems.map((item, idx) => {
                            const isLastOddItem =
                                activeItems.length % 2 === 1 &&
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div
                                        className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border ${item.border} ${item.bg} transition-transform duration-300 sm:h-11 sm:w-11 sm:rounded-2xl`}
                                    >
                                        <span
                                            className={`${item.color} block transition-transform duration-300 group-hover:scale-110`}
                                        >
                                            {item.icon}
                                        </span>
                                    </div>
