...
export function LiveMarketRatesMock() {
  ...
  const [eurusdDirection, setEurusdDirection] = useState<"up" | "down" | null>(null);
  const [xauusdDirection, setXauusdDirection] = useState<"up" | "down" | null>(null);
  ...
  useEffect(() => {
    ...
    if (hovered) {
      interval = setInterval(() => {
        ...
        setEurusd((prev) => parseFloat((prev + (eurUp ? 0.0003 : -0.0003)).toFixed(4)));

        // XAUUSD fluctuation
        const xauUp = Math.random() > 0.5;
        setXauusdDirection(xauUp ? "up" : "down");
        setXauusd((prev) => parseFloat((prev + (xauUp ? 0.8 : -0.8)).toFixed(2)));
    ...
    >
      ...
      <div className="flex items-center justify-between z-10 font-mono text-[9px] font-bold text-slate-400 uppercase tracking-widest">
        ...
        <div className="flex items-center gap-1 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
          ...
          <span className="text-[8px] text-emerald-600 font-extrabold uppercase tracking-wider">TICK FEED</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center gap-1.5 z-10 my-1 font-mono text-[9.5px] font-bold">
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-1">
          ...
          <span className={`px-1.5 py-0.5 rounded transition-colors duration-200 ${eurusdDirection === "up" ? "bg-emerald-100 text-emerald-700" : eurusdDirection === "down" ? "bg-rose-100 text-rose-700" : "text-slate-800"}`}>
            {eurusd.toFixed(4)}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-100 pb-1">
          ...
          <span className={`px-1.5 py-0.5 rounded transition-colors duration-200 ${xauusdDirection === "up" ? "bg-emerald-100 text-emerald-700" : xauusdDirection === "down" ? "bg-rose-100 text-rose-700" : "text-slate-800"}`}>
            ${xauusd.toFixed(2)}
          </span>
        </div>
