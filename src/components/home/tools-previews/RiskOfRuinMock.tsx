  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between z-10 font-mono text-[9px] font-bold text-slate-400 uppercase tracking-widest">
        ...
        <span className="text-red-600/80 font-black">PROBABILITY ENGINE</span>
      </div>

      {/* Middle Gauge Visual */}
      <div className="flex-1 flex items-center justify-between z-10 py-1 gap-2">
        <div className="flex flex-col justify-center">
          <span className="text-[8px] font-black text-slate-400 uppercase tracking-wider">Win Rate: 45%</span>
          <span className="text-[10px] font-bold text-slate-700 mt-1 font-mono">
            Risk: <span className="text-red-500 font-extrabold">{riskPerTrade}% / trade</span>
          </span>
        </div>

        <div className="flex flex-col items-center justify-center pr-2">
          <span className={`text-3xl font-black font-mono transition-colors duration-300 ${ruinProb > 50 ? 'text-red-600 animate-pulse' : 'text-emerald-600'}`}>
            {ruinProb.toFixed(1)}%
          </span>
      ...
      <div className="z-10 border-t border-slate-100 pt-3 flex justify-between items-center text-[8px] font-bold text-slate-400 font-mono">
        ...
        <span className={`px-2 py-0.5 rounded-full text-[7px] uppercase font-black tracking-widest ${ruinProb > 50 ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-200'}`}>
