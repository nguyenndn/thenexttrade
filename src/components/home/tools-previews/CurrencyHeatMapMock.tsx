...
export function CurrencyHeatMapMock() {
  ...
  const currencies = [
    { name: "USD", strength: 3.2, color: "bg-emerald-500", barWidth: "85%" },
    { name: "EUR", strength: 1.8, color: "bg-emerald-400/70", barWidth: "60%" },
    { name: "GBP", strength: -0.6, color: "bg-rose-400/70", barWidth: "40%" },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between z-10 font-mono text-[9px] font-bold text-slate-400 uppercase tracking-widest">
        ...
        <span className="text-emerald-600/80 font-black">24H TIMEFRAME</span>
      </div>
          ...
          <div key={curr.name} className="flex items-center gap-2">
            ...
            <div className="flex-1 h-2 bg-slate-200/70 rounded-full overflow-hidden">
              <div 
                ...
                style={{ width: hovered ? hoveredWidths[idx] : curr.barWidth }}
              />
            </div>
            <span className={`w-8 text-right font-black ${curr.strength >= 0 ? "text-emerald-600" : "text-rose-600"}`}>
              {curr.strength >= 0 ? "+" : ""}{hovered ? (curr.strength * 1.25).toFixed(1) : curr.strength.toFixed(1)}%
            </span>
          </div>
        ))}
      </div>
