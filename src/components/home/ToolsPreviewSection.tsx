...
export function ToolsPreviewSection({ nextEvent }: ToolsPreviewProps) {
    ...
    const getSessions = () => {
        ...
        return [
            {
                name: "Sydney",
                status: isSydney ? "Open" : "Closed",
                color: isSydney ? "text-green-500" : "text-gray-500",
            },
            {
                name: "Tokyo",
                status: isTokyo ? "Open" : "Closed",
                color: isTokyo ? "text-cyan-500" : "text-gray-500",
            },
            {
                name: "London",
                status: isLondon ? "Open" : "Closed",
                color: isLondon ? "text-orange-500" : "text-gray-500",
            },
            {
                name: "New York",
                status: isNY ? "Open" : "Closed",
                color: isNY ? "text-blue-500" : "text-gray-500",
            },
        ];
    };
    ...
    return (
        <div className="bg-white dark:bg-transparent border-t border-dashboard">
                            ...
                            <div className="flex flex-wrap gap-2 text-xs font-medium text-gray-600 dark:text-gray-300">
                                {sessions.map((s) => (
                                    ...
                                    >
                                        <span
                                            suppressHydrationWarning
                                            className={`w-1.5 h-1.5 rounded-full ${s.status === "Open" ? "bg-green-500" : "bg-gray-400"}`}
                                        ></span>
                                        <span
                                            suppressHydrationWarning
                                            className={
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {nextEvent ? (
                                <div className="flex items-start gap-3">
                                    <div
                                        className={`mt-1 h-2 w-2 rounded-full flex-shrink-0 ${nextEvent.impact === "HIGH" ? "bg-red-500 animate-pulse" : "bg-yellow-500"}`}
                                    ></div>
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-600 mt-0.5">
                                            Happen {timeString} •{" "}
                                            <span
                                                className={`${nextEvent.impact === "HIGH" ? "text-red-500" : "text-yellow-600"}`}
                                            >
