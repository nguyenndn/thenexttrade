...
import { Users, Activity, Crown, Copy, Magnet, ShieldCheck, GraduationCap, BarChart3 } from "lucide-react";
...
export function AdminDashboardClient(props: Props) {
  ...
  return (
    <div className="space-y-4">
      ...
      <CompactStatsRow
        stats={[
          { title: "IB Leads", value: props.ibLeadsCount.toLocaleString(), icon: Magnet, color: "amber" },
          { title: "Pro Users", value: props.proEntitlementsCount.toLocaleString(), icon: ShieldCheck, color: "teal" },
          { title: "Volume", value: `${props.tradingVolume.toFixed(2)} Lots`, icon: BarChart3, color: "indigo" },
          { title: "Lessons", value: props.lessonsCount.toLocaleString(), icon: GraduationCap, color: "rose" },
        ]}
      />
    </div>
  );
