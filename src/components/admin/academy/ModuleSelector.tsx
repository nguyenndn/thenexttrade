    ...
    return (
        <div ref={ref} className={cn("relative", className)}>
            ...
            >
                <div className="flex items-center gap-2 truncate">
                    ...
                    {selected ? (
                        <span className="text-gray-700 dark:text-white truncate">
                            <span className="text-gray-500 text-xs">{selected.levelTitle} →</span>{" "}
                            {selected.title}
                        </span>
                    ) : (
                ...
                <ChevronDown size={14} className={cn("text-gray-500 transition-transform flex-shrink-0", isOpen && "rotate-180")} />
            </button>
                ...
                <div className="absolute z-50 mt-1 w-full bg-white dark:bg-[#1a1e2e] border border-gray-200 dark:border-white/10 rounded-xl shadow-xl overflow-hidden animate-in fade-in-0 zoom-in-95 duration-100">
                    ...
                    <div className="p-2 border-b border-gray-100 dark:border-white/5">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full pl-8 pr-3 py-2 text-sm bg-gray-50 dark:bg-white/5 rounded-lg border-none focus:outline-none text-gray-700 dark:text-white placeholder:text-gray-500"
                            />
                        </div>
                    </div>
                    ...
                    <div className="max-h-64 overflow-y-auto py-1">
                        {Object.keys(filtered).length === 0 ? (
                            <p className="text-sm text-gray-500 text-center py-4">No modules found</p>
                        ) : (
                            Object.entries(filtered).map(([levelTitle, mods]) => (
                                <div key={levelTitle}>
                                    {/* Level group header */}
                                    <div className="px-3 py-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-widest bg-gray-50 dark:bg-white/[0.02] sticky top-0">
                                        {levelTitle}
                                    </div>
                                    {mods.map(mod => (
                                        <button
