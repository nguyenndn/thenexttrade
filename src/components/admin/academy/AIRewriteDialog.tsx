...
export const AIRewriteDialog = forwardRef<
    AIRewriteDialogRef,
...
>(function AIRewriteDialogInner({ onApply, lessonTitle, focusKeyword }, ref) {
    ...
    const [inputMode, setInputMode] = useState<"url" | "paste" | "search">(
        "url"
    );
    ...
    const handleSearch = async () => {
        if (inputMode === "url" && !primaryUrl.trim())
            return toast.warning("Please enter a URL first");
        if (inputMode === "search" && !searchKeyword.trim())
        ...
        try {
            ...
            const res = await fetch("/api/ai/search", {
                ...
                body: JSON.stringify({
                    topic,
                    primaryUrl:
                        inputMode === "url" ? primaryUrl.trim() : undefined,
                }),
            });
    ...
    const handleGenerate = async () => {
        if (inputMode === "url" && !primaryUrl.trim())
            return toast.warning("Please paste a URL");
        if (inputMode === "paste" && !pastedContent.trim())
            return toast.warning("Please paste some content");
        if (inputMode === "search" && selectedUrls.size === 0)
            return toast.warning("Please search and select at least 1 source");
            ...
            const snippetsMap: Record<string, string> | undefined =
                inputMode === "search"
                    ? Object.fromEntries(
                          searchResults
                              .filter(
                                  (r) => selectedUrls.has(r.url) && r.snippet
                              )
                              .map((r) => [r.url, `${r.title}\n\n${r.snippet}`])
                      )
                    : undefined;
                    ...
                    >
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-gray-600">
                                        Multi-source rewrite with copyright
                                        protection
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="p-5 space-y-4 overflow-y-auto flex-1">
                                                        ...
                                                        <button
                                                            ...
                                                            className={cn(
                                                                ...
                                                                selectedUrls.has(
                                                                    ...
                                                                    : "bg-gray-50 dark:bg-white/5 border-gray-150 hover:bg-gray-100 dark:hover:bg-white/10"
                                                            )}
                                                        >
                                                            <div
                                                                className={cn(
                                    ...
                                    {hasSearched &&
                                        searchResults.length === 0 && (
                                            <p className="text-xs text-gray-500 text-center py-2">
                                                No additional sources found. You
                                                can still generate with the
                                                primary URL.
                                            </p>
                                        )}
                                </>
                            ...
                            {inputMode === "search" && (
                                <>
                                    ...
                                    {hasSearched &&
                                        searchResults.length === 0 && (
                                            <p className="text-xs text-gray-500 text-center py-2">
                                                No sources found. Try different
                                                keywords.
                                            </p>
                                        )}
                                </>
                            ...
                            >
                                <Shield size={14} />
                                <span>
                                    Copyright Protection: {copyrightScore}/5
                                    layers
                                </span>
                                {totalSources >= 2 && (
                                ...
                                {inputMode === "search" &&
                                    totalSources === 0 && (
                                        <span className="text-amber-500">
                                            • Search and select sources above
                                        </span>
                                    )}
                                {inputMode === "search" &&
                                    totalSources === 1 && (
                                        <span className="text-amber-500">
                                            • Select more sources for stronger
                                            protection
                                        </span>
                                    )}
                            </div>
                            ...
                            {isLoading && (
                                <div className="flex flex-col items-center justify-center py-8 gap-3">
                                    ...
                                    <p className="text-sm text-gray-600">
                                        {mode === "summary"
                                            ? "Analyzing content..."
                                            : `Rewriting from ${totalSources} source${totalSources > 1 ? "s" : ""}...`}
                                    </p>
                                    <p className="text-xs text-gray-500">
                                        {totalSources > 1
                                            ? "Scraping + merging + rewriting... this may take 30-60 seconds"
                            ...
                            {result && !isLoading && (
                                <div className="space-y-3">
                                    ...
                                    <div className="p-3 bg-gray-50 dark:bg-white/5 rounded-xl border border-gray-200 dark:border-white/10">
                                        ...
                                        <p className="text-base font-bold text-gray-700 dark:text-white mt-1">
                                            {result.title}
                                        </p>
                                    </div>
