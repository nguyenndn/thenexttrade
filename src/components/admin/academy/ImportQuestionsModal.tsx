...
export function ImportQuestionsModal({
    ...
    const [step, setStep] = useState<"quiz-select" | "question-select">(
        "quiz-select"
    );
    ...
    const handleImport = async () => {
        ...
        try {
            const res = await fetch(
                `/api/academy/quizzes/${targetQuizId}/import`,
                {
                    method: "POST",
            ...
            toast.success(
                `Imported ${selectedQuestionIds.size} questions successfully`
            );
            onImportSuccess();
            onClose();
            setStep("quiz-select");
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent
                ...
                className="max-w-2xl h-[80vh] flex flex-col p-0 overflow-hidden bg-white dark:bg-[#151925]"
            >
                <DialogHeader className="p-6 pb-2 shrink-0">
                    <DialogTitle className="text-xl">
                        {step === "quiz-select"
                            ? "Select Source Quiz"
                            : `Import from: ${selectedSourceQuiz?.title}`}
                    </DialogTitle>
                </DialogHeader>
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <div className="p-6 border-t border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex justify-end gap-3">
                                ...
                                >
                                    ...
                                    <span>
                                        Import {selectedQuestionIds.size}{" "}
                                        Questions
                                    </span>
                                </Button>
                            </div>
