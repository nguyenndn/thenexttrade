...
export function CourseBuilder({ level }: CourseBuilderProps) {
    ...
    const handleLessonDragEnd = async (
        ...
        // Sync API
        const reorderData = newLessons.map((item: any, index: number) => ({
            id: item.id,
            order: index,
        }));
    ...
    const handleDeleteLesson = async () => {
        ...
        try {
            const res = await fetch(
                `/api/academy/lessons/${lessonToDelete.lessonId}`,
                {
                    method: "DELETE",
                }
            );
                    ...
                    m.id === lessonToDelete.moduleId
                        ? {
                              ...
                              lessons: m.lessons.filter(
                                  (l: any) => l.id !== lessonToDelete.lessonId
                              ),
                          }
                        : m
                )
 ...
 */}
                                <DndContext
                                    ...
                                    collisionDetection={closestCenter}
                                    onDragEnd={(e) =>
                                        handleLessonDragEnd(e, module.id)
                                    }
                                >
                                    <SortableContext
                                        items={
