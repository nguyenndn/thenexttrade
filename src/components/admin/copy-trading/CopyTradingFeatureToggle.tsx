    ...
    const handleToggle = async () => {
        ...
        try {
            ...
            if (res.ok) {
                setEnabled(newValue);
                toast.success(
                    `Copy Trading ${newValue ? "enabled" : "disabled"} for users`
                );
            } else {
        ...
        <div className="bg-white dark:bg-[#151925] rounded-xl border border-gray-200 dark:border-white/10 p-6">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <div
                        className={`p-3 rounded-xl ${enabled ? "bg-emerald-500/10" : "bg-gray-100 dark:bg-white/5"}`}
                    >
                        <Power
                            size={20}
                            className={
                                enabled ? "text-emerald-500" : "text-gray-400"
                            }
                        />
                    </div>
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                            {enabled
                                ? "Copy Trading page is visible to users"
                                : "Copy Trading page is hidden from users"}
                        </p>
                    </div>
                </div>
