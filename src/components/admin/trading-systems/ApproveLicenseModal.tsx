...
export function ApproveLicenseModal({
    isOpen,
    onClose,
    license,
    adminId,
}: ApproveLicenseModalProps) {
    ...
    const handleApprove = async () => {
        setIsLoading(true);
        try {
            const res = await approveAccount(license.id, {
            ...
            if (res.success) {
                toast.success(
                    `Account ${license.accountNumber} approved successfully`
                );
                toast.info(
                    "Don't forget to add this account to the Google Sheet manually!",
                    {
                        duration: 10000,
                    }
                );
                ...
                <div className="space-y-6 py-4">
                    ...
                    <div className="p-4 bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-100 dark:border-yellow-900/20 rounded-xl flex gap-3 text-sm text-yellow-700 dark:text-yellow-400">
                        ...
                        <div>
                            ...
                            <p>
                                After approving, please manually add this
                                account number to the central Google Sheet for
                                the EA to work.
                            </p>
                        </div>
                    </div>
