...
import { ApproveLicenseModal } from "@/components/admin/trading-systems/ApproveLicenseModal";
import { RejectLicenseModal } from "@/components/admin/trading-systems/RejectLicenseModal";
...
export function PendingLicensesList({
    licenses,
    adminId,
}: PendingLicensesListProps) {
    ...
    const getIBLink = (broker: string) => {
        ...
        if (broker === "VANTAGE")
            return "https://portal.vantagemarkets.com/ib/login";
        return "#";
    };
        ...
        return (
            <div className="text-center py-20 bg-gray-50 dark:bg-white/5 rounded-xl border-2 border-dashed border-gray-200 dark:border-white/10">
                ...
                <p className="text-gray-600 mt-2">
                    There are no pending license requests.
                </p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-4">
            {licenses.map((req) => (
                <Card
                    key={req.id}
                    className="p-6 transition-all hover:shadow-md border-l-4 border-l-yellow-500"
                >
