...
import { createEAProduct, updateEAProduct } from "@/app/admin/trading-systems/actions";
import { uploadEAFile } from "@/app/admin/trading-systems/actions";
...
const productSchema = z.object({
    ...
    version: z
        .string()
        .regex(/^\d+\.\d+\.\d+$/, "Version must be x.y.z (e.g. 1.0.0)"),
    ...
    const handleFileUploads = async (productId: string) => {
        ...
        if (promises.length > 0) {
            ...
            if (failed.length > 0) {
                ...
                toast.error(
                    `Failed to upload ${failed.length} file(s): ${failed.map((f: any) => f.error).join(", ")}`
                );
                return false;
            }
        }
        ...
        try {
            ...
            if (success && productId) {
                ...
                if (uploadSuccess) {
                    ...
                    router.push("/admin/trading-systems/products");
                    router.refresh();
                }
            }
        } catch (err: any) {
        ...
        >
            ...
            {!initialData && (
                <AdminPageHeader
                    ...
                    backHref="/admin/trading-systems/products"
                >
                    <Link href="/admin/trading-systems/products">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                    <div className="bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl p-6 space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[11px] font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wider">
                                Description
                            </label>
                            <RichTextEditor
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl p-6 space-y-4">
                        ...
                        <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-xl text-center hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group relative flex flex-col items-center justify-center min-h-[120px]">
                            <input
                                ...
                                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                                onChange={(e) =>
                                    setMt5File(e.target.files?.[0] || null)
                                }
                            />
                    ...
                    <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-xl flex items-start gap-3 text-gray-600 text-xs border border-gray-200 dark:border-white/10">
                        ...
                        <p>
                            Files are uploaded to secure storage. Uploading a
