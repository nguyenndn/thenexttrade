...
export function BulkImagePromptModal({
    articleIds,
    open,
    onClose,
}: BulkImagePromptModalProps) {
    ...
    const copyAll = () => {
        const text = prompts
            .map((p) => {
                const lines = [
                    ...
                    `## Featured`,
                    p.featured.prompt,
                    `Path: ${p.featured.suggestedPath}`,
                ];
                p.inline.forEach((inl: any, i: number) => {
                    lines.push(
                        `## Inline ${i + 1}`,
                        inl.prompt,
                        `Path: ${inl.suggestedPath}`
                    );
                });
                return lines.join("\n");
            })
        ...
        trackEvent("article_ops_bulk_image_prompts_exported", {
            surface: "article_ops",
            action: "copy_all",
                    ...
                    {phase === "idle" && (
                        <div className="text-center py-12">
                            <p className="text-sm text-gray-500 mb-4">
                                Generate image prompts for {articleIds.length}{" "}
                                selected articles.
                            </p>
                            <Button
                                variant="primary"
                            ...
                            {prompts.length === 0 && (
                                <div className="text-center py-8">
                                    <p className="text-sm text-gray-500">
                                        No prompts generated. Articles may not
                                        exist.
                                    </p>
                                </div>
                            )}
