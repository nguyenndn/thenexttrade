...
import {
    AIRewriteDialog,
    AIRewriteDialogRef,
} from "@/components/admin/academy/AIRewriteDialog";
...
export function ArticleForm({
    ...
    const [autoKeyphrase, setAutoKeyphrase] = useState(
    ...
    const [formData, setFormData] = useState({
        ...
        publishedAt: initialData?.publishedAt
            ...
            : "",
        tags:
            initialData?.tags?.map((t) => t.tagId || t.id) || ([] as string[]),
    ...
    const { getDraft, clearDraft, draftAutoSavedAt } = useAutoSave(
        formData,
        initialData?.id
    );
        ...
        <div className="w-full space-y-6 pb-24">
            ...
            >
                <span className="text-xs font-normal px-2 py-1 rounded-full bg-gray-100 dark:bg-white/5 text-gray-600 flex items-center gap-2">
                    {draftAutoSavedAt && (
                        <span className="flex items-center gap-1 text-gray-500 border-r border-gray-300 pr-2 mr-1">
                            <Save size={12} /> Local:{" "}
                            {draftAutoSavedAt.toLocaleTimeString("en-US")}
                        </span>
                    )}
                    {saveStatus === "saved" && (
                        <span className="flex items-center gap-1 text-green-500">
                            <CheckCircle size={12} /> Cloud Saved
                        </span>
                    )}
                    {saveStatus === "saving" && (
                ...
                <AIRewriteDialog
                    ...
                    onApply={({
                        ...
                        setFormData((prev) => {
                            const expectedOldSlug = prev.title
                                .toLowerCase()
                                .replace(/[^a-z0-9]+/g, "-")
                                .replace(/^-|-$/g, "");
                            const shouldUpdateSlug =
                                !prev.slug || prev.slug === expectedOldSlug;
                            const newSlug = title
                                .toLowerCase()
                                .replace(/[^a-z0-9]+/g, "-")
                                .replace(/^-|-$/g, "");
                ...
                {formData.content &&
                    formData.content.replace(/<[^>]*>?/gm, "").trim().length >
                        50 && (
                        <Button
                            variant="outline"
                            onClick={() =>
                                rewriteRef.current?.openWithContent(
                                    formData.content
                                )
                            }
                ...
                <div className="bg-yellow-50 dark:bg-yellow-500/10 border-l-4 border-yellow-400 p-4 rounded-r-xl flex items-center justify-between mb-6 shadow-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-sm font-medium text-yellow-800 dark:text-yellow-200">
                                Found an unsaved local draft (
                                {localDraftTime?.toLocaleTimeString("en-US")}).
                            </p>
                            <p className="text-xs text-yellow-600 dark:text-yellow-400 mt-0.5">
                                Would you like to restore this draft and
                                continue editing?
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <Button
                            ...
                            className="text-yellow-700 bg-white hover:bg-yellow-100 border-yellow-200"
                        >
                            Discard
                        </Button>
                        <Button
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className="lg:col-span-3 space-y-6">
                    ...
                    <SeoAnalysisPanel
                        focusKeyword={formData.focusKeyword || ""}
                        setFocusKeyword={(val) =>
                            setFormData({ ...formData, focusKeyword: val })
                        }
                        ...
                        onAiGenerate={(field) => {
                            ...
                            if (field === "description") {
                                ...
                                const text = formData.content.replace(
                                    /<[^>]*>?/gm,
                                    ""
                                );
                                const fallbackTitle =
                    ...
                    <div className="bg-white dark:bg-[#151925] rounded-xl border border-gray-200 dark:border-white/10 shadow-sm p-5 space-y-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center border-b border-gray-200 dark:border-white/10 pb-2">
                                <h3 className="font-bold text-gray-700 dark:text-white text-sm uppercase tracking-wider">
                                    Publish Info
                                </h3>
                                <Button
                                    type="button"
 ...
 *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
 ...
 `);
                            ...
                            <FormSelect
                                ...
                                onChange={(val) =>
                                    setFormData({ ...formData, authorId: val })
                                }
                        ...
                        <div className="space-y-4">
                            ...
                            {formData.thumbnail ? (
                                ...
                                >
                                    ...
                                    <p className="text-xs font-bold text-gray-600 group-hover:text-primary transition-colors">
                                        Choose from Media Library
                                    </p>
                                    <p className="text-[10px] text-gray-500 mt-1">
                                        Click to browse your uploads
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="border-b border-gray-200 dark:border-white/10 pb-2 flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    title={
                                        autoExcerpt
                                            ? "Auto-excerpt ON — click to edit manually"
                                            : "Auto-excerpt OFF — click to generate from content"
                                    }
                                >
                                    {autoExcerpt ? (
                                        <Lock size={11} />
                            ...
                            {autoExcerpt && (
                                <p className="text-[10px] text-emerald-600 dark:text-emerald-400 italic">
                                    Auto-generated from first 200 characters of
                                    content
                                </p>
                            )}
                        </div>
                                        ...
                                        <div
                                            ...
                                            className={`text-sm py-1.5 px-3 rounded-lg cursor-default transition-colors hover:bg-gray-50 dark:hover:bg-white/5 ${
                                                heading.level === 1
                                                    ...
                                                    : heading.level === 2
                                                      ? "pl-5 text-gray-700 dark:text-gray-300"
            ...
            <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-[#1E2028] border-t border-gray-200 dark:border-white/10 p-2 flex flex-wrap gap-x-6 gap-y-1 text-xs text-gray-600 dark:text-gray-300 justify-end px-8 z-40">
                ...
                <span
                    className={`flex items-center gap-1 ${readingLevel.color}`}
                >
                    <BookOpen size={14} /> Level: {readingLevel.label} (
                    {readingLevel.score})
                </span>
                {tocHeadings.length > 0 && (
                    <span className="flex items-center gap-1">
                        <ListTree size={14} /> {tocHeadings.length} headings
                    </span>
                )}
            </div>
