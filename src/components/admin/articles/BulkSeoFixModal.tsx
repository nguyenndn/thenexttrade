    ...
    return (
        <div className="text-xs">
            ...
            <div className="grid grid-cols-2 gap-2 mt-0.5">
                <span className="text-red-500/70 line-through line-clamp-1">
                    {current || "—"}
                </span>
...
export function BulkSeoFixModal({
    articleIds,
    open,
    onClose,
    onApplied,
}: BulkSeoFixModalProps) {
    ...
    const [genErrors, setGenErrors] = useState<
        { articleId: string; message: string }[]
    >([]);
    const [phase, setPhase] = useState<"idle" | "review" | "done" | "undone">(
        "idle"
    );
    ...
    const loadSuggestions = async () => {
        ...
        try {
            const result =
                await generateBulkArticleSeoFixSuggestions(articleIds);
                    ...
                    {phase === "idle" && (
                        <div className="text-center py-12">
                            <p className="text-sm text-gray-500 mb-4">
                                Generate SEO fix suggestions for{" "}
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 min-w-0 space-y-1.5">
                                            ...
                                            <FieldDiff
                                                label="Meta Title"
                                                current={s.current.metaTitle}
                                                suggested={
                                                    s.suggested.metaTitle
                                                }
                                            />
                                            <FieldDiff
                                                label="Focus Keyword"
                                                current={s.current.focusKeyword}
                                                suggested={
                                                    s.suggested.focusKeyword
                                                }
                                            />
                                            <FieldDiff
                                                label="Meta Description"
                                                current={
                                                    s.current.metaDescription
                                                }
                                                suggested={
                                                    s.suggested.metaDescription
                                                }
                                            />
                                            <FieldDiff
                                                label="Schema"
                                                current={s.current.schemaType}
                                                suggested={
                                                    s.suggested.schemaType
                                                }
                                            />
                                            <FieldDiff
                                                label="Excerpt"
                                                current={s.current.excerpt}
                                                suggested={s.suggested.excerpt}
                                            />
                                            {s.notes.length > 0 && (
                        ...
                        <div className="text-center py-12 space-y-4">
                            ...
                            {applyResult.errors.length > 0 && (
                                <p className="text-xs text-red-500">
                                    {applyResult.errors.length} articles had
                                    errors
                                </p>
                            )}
                            <Button
                            ...
                            {undoResult.skipped.length > 0 && (
                                <div className="text-left max-w-sm mx-auto mt-2 space-y-1">
                                    <p className="text-xs font-semibold text-amber-600">
                                        {undoResult.skipped.length} skipped:
                                    </p>
