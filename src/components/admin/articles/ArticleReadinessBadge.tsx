...
export function ArticleReadinessBadge({ score, compact }: ArticleReadinessBadgeProps) {
 let color: string;
 let label: string;
 ...
 if (compact) {
 return (
 <span className={`inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1 text-[11px] font-bold ${color}`}>
 {score}
 </span>
 );
 }
 ...
 return (
 <span className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-1.5 text-xs font-bold ${color}`}>
