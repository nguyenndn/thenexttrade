...
export const ArticleList = memo(function ArticleList({
    ...
    const executeBulkAction = async (
        ...
        try {
            ...
            setSelectedIds(new Set());
            toast.success(
                `Successfully processed ${selectedIds.size} articles`
            );
    ...
    const saveQuickEdit = async () => {
        ...
        try {
            ...
            setArticles(
                articles.map((a) =>
                    a.id === quickEditId ? { ...a, ...quickEditData } : a
                )
            );
            setQuickEditId(null);
            router.refresh();
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-white/10">
                            {articles.map((article) => (
                                <tr
                                    key={article.id}
                                    className={`group hover:bg-gray-50 dark:hover:bg-white/[0.01] transition-colors ${selectedIds.has(article.id) ? "bg-primary/5" : ""}`}
                                >
                                    <td className="pl-6 pr-4 py-5 w-14">
                                        <Button
                                            ...
                                            className={`w-5 h-5 min-w-0 min-h-0 p-0 flex items-center justify-center text-gray-500 hover:bg-transparent hover:text-gray-600 ${selectedIds.has(article.id) ? "text-primary" : ""}`}
                                    ...
                                    {quickEditId === article.id ? (
                                        <td
                                            colSpan={5}
                                            className="px-6 py-5 bg-gray-50 dark:bg-white/5"
                                        >
                                            ...
                                            <td className="px-6 py-5">
                                                <div className="flex items-start gap-4">
                                                    ...
                                                    <div>
                                                        <Link
                                                            href={`/admin/articles/${article.id}/edit`}
                                                            className="font-bold text-gray-700 dark:text-white line-clamp-1 group-hover:text-primary transition-colors hover:underline"
                                                        >
                                                            {article.title}
                                                        </Link>
