...
export default function AIStudioLayout() {
    ...
    return (
        <div className="space-y-8 relative">
            ...
            <div className="absolute -top-20 -right-20 w-96 h-96 bg-[#00C888]/10 rounded-full blur-[100px] pointer-events-none" />
...
function TabButton({ active, onClick, label, icon: Icon }: { active: boolean; onClick: () => void; label: string, icon: any }) {
    return (
        <button
            onClick={onClick}
            className={`flex items-center gap-2 px-6 py-2.5 text-sm font-bold rounded-lg transition-all duration-300 ${active
                ? "bg-white dark:bg-[#1E2028] text-[#00C888] shadow-sm shadow-gray-200/50 dark:shadow-none translate-y-0"
                : "text-gray-500 hover:text-gray-900 dark:hover:text-gray-300 hover:bg-black/5 dark:hover:bg-white/5"
                }`}
        >
            <Icon size={18} className={active ? "text-[#00C888]" : "text-gray-400"} />
            {label}
        </button>
    );
}
