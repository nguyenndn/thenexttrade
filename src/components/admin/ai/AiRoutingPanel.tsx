    ...
    <div className="space-y-4">
      {isAdding && (
        <div className="bg-white dark:bg-[#1E2028] border border-gray-200 dark:border-white/10 rounded-xl p-5 shadow-sm">
          <h3 className="text-gray-900 dark:text-white font-medium mb-4">New Routing Policy</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Policy Name</label>
              <input 
            ...
            <div>
              ...
              >
                <option value="FIXED">Fixed (Single Model)</option>
                <option value="AUTO_FAILOVER">Auto Failover</option>
              </select>
            </div>
            <div>
            ...
            <div>
              ...
              <input 
                type="number"
                className="w-full bg-gray-50 dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl px-3 py-2 text-gray-900 dark:text-white text-sm focus:outline-none focus:border-primary"
                value={form.timeoutMs} onChange={e => setForm({...form, timeoutMs: parseInt(e.target.value)})}
              />
            </div>
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setIsAdding(false)} className="px-4 py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">Cancel</button>
            <button onClick={handleCreate} className="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-xl text-sm font-semibold transition-colors shadow-sm">Save Policy</button>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-4">
        {policies.map(policy => (
          <div key={policy.id} className="bg-white dark:bg-[#1E2028] border border-gray-200 dark:border-white/10 rounded-xl p-5 shadow-sm">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-gray-900 dark:text-white font-semibold text-lg">{policy.name}</h3>
                  <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                    <span className="font-mono">v{policy.version}</span>
                    <span>•</span>
                    <span className="bg-gray-100 dark:bg-white/10 px-2 py-0.5 rounded-lg text-xs">{policy.mode}</span>
                    <span>•</span>
                    <span>Scope: {policy.scopeType}</span>
                  </div>
                </div>
              </div>
              <span className={`px-2 py-1 rounded-lg text-xs font-medium flex items-center ${policy.enabled ? 'bg-green-100 dark:bg-green-500/10 text-green-700 dark:text-green-500' : 'bg-gray-100 dark:bg-white/10 text-gray-500 dark:text-gray-400'}`}>
                <Power className="w-3 h-3 mr-1" /> {policy.enabled ? 'Enabled' : 'Disabled'}
              </span>
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-white/10 grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-900 dark:text-white">{policy.timeoutMs}ms</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Max Attempts</p>
                <p className="text-sm text-gray-900 dark:text-white">{policy.maxAttempts}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Last Updated</p>
                <p className="text-sm text-gray-900 dark:text-white">{format(new Date(policy.updatedAt), "MMM d, yyyy HH:mm")}</p>
              </div>
            </div>
          </div>
        ))}
        {policies.length === 0 && (
          <div className="text-center py-12 text-sm text-gray-500 bg-white dark:bg-[#1E2028] rounded-xl border border-gray-200 dark:border-white/10 shadow-sm">
            No routing policies configured. The system will use default routing.
          </div>
        )}
      </div>
    </div>
