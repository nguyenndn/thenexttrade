  ...
  return (
    <div className="bg-white/50 dark:bg-[#1E2028] border border-gray-200 dark:border-white/10 dark:border-white/5 rounded-xl p-5 backdrop-blur-sm shadow-sm">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-sm font-bold text-gray-900 dark:text-white">Trading Anomalies</h2>
      </div>

      <div className="space-y-3">
        {anomalies.length > 0 ? (
          anomalies.map((anomaly, i) => (
            <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-600 dark:text-gray-400 truncate">
                  IP: <span className="font-mono text-gray-900 dark:text-gray-300 font-bold">{anomaly.ip}</span>
                </p>
                {anomaly.detail && (
                  <p className="text-[11px] text-gray-500 mt-1 line-clamp-1">{anomaly.detail}</p>
                )}
              </div>
            </div>
          ))
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-500">No trading anomalies detected.</p>
          </div>
        )}
      </div>
    </div>
