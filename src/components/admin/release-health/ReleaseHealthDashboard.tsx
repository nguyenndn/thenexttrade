...
import {
  FileText, Users, BarChart3, Zap, Activity,
} from "lucide-react";
...
function HealthCard({
  ...
  return (
    <div className="rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-[#151925] p-5">
      ...
      {href && (
        <Link href={href} className="inline-block mt-3 text-xs text-primary font-semibold hover:underline">
          View details →
        </Link>
      )}
    </div>
...
function StatRow({ label, value, warn }: { label: string; value: number | string; warn?: boolean }) {
  return (
    <div className="flex items-center justify-between text-sm">
      ...
      <span className={`font-bold tabular-nums ${warn ? "text-amber-600 dark:text-amber-400" : "text-gray-800 dark:text-white"}`}>
        {value}
      </span>
    </div>
  );
...
function FunnelRow({ label, count, rate, total }: { label: string; count: number; rate: number; total: number }) {
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-sm">
        <span className="text-gray-500 dark:text-gray-400">{label}</span>
        <div className="flex items-center gap-2">
          <span className="font-bold tabular-nums text-gray-800 dark:text-white">{count}</span>
          <span className="text-xs text-gray-400 tabular-nums w-10 text-right">{rate}%</span>
        </div>
      </div>
...
export function ReleaseHealthDashboard({ data }: Props) {
  ...
  const activationStatus: "green" | "amber" | "red" =
    data.activation.newUsersLast7Days === 0 || data.activation.firstTradeRate >= 25
      ? "green"
  ...
  const accountSyncCard = (
    <HealthCard title="Account Sync" status={accountStatus} icon={Activity} href="/admin/ea">
      ...
      <StatRow label="Stale Sync (>24h)" value={data.accounts.staleSync} warn={data.accounts.staleSync > 0} />
      <StatRow label="Never Synced" value={data.accounts.neverSynced} warn={data.accounts.neverSynced > 0} />
    </HealthCard>
  );

  const weeklyReviewsCard = (
    <HealthCard title="Weekly Reviews" status="green" icon={BarChart3} href="/dashboard/reports">
      <StatRow label="Generated (7d)" value={data.reports.weeklyReportsLast7Days} />
      <StatRow
  ...
  const newUsersCard = (
    <HealthCard title="New User Activation" status={activationStatus} icon={Users}>
      {data.activation.newUsersLast7Days === 0 ? (
        <p className="text-xs text-gray-400">No new users in the last 7 days</p>
      ) : (
        <div className="space-y-2.5">
          <FunnelRow label="New Users" count={data.activation.newUsersLast7Days} rate={100} total={data.activation.newUsersLast7Days} />
          <FunnelRow label="→ Added Account" count={data.activation.usersWithAccount} rate={data.activation.accountRate} total={data.activation.newUsersLast7Days} />
          <FunnelRow label="→ First Trade" count={data.activation.usersWithFirstTrade} rate={data.activation.firstTradeRate} total={data.activation.newUsersLast7Days} />
          <FunnelRow label="→ Weekly Review" count={data.activation.usersWithWeeklyReview} rate={data.activation.weeklyReviewRate} total={data.activation.newUsersLast7Days} />
          <FunnelRow label="→ Partner Pro Req" count={data.activation.usersWithPartnerProRequest} rate={data.activation.partnerProRequestRate} total={data.activation.newUsersLast7Days} />
        </div>
      )}
    </HealthCard>
  );
  ...
  const trackingCard = (
    <HealthCard title="Action Tracking (7d)" status="green" icon={Zap} href="/admin/analytics">
      <StatRow label="Activation Clicks" value={data.analytics.activationCtaClicksLast7Days} />
      <StatRow label="Empty State Clicks" value={data.analytics.emptyStateClicksLast7Days} />
      <StatRow label="Mission → Report" value={data.analytics.missionReportCtaClicksLast7Days} />
