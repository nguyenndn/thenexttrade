        ...
        if (type === "lesson") {
            ...
            for (const level of newLevels) {
                ...
                if (module) {
                    ...
                    saveOrder(
                        "lesson",
                        module.lessons.map((l: Lesson, idx: number) => ({
                            id: l.id,
                            order: idx + 1,
                        }))
                    );
            ...
            if (level) {
                const oldIndex = level.modules.findIndex(
                    (m: Module) => m.id === active.id
                );
                const newIndex = level.modules.findIndex(
                ...
                saveOrder(
                    "module",
                    level.modules.map((m: Module, idx: number) => ({
                        id: m.id,
                        order: idx + 1,
                    }))
                );
    ...
    return (
        <div className="grid gap-8">
            {levels.map((level) => (
                <div
                    key={level.id}
                    className="bg-white dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden shadow-sm"
                >
                    ...
                    <div className="p-4 bg-gray-50/50 dark:bg-white/[0.02] border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="font-bold text-lg text-gray-700 dark:text-white">
                                {level.title}
                            </h2>
                        </div>
                    </div>
                    ...
                    <div className="p-4 space-y-4">
                        ...
                        <DndContext
                            ...
                            onDragEnd={(e) =>
                                handleDragEnd(e, level.id, "module")
                            }
                        >
                            <SortableContext
                                items={level.modules.map((m) => m.id)}
