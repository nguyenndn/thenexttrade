...
const stageConfig: Record<
    string,
    { stage: string; action: string; badgeColor: string }
> = {
    NO_ACCOUNT: {
...
export function AdminActivationInboxPanel() {
                ...
                setSignals((prev) =>
                    prev.map((s) =>
                        s.id === sigId
                            ? {
                                  ...s,
                                  metadata: { ...s.metadata, adminNotes: note },
                              }
                            : s
                    )
                );
    ...
    const filteredSignals = signals.filter((sig) => {
        ...
        const matchesStage =
            selectedStage === "ALL" || sig.signalType === selectedStage;
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center justify-between p-4 bg-gray-50 dark:bg-[#151925] border border-gray-200 dark:border-white/10 rounded-xl backdrop-blur-sm">
                ...
                <div className="flex gap-2 w-full sm:w-auto shrink-0 justify-end">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            ...
                            >
                                <span className="truncate">
                                    {selectedStage === "ALL"
                                        ? "All Stuck Stages"
                                        : stageConfig[selectedStage]?.stage ||
                                          selectedStage}
                                </span>
                                <ChevronDown
                                    size={16}
                        ...
                        <DropdownMenuContent
                            align="end"
                            className="w-[240px] rounded-xl border-gray-200 dark:border-white/10 bg-white dark:bg-[#1E2028] shadow-[0_10px_40px_rgba(0,0,0,0.1)] p-1.5"
                        >
                            <DropdownMenuItem
                                ...
                                className={cn(
                                    ...
                                    selectedStage === "ALL" &&
                                        "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 font-bold"
                                )}
                            >
                                All Stuck Stages
                            </DropdownMenuItem>
            ...
            {isLoading ? (
                ...
                <div className="p-6 text-center border border-red-500/10 bg-red-500/5 rounded-xl">
                    ...
                    <Button
                        onClick={loadInbox}
                        className="mt-3 text-xs bg-red-500 hover:bg-red-600 text-white rounded-xl"
                    >
                        Retry
                    </Button>
                </div>
                ...
                <div className="text-center py-16 bg-white dark:bg-[#1E2028] border border-gray-200 dark:border-white/10 rounded-xl shadow-sm">
                    <CheckCircle2
                        size={44}
                        className="mx-auto text-emerald-500 mb-3 animate-pulse"
                    />
                    <h3 className="text-base font-black text-slate-800 dark:text-white">
                        Inbox Completely Clear!
                    </h3>
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 max-w-sm mx-auto">
                        No active users are currently stuck in onboarding or
                        activation pipelines. Keep up the high activation rates!
                    </p>
                </div>
            ) : (
                <div className="space-y-4">
                    {filteredSignals.map((sig) => {
                        const stage = stageConfig[sig.signalType] || {
                            stage: sig.signalType,
                            action: "Send support nudge.",
                            badgeColor:
                                "bg-gray-500/10 text-gray-500 border-gray-500/20",
                        };
                        const severity =
                            severityConfig[sig.severity] || severityConfig.INFO;
                            ...
                            <div
                                ...
                                className={cn(
                                    ...
                                    sig.metadata.adminContactedAt
                                        ? "border-emerald-500/20 dark:border-emerald-500/10 opacity-80"
                                ...
                                <div className="flex flex-col md:flex-row gap-5">
                                    ...
                                    <div className="flex items-start gap-3 w-full md:w-1/4 shrink-0 min-w-0">
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <p className="text-[11px] text-gray-400 dark:text-gray-500 truncate">
                                                {sig.user.email || "No Email"}
                                            </p>
                                            <div className="flex items-center gap-1.5 mt-1">
                                                <span className="text-[9px] font-black tracking-wider uppercase text-amber-600 dark:text-amber-400">
                                                    LV {sig.user.level}
                                                </span>
                                    ...
                                    <div className="flex-1 min-w-0 space-y-2">
                                        <div className="flex flex-wrap items-center gap-2">
                                            <span
                                                className={cn(
                                                    "px-2.5 py-0.5 rounded-lg text-[10px] font-black border uppercase tracking-wider",
                                                    stage.badgeColor
                                                )}
                                            >
                                                {stage.stage}
                                        ...
                                        <div className="flex items-center gap-4 text-[10px] text-gray-400 dark:text-gray-500 pt-1 border-t border-gray-200 dark:border-white/10 dark:border-white/[0.03]">
                                            <span className="flex items-center gap-1">
                                                <Calendar size={11} /> First
                                                seen:{" "}
                                                {format(
                                                    new Date(sig.firstSeenAt),
                                                    "MMM d HH:mm"
                                                )}
                                            </span>
                                            <span className="flex items-center gap-1">
                                    ...
                                    <div className="w-full md:w-1/3 shrink-0 flex flex-col gap-3 justify-between">
                                        <div className="p-3 bg-amber-500/[0.02] dark:bg-white/[0.01] border border-amber-900/[0.04] rounded-xl space-y-1">
                                            <span className="text-[9px] font-black tracking-wider uppercase text-amber-700 dark:text-amber-400/80">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Button
                                                onClick={() =>
                                                    handleSaveNote(sig.id)
                                                }
                                                disabled={
                                                    savingNotes[sig.id] ||
                                                    activeNotes[sig.id] ===
                                                        sig.metadata.adminNotes
                                                }
                                                variant="outline"
                                                size="sm"
                                                className={cn(
