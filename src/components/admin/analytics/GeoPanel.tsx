...
const WorldMap = dynamic(
    () => import("./WorldMap").then((mod) => ({ default: mod.WorldMap })),
    {
        ssr: false,
        loading: () => (
            <div className="w-full h-[260px] bg-gray-100 dark:bg-white/5 rounded-lg animate-pulse" />
        ),
    }
);
...
export function GeoPanel({ countries }: Props) {
    ...
    return (
        <div className="bg-white dark:bg-[#1E2028] rounded-xl border border-gray-200 dark:border-white/10 p-5">
            ...
            <div
                className={`space-y-0.5 overflow-y-auto pr-1 ${view === "map" ? "max-h-[200px]" : "max-h-[400px]"}`}
            >
                {countries.map((c, i) => {
                    const pct =
                        total > 0 ? Math.round((c.views / total) * 100) : 0;
                    return (
                        <div
                            key={c.country}
                ...
                {!countries.length && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-sm text-gray-400">
                            No geo data yet. Data will appear after visitors
                            start arriving.
                        </p>
                    </div>
                )}
