...
import { Plus, Shield, Award, ClipboardCheck, Sparkles, AlertCircle, RefreshCw, BarChart2 } from "lucide-react";
          ...
          {filteredRules.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-[#11141d] rounded-2xl border border-dashboard shadow-sm max-w-xl mx-auto mt-6">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 max-w-xs mx-auto">
                Define entry checklists or risk boundaries. Alternatively, set up starter rules below.
              </p>
        ...
        <TabsContent value="goals">
          {initialGoals.length === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-[#11141d] rounded-2xl border border-dashboard shadow-sm max-w-xl mx-auto">
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 max-w-xs mx-auto">
                Set behavior targets, such as logging a specific number of trades or reviewing losses.
              </p>
              <div className="mt-6">
                <Button
                  variant="primary"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {initialGoals.map((goal) => (
                <GoalCard key={goal.id} goal={goal} onUpdate={() => router.refresh()} />
              ))}
            </div>
          )}
        </TabsContent>
          ...
          {loadingStats ? (
            ...
            <div className="text-center py-12">
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                Log trades and check rules inside your journal to build compliance data.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-[#151925] border border-dashboard/80 dark:border-white/[0.08] p-5 rounded-2xl">
                  ...
                  <div className="divide-y divide-dashboard dark:divide-white/[0.05]">
                    {complianceStats.map((stat) => {
                      const totalChecks = stat.followed + stat.broken + stat.skipped;
                      const rate = totalChecks > 0 ? Math.round((stat.followed / totalChecks) * 100) : 0;
                      return (
                        <div key={stat.ruleId} className="py-3 flex items-center justify-between gap-4">
                          <div className="min-w-0">
                            ...
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 font-semibold mt-0.5">
                              Checked {totalChecks} times • {stat.followed} Followed, {stat.broken} Broken
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className={`text-sm font-black ${rate >= 75 ? "text-emerald-500" : rate >= 50 ? "text-amber-500" : "text-red-500"}`}>
                              {totalChecks > 0 ? `${rate}%` : "—"}
                            </span>
                          </div>
                        </div>
                ...
                <div className="bg-white dark:bg-[#151925] border border-dashboard/80 dark:border-white/[0.08] p-5 rounded-2xl flex flex-col justify-between">
                  <div>
                    ...
                    {(() => {
                      ...
                      const mostBroken = [...complianceStats]
                        .filter((s) => s.broken > 0)
                        ...
                        return (
                          <div className="space-y-3">
                            <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
                              Excellent! You haven't broken any rules yet on checked trades.
                            </p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">
                              Enforce your rules systematically on every trade setup to build data.
                            </p>
                          </div>
                        );
                      }
                        ...
                        <div className="space-y-4">
                          <div className="p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                              Broken <span className="font-bold text-red-500">{mostBroken.broken} times</span>.
                            </p>
                          </div>
                          <p className="text-xs text-gray-600 dark:text-gray-300 leading-relaxed">
                            Focus strictly on this rule during the coming week. Consider setting up a behavior goal in the Goals tab to track this habit.
                          </p>
                        </div>
                      );
                    })()}
