        ...
        try {
            ...
            toast.error(
                error instanceof Error
                    ? error.message
                    : error?.message || "Failed to load psychology data"
            );
        } finally {
            setIsLoading(false);
        }
        ...
        <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                {[
                    ...
                    },
                    {
                        ...
                        value:
                            data.emotionBeforeStats.length > 0
                                ? data.emotionBeforeStats.reduce((a, b) =>
                                      a.totalTrades > b.totalTrades ? a : b
                                  ).emotion
                                : "—",
                        sub: "dominant mood",
                    ...
                    },
                    {
                        ...
                        value: `${
                            data.planAdherenceStats.followed.count +
                                data.planAdherenceStats.notFollowed.count >
                            0
                                ? (
                                      (data.planAdherenceStats.followed.count /
                                          (data.planAdherenceStats.followed
                                              .count +
                                              data.planAdherenceStats
                                                  .notFollowed.count)) *
                                      100
                                  ).toFixed(0)
                                : 0
                    ...
                    return (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                <div
                                    className={`w-9 h-9 shrink-0 rounded-xl flex items-center justify-center ${stat.bg} ${stat.color}`}
                                >
                                    <Icon size={16} aria-hidden="true" />
                                </div>
                                <div>
                                    <p
                                        className={`text-xl font-black tabular-nums leading-none ${stat.color}`}
                                    >
                                        {stat.value}
                                    </p>
                        ...
                        <ul className="text-xs text-red-600 dark:text-red-300 list-disc list-inside space-y-0.5">
                            {data.tiltIndicators.revengeTradeCount > 0 && (
                                <li>
                                    You made{" "}
                                    <strong>
                                        {data.tiltIndicators.revengeTradeCount}{" "}
                                        revenge trade(s)
                                    </strong>
                                    . Stop and reset.
                                </li>
                            ...
                            {data.tiltIndicators.fomoTradeCount > 0 && (
                                <li>
                                    <strong>
                                        {data.tiltIndicators.fomoTradeCount}{" "}
                                        FOMO trade(s)
                                    </strong>{" "}
                                    detected. Stick to your plan.
                                </li>
                            )}
                            {data.tiltIndicators.sizingUpCount > 0 && (
                                <li>
                                    <strong>
                                        {data.tiltIndicators.sizingUpCount}{" "}
                                        trades
                                    </strong>{" "}
                                    with significant size increase after loss
                                    (Martingale behavior?).
                                </li>
                            )}
                            {data.tiltIndicators.avgPnLAfterLoss < 0 && (
                                <li>
                                    Performance drops after losses. Take a break
                                    to reset mental state.
                                </li>
                            )}
                            {data.tiltIndicators.winStreakSizeUp > 0 && (
                                <li>
                                    <strong>
                                        {data.tiltIndicators.winStreakSizeUp}{" "}
                                        trades
                                    </strong>{" "}
                                    with size increase after wins
                                    (Overconfidence).
                                </li>
                            )}
                        </ul>
            ...
            {bestEmotion && (
                <div className="bg-gradient-to-r from-primary/10 to-transparent border border-primary/20 rounded-xl px-4 py-3 flex items-center gap-3">
                    <span className="text-[10px] uppercase tracking-widest text-primary font-black shrink-0">
                        Key Insight
                    </span>
            ...
            {(() => {
                const worstEmotion =
                    data.emotionBeforeStats.length > 1
                        ? data.emotionBeforeStats.reduce((w, c) =>
                              c.winRate < w.winRate ? c : w
                          )
                        : null;
                const bestConfidence =
                    data.confidenceCorrelation.length > 0
                        ? data.confidenceCorrelation.reduce((b, c) =>
                              c.winRate > b.winRate ? c : b
                          )
                        : null;
                            ...
                            worstEmotion.emotion !== bestEmotion.emotion && (
                                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-200/50 dark:border-amber-500/20 text-xs text-gray-600 dark:text-gray-300">
                                    ...
                                    <span>
                                        ...
                                        — only {worstEmotion.winRate.toFixed(0)}
                                        % win rate
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-50 dark:bg-blue-500/10 border border-blue-200/50 dark:border-blue-500/20 text-xs text-gray-600 dark:text-gray-300">
                                ...
                                <span>
                                    ...
                                    <strong className="text-blue-600 dark:text-blue-400">
                                        {bestConfidence.level}/10
                                    </strong>{" "}
                                    — {bestConfidence.winRate.toFixed(0)}% win
                                    rate
                                </span>
                            </div>
                        )}
            ...
            {data.emotionTrend.length >= 2 &&
                (() => {
                    const recent =
                        data.emotionTrend[data.emotionTrend.length - 1];
                    const prev =
                        data.emotionTrend[data.emotionTrend.length - 2];
                    ...
                    return (
                        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 dark:bg-white/5 border border-dashboard/50 text-xs text-gray-600 dark:text-gray-300">
                            ...
                            <span>
                                Win rate {improving ? "improved" : "declined"}{" "}
                                from {prev.winRate.toFixed(0)}% to{" "}
                                <strong
                                    className={
                                        improving
                                            ? "text-primary"
            ...
            {data.planAdherenceStats.followed.count > 0 &&
                data.planAdherenceStats.notFollowed.count > 0 &&
                (() => {
                    const diff =
                        data.planAdherenceStats.followed.winRate -
                        data.planAdherenceStats.notFollowed.winRate;
