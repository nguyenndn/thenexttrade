...
function getCellStyle(slot: HeatmapSlot): string {
    if (slot.trades === 0)
        return "bg-gray-50 dark:bg-white/[0.02] border-dashboard ";
    ...
    return "bg-gray-100 dark:bg-white/5 border-dashboard ";
}
...
export function TradingMoodHeatmap({ data }: TradingMoodHeatmapProps) {
    ...
    return (
        <div className="bg-white dark:bg-[#0B0E14] rounded-xl border border-dashboard shadow-sm border-t-4 border-t-pink-500 h-auto sm:h-[530px] flex flex-col overflow-hidden">
                ...
                {!hasTrades ? (
                    <div className="flex-1 flex items-center justify-center">
                        <ChartEmptyState
                            ...
                            description="Log emotions when entering trades to see patterns."
                        />
                    </div>
                ) : (
                    <div className="flex-1 flex flex-col gap-2">
                            ...
                            {SESSION_META.map((session) => (
                                <div key={session.name} className="text-center">
                                    <div className="flex items-center justify-center gap-1 sm:gap-1.5 mb-0.5">
                                        <span
                                            className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${session.dot}`}
                                        />
                                        <span className="text-[10px] sm:text-xs font-black text-gray-700 dark:text-gray-200 uppercase tracking-wide">
                                            {session.name}
                                        </span>
                                    </div>
                        ...
                        {data.map((row) => (
                            <div
                                key={row.day}
                                className="grid grid-cols-[36px_1fr_1fr_1fr_1fr] sm:grid-cols-[48px_1fr_1fr_1fr_1fr] gap-1 sm:gap-2 flex-1"
                            >
                                ...
                                {row.slots.map((slot) => (
                                    <div
                                        ...
                                        title={
                                            slot.trades > 0
                                                ? `${slot.dominantEmotion} (${slot.trades} trades, ${slot.winRate.toFixed(0)}% WR)`
                                                : "No trades"
                                        }
                                    >
                                        {slot.trades === 0 ? (
