                        ...
                        >
                            <div className="relative w-14 h-14 overflow-hidden rounded-full border-2 border-dashboard group-hover:border-primary transition-all shrink-0 shadow-sm">
                                {article.thumbnail ? (
                                    ...
                                    <div className="w-full h-full bg-gray-100 dark:bg-white/5 flex items-center justify-center text-[10px] text-gray-500">
                                        N/A
                                    </div>
                                )}
                            </div>
...
function TopBrokersWidget() {
    const brokers = partnersData.brokers.items
                    ...
                    {brokers.map((broker: any, i: number) => {
                        ...
                        return (
                            <div key={broker.name} className="group">
                                ...
                                {broker.badge && (
                                    <div
                                        className={`text-center py-1 mb-2.5 rounded-lg text-[9px] font-bold uppercase tracking-wider ${BADGE_STYLES[broker.badgeType || "green"]}`}
                                    >
                                        {broker.badge}
                                    </div>
                                )}
                                ...
                                <div className="flex items-start justify-between mb-2.5 gap-2">
                                    <div className="flex items-center gap-3 min-w-0">
                                        <div className="w-10 h-10 rounded-full bg-gray-50 dark:bg-white flex items-center justify-center overflow-hidden p-[5px] shrink-0 border border-dashboard shadow-sm">
                                            {broker.logo ? (
                                                <Image
                                                    src={broker.logo}
                                                    alt={broker.name}
                                ...
                                {(broker.minDeposit ||
                                    broker.maxLeverage ||
                                    broker.regulation) && (
                                    <div className="grid grid-cols-3 gap-px bg-gray-100 dark:bg-white/5 rounded-lg overflow-hidden border border-dashboard mb-2.5">
                                        {broker.minDeposit && (
                                            <div className="bg-white dark:bg-[#1E2028] py-2 text-center">
                                                ...
                                                <div className="text-[11px] font-bold text-gray-700 dark:text-white mt-0.5">
                                                    {broker.minDeposit}
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {broker.regulation && (
                                            <div className="bg-white dark:bg-[#1E2028] py-2 text-center">
                                                ...
                                                <div className="text-[11px] font-bold text-gray-700 dark:text-white mt-0.5 truncate px-1">
                                                    {broker.regulation}
                                                </div>
                                            </div>
                                        )}
                                            ...
                                            {broker.features
                                                ...
                                                .map(
                                                    (
                                                        ...
                                                        <div
                                                            key={j}
                                                            className="flex items-center gap-1 px-2 py-1 rounded-lg bg-green-50 dark:bg-[#00C888]/10 text-[10px] font-bold text-green-600 dark:text-[#00C888] leading-none"
                                                        >
                                                            <Check
                                                                size={10}
                                                                strokeWidth={3}
                                ...
                                {i < brokers.length - 1 && (
                                    <div className="h-px w-full bg-gray-100 dark:bg-white/5 mt-4"></div>
                                )}
                            </div>
                        );
                    })}
