...
export function HelpfulButton({
    ...
    if (isLoading) {
        if (vertical) {
            return (
                <div className="w-[52px] h-[52px] rounded-full bg-gray-100 dark:bg-white/5 animate-pulse" />
            );
        }
        return (
            ...
            <div className="flex flex-col items-center gap-1.5">
                <Button
                    ...
                    className={`w-[52px] h-[52px] rounded-full flex items-center justify-center shadow-[0_8px_30px_rgb(0,0,0,0.12)] border transition-all duration-300 active:scale-90 disabled:opacity-70 p-0 ${
                        voted
                            ? "bg-gold/15 dark:bg-gold/20 border-gold/40 text-gold hover:bg-gold/20"
                            : "bg-white dark:bg-[#1E2028] border-dashboard text-gray-500 hover:text-gold hover:border-gold/30 hover:bg-white"
                    }`}
                ...
                >
                    <ThumbsUp
                        size={20}
                        strokeWidth={2.5}
                        className={`transition-transform duration-300 ${voted ? "fill-gold" : "hover:scale-110"}`}
                    />
                </Button>
                {count > 0 && (
                    <span
                        className={`text-xs font-bold tabular-nums ${voted ? "text-gold" : "text-gray-600 dark:text-gray-300"}`}
                    >
                        {count}
                    </span>
                )}
    ...
    return (
        <Button
            ...
            className={`group inline-flex items-center gap-2.5 px-5 py-2.5 rounded-full font-bold text-sm transition-all duration-300 border-2 active:scale-95 disabled:opacity-70 ${
                voted
                    ? "bg-gold/15 dark:bg-gold/20 border-gold/40 text-gold hover:bg-gold/20 dark:hover:bg-gold/25"
                    : "bg-white dark:bg-white/5 border-dashboard text-gray-600 dark:text-gray-300 hover:border-gold/30 hover:text-gold hover:bg-gold/5"
            }`}
            title={voted ? "Remove your vote" : "Mark as helpful"}
        >
            <ThumbsUp
                size={16}
                strokeWidth={2.5}
                className={`transition-transform duration-300 ${voted ? "fill-gold" : "group-hover:scale-110"}`}
            />
