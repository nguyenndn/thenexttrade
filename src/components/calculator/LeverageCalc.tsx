...
function getPairContractSize(pair: string): number {
    ...
    if (p.includes("US30") || p.includes("US100") || p.includes("US500"))
        return 1;
...
export function LeverageCalc() {
    ...
    const results = useMemo(() => {
        ...
        const notionalValue =
            inputs.lotSize * contractSize * inputs.currentPrice;
        const effectiveLeverage =
            inputs.accountBalance > 0
                ? notionalValue / inputs.accountBalance
                : 0;
        const marginUsed =
            inputs.accountBalance > 0
                ? (notionalValue / inputs.accountBalance) * 100
                : 0;
    ...
    return (
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
                <div
                    className={cn(
                        "border p-6 rounded-2xl text-center shadow-lg",
                        risk.bg
                    )}
                >
                ...
                <CalcResultGrid cols={3}>
                    <CalcResultCard
                        label="Multiplier"
                        value={`${results.effectiveLeverage.toFixed(1)}x`}
                        valueColor="text-primary"
                    />
                    <CalcResultCard
                        label="Notional"
                        value={`$${results.notionalValue.toLocaleString("en-US", { maximumFractionDigits: 0 })}`}
                    />
                    <CalcResultCard
                        label="1% Move"
                        value={`$${(results.notionalValue * 0.01).toFixed(0)}`}
