...
Có **2 kiểu** Page Header tùy context:

#### A. Full Header (Admin pages — `/admin/*`)
...
#### B. Minimal Header (Dashboard pages — `/dashboard/*`)
**BỎ tiêu đề H1** vì sidebar đã highlight menu hiện tại. BẮT BUỘC dùng component `PageHeader` (`@/components/ui/PageHeader`).

**Component: `PageHeader`**
- Tự động render `<h1 className="sr-only">` cho SEO (qua prop `title`).
- **Description:** Dòng mô tả xanh brand với `border-l-4 border-primary`.
- **Children:** Filter / Button / Badge bên phải (trên tablet+ cùng hàng, mobile stack dọc).
- **Props đặc biệt:**
  - `mobileFullWidthButton` — children chiếm full width trên mobile (`w-full`). Dùng cho nút CTA hoặc DateRangePicker.

**Responsive Behavior:**

| Breakpoint | Layout |
|:---|:---|
| **Mobile** (< 640px) | Stack dọc. Description **full-width background** (`w-full rounded-lg`). Children xuống hàng riêng. |
| **Tablet** (≥ 640px / `sm:`) | Description + Children cùng 1 hàng. Description thu gọn `w-fit rounded-r-lg`. |

```tsx
// Chuẩn — mô tả + filter cùng hàng (tablet+), stack dọc (mobile)
<PageHeader
    title="Journal"
...
// Với button fullwidth trên mobile
<PageHeader
    title="Strategies"
...
**Component: `TabBar` — Responsive `equalWidth`**
Prop `equalWidth` cho phép tabs dàn đều nhau (50% cho 2 tabs, 33% cho 3 tabs):

| Breakpoint | Behavior |
|:---|:---|
| **Mobile/Tablet** (< 1024px) | Tabs dàn đều `flex-1`, container `w-full flex` |
| **Laptop+** (≥ 1024px / `lg:`) | Tabs thu gọn bình thường `lg:flex-none`, container `lg:w-auto lg:inline-flex` |

```tsx
<TabBar tabs={journalTabs} equalWidth />
```

**Component: `DashboardFilter` — Responsive `equalWidth`**
Prop `equalWidth` cho AccountSelector và DateRangePicker chia đều 50% trên tablet+:

```tsx
// 50% mỗi filter block trên tablet
<DashboardFilter currentAccountId={accountId || undefined} equalWidth />

// Order swap trên mobile: dùng className với CSS order
<DashboardFilter
...
| Page | Layout (Mobile → Tablet → Desktop) |
|:---|:---|
| `/accounts` | PageHeader: mô tả + buttons |
| `/strategies` | PageHeader: mô tả + button (`mobileFullWidthButton`) |
...
| `/academy` | PageHeader + 2 badges (50% each via `flex-1 sm:flex-none`) |
| `/psychology` | PageHeader + DateRangePicker (`mobileFullWidthButton`) |
| `/trading-systems` | PageHeader (mô tả only) |
